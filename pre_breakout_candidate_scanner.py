...
class PreBreakoutCandidateScanner:
    ...
    def _compression_structure_features(self, frames: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        for row in reversed(list(c15.tail(30))):
            if recent_low <= safe_float(row, 0.0) <= recent_high:
                ...
                break
        prebreakout_side_hint = "neutral"
        if df5 is not None and not df5.empty and len(df5) >= 25:
            h5 = list(df5["h"].tail(8))
            l5 = list(df5["l"].tail(8))
            h5_up = sum(1 for idx in range(1, len(h5)) if safe_float(h5[idx], 0.0) > safe_float(h5[idx - 1], 0.0))
            l5_up = sum(1 for idx in range(1, len(l5)) if safe_float(l5[idx], 0.0) > safe_float(l5[idx - 1], 0.0))
            h5_dn = sum(1 for idx in range(1, len(h5)) if safe_float(h5[idx], 0.0) < safe_float(h5[idx - 1], 0.0))
            l5_dn = sum(1 for idx in range(1, len(l5)) if safe_float(l5[idx], 0.0) < safe_float(l5[idx - 1], 0.0))
            if higher_lows_count >= 4 and l5_up >= 4 and close_position_in_range >= 0.55:
                prebreakout_side_hint = "long"
            elif lower_highs_count >= 4 and h5_dn >= 4 and close_position_in_range <= 0.45:
                prebreakout_side_hint = "short"
            elif h5_up > h5_dn and l5_up > l5_dn:
                prebreakout_side_hint = "long"
            elif h5_dn > h5_up and l5_dn > l5_up:
                prebreakout_side_hint = "short"
        return {
            ...
            "prebreakout_side_hint": prebreakout_side_hint,
    ...
    def _hard_filters(
        ...
        *,
        ...
        range_high = safe_float(features.get("range_high_15m"), 0.0)
        range_low = safe_float(features.get("range_low_15m"), 0.0)
        if side_hint == "long" and dist_high < 0.15 and close_15m <= range_high:
            reasons.append("long_too_close_to_resistance_without_break")
        if side_hint == "short" and dist_low < 0.15 and close_15m >= range_low:
            reasons.append("short_too_close_to_support_without_break")
        ...
        *,
        ...
        for symbol, market, ticker in scoped_universe:
            try:
                ...
                side_hint = self._side_hint(features, flow, order_book, derivatives)
                ...
                scores = self._score(ticker, features, order_book, flow, derivatives, side_hint, disqualify_reasons)
                prebreakout_score = self._prebreakout_score(scores)
                reason_tags = []
                if safe_float(scores.get("compression_score"), 0.0) >= 60:
                    reason_tags.append("compression")
                if safe_float(scores.get("flow_score"), 0.0) >= 60:
                    reason_tags.append("flow_confirm")
                if side_hint in ("long", "short"):
                    reason_tags.append("side_hint_" + side_hint)
                if bool(order_book.get("ask_wall_getting_thinner")):
                    reason_tags.append("ask_wall_thinner")
                if bool(order_book.get("bid_wall_following_price")):
                    reason_tags.append("bid_wall_following")
                if safe_float(derivatives.get("open_interest_change_pct_15m"), 0.0) > 0:
                    reason_tags.append("oi_rising")
                if disqualify_reasons:
                    reason_tags.extend(["disq_" + str(tag) for tag in disqualify_reasons[:4]])
                raw_payload = {
                    "symbol": symbol,
        ...
        ranked = sorted(
            ...
            key=lambda row: safe_float(row.get("prebreakout_score"), -999.0),
            reverse=True,
        )
            ...
            top_candidates.append(
                {
                    "rank": idx,
                    "symbol": symbol,
                    "side_hint": str(row.get("side_hint") or "neutral"),
                    "prebreakout_score": round(safe_float(row.get("prebreakout_score"), 0.0), 4),
                    "liquidity_score": round(safe_float(row.get("liquidity_score"), 0.0), 4),
                    "compression_score": round(safe_float(row.get("compression_score"), 0.0), 4),
                    "structure_score": round(safe_float(row.get("structure_score"), 0.0), 4),
                    "flow_score": round(safe_float(row.get("flow_score"), 0.0), 4),
                    "derivatives_score": round(safe_float(row.get("derivatives_score"), 0.0), 4),
                    "trigger_distance_score": round(safe_float(row.get("trigger_distance_score"), 0.0), 4),
                    "penalty_score": round(safe_float(row.get("penalty_score"), 0.0), 4),
                    "reason_tags": list(row.get("reason_tags") or [])[:8],
                    "disqualify": False,
                }
            )
        return {
