...
def _build_messages(candidate: Dict[str, Any]) -> list[Dict[str, Any]]:
    ...
    system_text = (
        ...
        'Use numeric evidence first: trend alignment, entry quality, your own calculated reward-to-risk, liquidity/spread, volatility, portfolio exposure, and invalidation distance. '
        ...
        'Never leave the assistant output empty. Even when the setup is unclear, conflicted, or partially noisy, you must still return a complete JSON object with a conservative tactical plan. '
        'All numeric fields must be JSON numbers only, never strings, prose, units, ranges, comparisons, or commentary. Put explanations only in the text fields. '
        'If you are uncertain, answer with should_trade=false and a detailed observation/recheck plan instead of omitting fields or returning a partial object. '
    ...
    user_text = (
        ...
        '- the bot mostly provides organized market data; calculate entry_price, stop_loss, take_profit, and RR yourself from the supplied structure, current price, multi-timeframe pressure, liquidity, and invalidation logic\n'
        '- if the payload includes reference_trade_plan or machine-generated price hints, treat them as low-trust references only; they may be wrong, so do not anchor to them blindly\n'
        '- do not use any provided rr hint as ground truth; compute reward-to-risk from your own chosen entry, stop loss, and take profit\n'
        '- every numeric field in your JSON must be a raw JSON number only; never place text, units, comparisons, or explanations inside numeric fields\n'
        '- if you are uncertain or the payload is noisy, still return the full schema with should_trade=false, a concrete watch plan, and numeric placeholders derived from the nearest valid structure/current price rather than leaving fields blank\n'
        '- never return an empty message, partial object, or commentary outside the JSON object\n'
        '- mirror this schema exactly and fill every key once, even when the setup is poor\n'
        ...
        '- if order_type is limit, you must provide limit_cancel_price, limit_cancel_timeframe, limit_cancel_condition, and limit_cancel_note so the bot can cancel the resting order if the idea is invalidated before fill\n'
        '- if order_type is market, set limit_cancel_price to 0 and limit_cancel_timeframe / limit_cancel_condition / limit_cancel_note to empty strings\n'
        ...
        + '\n'
        'If you choose limit order, entry_price is the limit price you calculate from the structure.\n'
        'If you choose market order, entry_price is the intended market execution anchor around current price that you calculate yourself.\n'
