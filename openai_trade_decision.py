...
def _build_clean_payload(
    ...
    *,
    ...
    expected_side = str((src.get('constraints') or {}).get('expected_side') or '').strip().lower()
    side_lock = bool((src.get('constraints') or {}).get('side_lock', False))
    if leverage_policy:
        constraints['leverage_policy'] = leverage_policy[:40]
    constraints['trade_style'] = trade_style
    if expected_side in {'long', 'short'}:
        constraints['expected_side'] = expected_side
        constraints['side_lock'] = bool(side_lock)
...
def build_candidate_payload(
    *,
    ...
    return {
        ...
        'strategy_lane': 'countertrend_short_reversal' if str(signal.get('candidate_source') or '').lower() == 'short_gainers' else 'trend_following_execution',
        ...
        'constraints': _compact_mapping(
            ...
            [
                ...
                'max_open_positions', 'max_same_direction', 'max_btc_beta_cluster',
                'expected_side', 'side_lock', 'candidate_source', 'strategy_lane', 'aggressive_style', 'anti_chase_rule',
            ],
            text_limit=60,
        ),
...
def _constraint_brief(candidate: Dict[str, Any], constraints: Dict[str, Any]) -> str:
    ...
    expected_side = str(constraints.get('expected_side') or '').strip().lower()
    side_lock = bool(constraints.get('side_lock', False) and expected_side in {'long', 'short'})
    ...
    return (
        ...
        f'expected_side={expected_side or "none"}; '
        f'side_lock={str(side_lock).lower()}'
    )
...
def _build_messages(
    ...
    *,
    ...
    if not strategy_lane:
        strategy_lane = 'countertrend_short_reversal' if candidate_source == 'short_gainers' else 'trend_following_execution'
    lane_instruction = (
        'Source lane: short_gainers countertrend short-reversal mode. Focus on reversal/exhaustion rather than breakout continuation. '
        ...
        if strategy_lane == 'countertrend_short_reversal'
    ...
    if compact:
        ...
        user_text = (
            ...
            'Do not output a no-chase narrative with market entry. If you say no chase / wait pullback / avoid late extension, you must use ENTER_LIMIT with executable entry_price and cancellation rule.\n'
            'For stretched moves, prefer limit entry near meaningful FVG or pullback zone; if no executable limit zone exists, return OBSERVE instead of forcing market entry.\n'
...
def _normalize_decision(raw: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    raw = dict(raw or {})
    raw_side_hint = str(raw.get('trade_side') or raw.get('side') or '').strip().lower()
    ...
    expected_side = str(constraints.get('expected_side') or '').strip().lower()
    side_lock = bool(constraints.get('side_lock', False) and expected_side in {'long', 'short'})
    raw_side_conflict = side_lock and raw_side_hint in {'long', 'short'} and raw_side_hint != expected_side
    if side_lock and expected_side in {'long', 'short'}:
        side = expected_side
    ...
    decision = {
        ...
        'entry_price': _coerce_float(raw.get('entry_price', raw.get('entry', entry_default)), entry_default),
    ...
    if decision['rr_ratio'] <= 0:
        decision['rr_ratio'] = round(abs((decision['take_profit'] - decision['entry_price']) / max(abs(decision['entry_price'] - decision['stop_loss']), 1e-9)), 4)
    if decision['action'] == 'enter' and _looks_like_fake_breakout(decision):
        decision['action'] = 'observe'
        decision['should_trade'] = False
        if decision['timing_state'] not in {'wait_pullback', 'wait_breakout'}:
            decision['timing_state'] = 'wait_breakout'
        if decision['watch_trigger_type'] == 'none':
            decision['watch_trigger_type'] = _watch_trigger_type_from_side_timing(side, decision['timing_state'])
        trap_note = '疑似假突破或延伸失敗，先觀察確認後再進場。'
        reason_to_skip = str(decision.get('reason_to_skip') or '').strip()
        decision['reason_to_skip'] = trap_note if not reason_to_skip else '{} {}'.format(trap_note, reason_to_skip)
        if not str(decision.get('watch_volume_condition') or '').strip():
            decision['watch_volume_condition'] = '需要量能延續，避免假突破後快速衰退。'
        if not str(decision.get('watch_structure_condition') or '').strip():
            decision['watch_structure_condition'] = '等待 15m 收盤維持突破/跌破且不回到箱體內。'
    if side_lock:
        decision['trade_side'] = expected_side
        if raw_side_conflict and decision['action'] == 'enter':
            decision['action'] = 'observe'
            decision['should_trade'] = False
            decision['timing_state'] = 'wait_breakout'
            if decision['watch_trigger_type'] == 'none':
                decision['watch_trigger_type'] = _watch_trigger_type_from_side_timing(expected_side, 'wait_breakout')
            lock_note = '此來源方向已鎖定為 {}，先觀察，不執行反向進場。'.format(expected_side)
            reason_to_skip = str(decision.get('reason_to_skip') or '').strip()
            decision['reason_to_skip'] = lock_note if not reason_to_skip else '{} {}'.format(lock_note, reason_to_skip)
    _enforce_non_chase_entry(decision, candidate, side)
