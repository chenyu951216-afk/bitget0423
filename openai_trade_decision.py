...
def default_trade_config(env_getter: Callable[[str, str], str]) -> Dict[str, Any]:
    ...
    return {
        ...
        'max_output_tokens': max(_env_int(env_getter, 'OPENAI_TRADE_MAX_OUTPUT_TOKENS', 720), 420),
...
def _single_watch_path(text: Any) -> str:
    ...
    if not raw:
        return ''
    parts = re.split(r'\s+or\s+|(?:，|,)\s*or\s+|或者| 或 |，或', raw, maxsplit=1, flags=re.IGNORECASE)
...
def _cn_phrase(text: Any) -> str:
    ...
    lowered = raw.lower()
    mapping = {
        'market does not have trend': '市場目前沒有明確趨勢，偏向區間或混沌。',
...
def build_candidate_payload(
    *,
    ...
    for row in list(top_candidates or [])[:2]:
...
def _build_messages(candidate: Dict[str, Any], *, compact: bool = False) -> list[Dict[str, Any]]:
    ...
    if compact:
        ...
        user_text = (
            ...
            'Required enums: trend_state={trending_up,trending_down,range_mixed,transitioning,trend_unclear}; timing_state={enter_now,wait_pullback,wait_breakout,avoid_near_term}.\n'
            'Strict action map: enter_now=>enter+should_trade=true; wait_pullback/wait_breakout=>observe+should_trade=false; avoid_near_term=>skip+should_trade=false.\n'
        ...
        user_text = (
            ...
            'You must explicitly classify trend_state as trending_up, trending_down, range_mixed, transitioning, or trend_unclear.\n'
            'You must explicitly classify timing_state as enter_now, wait_pullback, wait_breakout, or avoid_near_term.\n'
            'Action mapping is strict: enter_now => action=enter and should_trade=true; wait_pullback/wait_breakout => action=observe and should_trade=false; avoid_near_term => action=skip and should_trade=false.\n'
            ...
            'Also decide whether a scale-in/add-on is recommended. If yes, return scale_in_recommended=true plus scale_in_price, scale_in_qty_pct (0-1), scale_in_condition, and scale_in_note. If not, return false and 0/empty fields.\n'
            ...
            'reason_to_skip must never be generic. State the concrete blockers from the payload, such as stretched entry, missing 5m confirmation, weak volume follow-through, nearby resistance, overheated funding, weak OI/CVD support, or poor liquidity.\n'
            'If trend exists and the setup is still good, prefer enter_now or a precise wait_pullback/wait_breakout plan instead of avoid_near_term.\n'
            ...
            'For observe mode, choose exactly one primary watch path only. Do not use OR, alternatives, or dual scenarios in watch_trigger, watch_note, or recheck_reason.\n'
            'For observe mode, also fill machine-readable tracking fields: watch_trigger_candle, watch_retest_rule, watch_volume_ratio_min, watch_micro_vwap_rule, and watch_micro_ema20_rule.\n'
            ...
            'Use Traditional Chinese for market_read, entry_plan, thesis, reason_to_skip, watch_note, recheck_reason, watch_structure_condition, watch_volume_condition, risk_notes, aggressive_note, limit_cancel_condition, and limit_cancel_note.\n'
            'Candidate payload JSON:\n'
            + json.dumps(candidate, ensure_ascii=False, separators=(',', ':'))
        )
    return [
...
def _fallback_trade_decision(candidate: Dict[str, Any], *, reason: str = '') -> Dict[str, Any]:
    ...
    if side == 'short':
        ...
        watch_note = '先等空方主觸發成立，再重新送 AI；目前不要預判進空。'
        structure_condition = '需要出現跌破或跌破後回測站不上，且短線結構轉弱。'
        volume_condition = '偏好跌破時賣量放大、回抽時買量轉弱。'
        confirmations = [
            '觀察週期收在觸發價下方且無法收回',
            '回測觸發價失敗或形成更低高點',
            '跌破或回測失敗時賣量擴大',
        ...
        reason_to_skip = '目前先不進空，因為還沒有確認跌破或跌破後回測失敗。'
        thesis = '只有在多方延續明確失敗後，這筆逆勢放空才具備執行價值。'
        aggressive_note = '若跌破乾淨且回測持續站不上，才考慮激進空單。'
    else:
        ...
        zone_low = min(watch_trigger_price, entry_price) if watch_trigger_price > 0 and entry_price > 0 else max(watch_trigger_price, 0.0)
        zone_high = max(watch_trigger_price, entry_price) if watch_trigger_price > 0 and entry_price > 0 else max(entry_price, 0.0)
        watch_note = '先等回踩承接或重新站回關鍵價，再重新送 AI；目前不要追價。'
        ...
        reason_to_skip = '目前先不進多，因為回踩承接或重新站回還沒有確認。'
        thesis = '趨勢可能仍有效，但執行點要等結構更乾淨。'
        aggressive_note = '只有重新站回乾淨且參與量改善時，才考慮激進多單。'
    return {
        'should_trade': False,
        'action': 'observe',
        ...
        'trade_side': side,
        'breakout_assessment': '等待確認，暫時保留為觀察計畫。',
        ...
        'market_read': str(ref_context.get('summary') or 'OpenAI 回覆為空，因此先保留成結構化觀察計畫。')[:280],
        'entry_plan': '現在先不要進場，先維持觀察，等觸發條件在指定週期明確出現後再重送 AI。',
        ...
        'if_missed_plan': '如果價格沒有照觸發條件走，不追價，等待下一次乾淨回測或重新失敗訊號。',
        ...
        'risk_notes': [
            '因為 OpenAI 回覆為空，所以先套用保守觀察方案。',
            '在主觸發沒有明確成立前，不要把它轉成實際交易。',
            '等觸發條件出現後，再用同類 payload 重新送審。',
        ],
...
def _build_request_body(
    ...
    *,
    ...
    effective_max_output_tokens = max(
        int(max_output_tokens or config.get('max_output_tokens', 720) or 720),
        420 if structured else 320,
    )
    body = {
...
def _normalize_decision(raw: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if decision['action'] == 'observe':
        ...
        if not decision['watch_structure_condition']:
            decision['watch_structure_condition'] = '需先完成主觸發與回測規則，再重新送 AI。'
        if not decision['watch_volume_condition']:
            decision['watch_volume_condition'] = '需確認量能不是虛破後立刻衰退。'
    ...
    if not decision['entry_plan']:
        timing_map = {
            ...
            'wait_pullback': '先不要追價，等回踩進入計畫區後再執行。',
            'wait_breakout': '先不要進場，等突破或跌破確認後再執行。',
            'avoid_near_term': '這筆近期待機，不適合直接執行。',
        }
    ...
    if not decision['should_trade'] and generic_skip:
        if decision['action'] == 'observe':
            ...
            decision['reason_to_skip'] = ('目前先不進場，等待{}。'.format(blocker[:140]) if blocker else '目前先不進場，還缺少關鍵確認。')
        else:
            blocker = decision['market_read'] or decision['breakout_assessment'] or decision['stop_loss_reason']
            decision['reason_to_skip'] = ('近期待機，不適合執行：{}。'.format(blocker[:140]) if blocker else '近期待機，因為結構與執行品質還沒有對齊。')
...
def consult_trade_decision(
    *,
    ...
    if (
        not force_recheck
        and last_status != 'empty_response'
        and cached_decision
    ...
    if not force_recheck and last_status != 'empty_response' and last_sent_ts > 0 and (now_ts - last_sent_ts) < cooldown_sec:
    ...
    try:
        ...
        max_tokens = max(int(config.get('max_output_tokens', 720) or 720), 420)
        attempts = [
            {
                'model': primary_model,
                'structured': True,
        ...
        if allow_upgrade and upgrade_model and upgrade_model != primary_model and rank <= 1 and score_abs >= max(float(config.get('min_score_abs', 43.0) or 43.0), 52.0):
            attempts.append({'model': upgrade_model, 'structured': True, 'effort': retry_effort, 'max_tokens': min(max_tokens, 380), 'compact_prompt': True})
        if fallback_model and fallback_model != primary_model:
            attempts.append({'model': fallback_model, 'structured': True, 'effort': retry_effort, 'max_tokens': min(max_tokens, 380), 'compact_prompt': True})
        ...
        for attempt_index, attempt in enumerate(attempts):
            ...
            if resp.status_code == 400 and bool(attempt.get('structured', True)):
                ...
                request_body = _build_request_body(
                    ...
                    max_output_tokens=max(380, min(max_tokens, 420)),
                    compact_prompt=True,
                )
                resp = requests.post(base_url, headers=headers, json=request_body, timeout=timeout_sec)
        ...
        if not raw_json:
            ...
            symbol_state.update({
                ...
                'last_decision': dict(fallback_decision),
                'last_status': 'empty_response_fallback',
            ...
            _append_recent(
                ...
                _build_recent_item(
                    ...
                    action=str(fallback_decision.get('action') or 'observe'),
