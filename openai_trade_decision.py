...
def _build_clean_payload(
    ...
    *,
    ...
    orderbook_history = _clean_numeric_mapping(
        dict(market_context.get('orderbook_history') or raw_candidate_payload_clean.get('orderbook_history') or {}),
        [
            'depth_imbalance_change_1m', 'depth_imbalance_change_5m',
            'largest_bid_wall_size_change_3m_pct', 'largest_ask_wall_size_change_3m_pct',
            'bid_wall_following_price', 'ask_wall_getting_thinner', 'wall_pull_or_spoof_risk',
    ...
    flow_context = _clean_numeric_mapping(
        dict(market_context.get('flow_context') or raw_candidate_payload_clean.get('flow_context') or {}),
        [
            'market_buy_notional_1m', 'market_sell_notional_1m',
            'market_buy_sell_ratio_1m', 'market_buy_sell_ratio_5m',
            'cvd_notional_1m', 'cvd_notional_5m',
    ...
    return {
        ...
        'market_context': {
            'basic_market_data': _compact_mapping(
                basic_market_data,
                ['symbol', 'exchange', 'market_type', 'current_price', 'change_24h_pct', 'quote_volume_24h', 'funding_rate', 'open_interest_value_usdt'],
                text_limit=120,
            ),
            'market_state': {
        ...
        'liquidity_context': _compact_mapping(
            liquidity_context,
            ['spread_pct', 'bid_depth_10', 'ask_depth_10', 'depth_imbalance_10', 'largest_bid_wall_price', 'largest_ask_wall_price', 'buy_sell_notional_ratio', 'cvd_notional', 'volume_anomaly_5m', 'volume_anomaly_15m'],
            text_limit=140,
        ),
        ...
        'derivatives_context': _compact_mapping(
            derivatives_context,
            ['funding_rate', 'open_interest_value_usdt', 'open_interest_change_pct_5m', 'open_interest_change_pct_15m', 'open_interest_change_pct_1h', 'basis_pct', 'mark_price', 'index_price'],
            text_limit=140,
        ),
        'risk': {
...
def _build_messages(
    ...
    *,
    ...
    rr_discipline_rules = (
        f'RR discipline: prefer trades with RR >= {max(min_rr_for_entry, 1.35):.2f}. '
        f'If RR is between {max(min_rr_for_entry - 0.20, 1.10):.2f} and {max(min_rr_for_entry, 1.35):.2f}, '
        'only ENTER when structure, invalidation clarity, and liquidity/flow confirmation are all strong. '
        f'If RR is below {max(min_rr_for_entry - 0.20, 1.10):.2f}, default to OBSERVE or SKIP unless an exceptional mispricing edge is obvious.'
    )
    soft_guidance_note = (
    ...
    if compact:
        ...
        user_text = (
            ...
            'Preferred mapping: enter_now=>enter+should_trade=true; wait_pullback/wait_breakout=>observe+should_trade=false; avoid_near_term=>skip+should_trade=false.\n'
...
def _normalize_decision(raw: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    side = str(raw.get('trade_side') or raw.get('direction') or raw.get('side') or '').strip().lower()
    if side == 'buy':
        side = 'long'
    elif side == 'sell':
        side = 'short'
    ...
    decision = {
        ...
        'trade_side': side,
        'breakout_assessment': _txt('breakout_assessment', 180),
        ...
        'market_read': _txt('market_read', 280),
        'entry_plan': _txt('entry_plan', 280),
        ...
        'reason_to_skip': _txt('reason_to_skip', 220),
        ...
        'watch_note': _txt('watch_note', 220),
        'recheck_reason': _txt('recheck_reason', 220),
        ...
        'stop_logic': _txt('stop_logic', 280),
    }

    errors: list[str] = []
    required_keys = ['should_trade', 'action', 'market_regime', 'trend_state', 'timing_state', 'trade_side', 'entry_price', 'stop_loss', 'take_profit', 'order_type', 'bot_instruction']
