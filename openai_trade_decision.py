...
def default_trade_config(env_getter: Callable[[str, str], str]) -> Dict[str, Any]:
    ...
    return {
        ...
        'max_output_tokens': max(_env_int(env_getter, 'OPENAI_TRADE_MAX_OUTPUT_TOKENS', 900), 700),
...
def _build_clean_payload(
    ...
    *,
    ...
    scanner_scores = _clean_numeric_mapping(
        dict(src.get('scanner_scores') or {}),
        [
            'prebreakout_score', 'liquidity_score', 'compression_score', 'structure_score',
            'flow_score', 'derivatives_score', 'trigger_distance_score', 'penalty_score',
        ],
    )
    scanner_features = _clean_numeric_mapping(
        dict(src.get('scanner_features') or {}),
        [
    ...
    clean_payload = {
        ...
        'scanner_intent': _short_text(src.get('scanner_intent') or '', 180),
        ...
        'scanner_note': _short_text(src.get('scanner_note') or '', 260),
        'scanner_rank': int(src.get('scanner_rank', src.get('rank', 1)) or 1),
        'scanner_scores': scanner_scores,
        'scanner_features': scanner_features,
        'side_hint': _short_text(src.get('side_hint') or '', 24),
        'reason_tags': [str(x).strip()[:80] for x in list(src.get('reason_tags') or []) if str(x).strip()][:8],
...
def build_candidate_payload(
    *,
    ...
    compact_market_context = {
        ...
        'signal_context': _compact_mapping(dict(market_context.get('signal_context') or {}), ['current_price', 'atr_15m', 'atr_4h'], text_limit=60),
    ...
    for row in list(top_candidates or [])[:1]:
        compact_top_candidates.append(
            _compact_mapping(
                dict(row or {}),
                ['symbol', 'score', 'priority_score', 'entry_quality', 'rr_ratio', 'candidate_source'],
                text_limit=60,
            )
        )
    ...
    scanner_scores = dict(
        signal.get('scanner_scores')
        or signal.get('prebreakout_scanner_scores')
        or {}
    )
    scanner_features = dict(
        signal.get('scanner_features')
        or signal.get('prebreakout_scanner_features')
        or {}
    )
    ...
    return {
        ...
        'scanner_note': _short_text(signal.get('scanner_note') or '', 260),
        'scanner_rank': int(signal.get('scanner_rank', rank_index + 1) or (rank_index + 1)),
        'side_hint': _short_text(signal.get('side_hint') or signal.get('prebreakout_side_hint') or signal.get('scanner_side') or '', 24),
        'reason_tags': [str(x).strip()[:80] for x in list(signal.get('reason_tags') or signal.get('prebreakout_reason_tags') or []) if str(x).strip()][:8],
        'scanner_scores': _compact_mapping(
            scanner_scores,
            [
                'prebreakout_score', 'liquidity_score', 'compression_score', 'structure_score',
                'flow_score', 'derivatives_score', 'trigger_distance_score', 'penalty_score',
        ...
        'scanner_features': _compact_mapping(
            scanner_features,
            [
                'bb_width_pct_15m', 'bb_width_percentile_15m_120', 'atr_percentile_15m_120',
                'range_tightness_atr_15m', 'range_duration_bars_15m', 'inside_bar_count_15m',
                ...
                'range_high_15m', 'range_low_15m', 'range_mid_15m', 'close_position_in_range_15m',
                'distance_to_range_high_atr15m', 'distance_to_range_low_atr15m',
                'breakout_level', 'breakdown_level', 'prebreakout_side_hint',
                'distance_to_breakout_atr15m', 'distance_to_breakdown_atr15m', 'already_extended_atr15m',
            ],
            text_limit=100,
        ),
        ...
        'constraints': _compact_mapping(
            ...
            [
                ...
                'candidate_source', 'strategy_lane', 'aggressive_style', 'anti_chase_rule',
            ],
            text_limit=60,
        ),
...
def _constraint_brief(candidate: Dict[str, Any], constraints: Dict[str, Any]) -> str:
    ...
    tp_mode = str(constraints.get('execution_tp_mode') or 'local_r_from_ai_stop')
    ...
    *,
    ...
    if not strategy_lane:
        if candidate_source == 'short_gainers':
            ...
            strategy_lane = 'trend_following_execution'
    if strategy_lane in {'countertrend_reversal', 'countertrend_short_reversal'}:
        lane_instruction = (
            'Source lane: short_gainers countertrend reversal mode. Focus on reversal/exhaustion rather than breakout continuation. '
            'Even in aggressive mode, avoid chasing breakdown at the lower extreme; prefer limit reversion entries around structure/FVG retrace.'
        )
    ...
    prebreakout_rules = (
        'If candidate_source=prebreakout_scanner: Treat scanner_rank, scanner_scores, side_hint, and reason_tags as low-trust candidate metadata only. '
        'Do not use them as direct reasons to ENTER, OBSERVE, or SKIP. Re-derive setup from raw OHLCV, multi-timeframe indicators, liquidity_context, orderbook_history, flow_context, derivatives_context, and risk constraints. '
        ...
        'If raw data shows wall pressure, too-close resistance/support, wide spread, unconfirmed flow, or unclear invalidation, return OBSERVE or SKIP even with high scanner score. '
    ...
    if compact:
        system_text = (
            ...
            'Aggressive does not mean always-enter: only ENTER when invalidation, structure edge, and execution price are all clear. '
            ...
            'take_profit in your JSON is analysis-only for trade quality screening; live execution TP/position management is recomputed locally from your stop_loss using local R-based partials and trailing logic (not a fixed RR>=2 gate). '
            + lane_instruction + ' '
            + prebreakout_rules + ' '
        ...
        user_text = (
            ...
            'Use 1D/4H bias and range boundaries, 1H trend quality, 15m execution, 5m confirmation, 1m micro-timing.\n'
            'Ignore all machine-generated scores, grades, setup labels, prior trade plans, timing states, and textual analysis if present.\n'
            'They are low-trust metadata and must not determine the final decision.\n'
            'Derive trend, structure, timing, entry, stop_loss, take_profit, RR, and should_trade only from raw OHLCV, multi-timeframe indicators, liquidity, derivatives, and risk constraints.\n'
            'Apply professional trading common sense: do not long directly into nearby resistance, do not short directly into nearby support, avoid chasing extended candles, and prefer pullback/FVG limit entry when stretched.\n'
            ...
            'For 15m short-term trading, stop_loss must sit at a reasonable break-of-structure invalidation level, not a fragile nearby wick.\n'
            ...
            'Your take_profit is for analysis quality only. The execution bot will recompute live TP locally from your stop_loss.\n'
            'Do not treat lower RR by itself as an automatic no-trade condition; if structure, liquidity, and invalidation are clear, you may still ENTER.\n'
            ...
            'Anti-chase: long near resistance (<0.25 ATR15m before valid break) or short near support (<0.25 ATR15m before valid break) must not use ENTER_MARKET.\n'
            'If narrative says no chase / wait pullback / avoid extension, bot_instruction must be ENTER_LIMIT or OBSERVE, never ENTER_MARKET. pre_breakout_anticipation can only be ENTER_LIMIT or OBSERVE.\n'
            ...
            'reason_to_skip must cite concrete blockers from the payload. market_read and entry_plan must be explicit and data-consistent.\n'
...
def _normalize_decision(raw: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    raw = dict(raw or {})
    raw_side_hint = str(raw.get('trade_side') or raw.get('direction') or raw.get('side') or '').strip().lower()
    if raw_side_hint == 'buy':
        raw_side_hint = 'long'
    elif raw_side_hint == 'sell':
        raw_side_hint = 'short'
    side = raw_side_hint if raw_side_hint in {'long', 'short'} else str(candidate.get('side') or 'long').lower()
    if side not in {'long', 'short'}:
        side = 'long'
    constraints = dict(candidate.get('constraints') or {})
    ...
    inferred_entry = _infer_entry_from_ai_hints(raw, candidate, side, entry_default)
    ...
    decision = {
        ...
        'trade_side': raw_side_hint if raw_side_hint in {'long', 'short'} else side,
        ...
        'entry_price': _coerce_float(raw.get('entry_price', raw.get('entry', inferred_entry)), inferred_entry),
        'candidate_entry_price': _coerce_float(raw.get('candidate_entry_price', raw.get('candidate_entry', 0)), 0.0),
        'stop_loss': _coerce_float(raw.get('stop_loss', raw.get('sl', stop_default)), stop_default),
        'take_profit': _coerce_float(raw.get('take_profit', raw.get('tp', tp_default)), tp_default),
    ...
    if raw_side_hint not in {'long', 'short'}:
        entry_val = _coerce_float(decision.get('entry_price', 0), 0.0)
        stop_val = _coerce_float(decision.get('stop_loss', 0), 0.0)
        tp_val = _coerce_float(decision.get('take_profit', 0), 0.0)
        if entry_val > 0 and stop_val > 0 and tp_val > 0:
            if stop_val < entry_val < tp_val:
                side = 'long'
                decision['trade_side'] = 'long'
            elif tp_val < entry_val < stop_val:
                side = 'short'
                decision['trade_side'] = 'short'
    ...
    instruction = str(decision.get('bot_instruction') or '').upper()
    if instruction == 'ENTER_LIMIT':
        decision['order_type'] = 'limit'
    elif instruction == 'ENTER_MARKET':
    ...
    if decision['timing_state'] == 'enter_now':
        ...
        decision['action'] = 'skip'
        decision['should_trade'] = False
    lock_enter_from_instruction = (
        instruction in {'ENTER_MARKET', 'ENTER_LIMIT'}
        and decision['timing_state'] == 'enter_now'
        and bool(decision.get('should_trade', False))
        and str(decision.get('watch_trigger_type') or 'none') == 'none'
        and not str(decision.get('recheck_reason') or '').strip()
        and str(decision.get('watch_trigger_candle') or 'none') in {'', 'none'}
        and str(decision.get('watch_retest_rule') or 'none') in {'', 'none'}
    )
    if decision['action'] != 'observe' and not lock_enter_from_instruction and _should_promote_to_observe(decision):
        decision['action'] = 'observe'
        decision['should_trade'] = False
        ...
        if decision['watch_trigger_type'] == 'none':
            decision['watch_trigger_type'] = _watch_trigger_type_from_side_timing(side, decision['timing_state'])
    if decision['trade_side'] in {'long', 'short'}:
...
def consult_trade_decision(
    *,
    ...
    try:
        ...
        max_tokens = max(int(config.get('max_output_tokens', 900) or 900), 700)
        attempts = [
            {
                'model': primary_model,
                'structured': True,
