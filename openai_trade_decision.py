...
def default_trade_config(env_getter: Callable[[str, str], str]) -> Dict[str, Any]:
    ...
    return {
        ...
        'model': str(env_getter('OPENAI_TRADE_MODEL', 'gpt-4.1-mini') or 'gpt-4.1-mini').strip(),
        'upgrade_model': str(env_getter('OPENAI_TRADE_UPGRADE_MODEL', 'gpt-4.1') or 'gpt-4.1').strip(),
        'fallback_model': str(env_getter('OPENAI_TRADE_FALLBACK_MODEL', 'gpt-4.1-mini') or 'gpt-4.1-mini').strip(),
...
def _build_clean_payload(
    ...
    *,
    ...
    risk = _clean_numeric_mapping(
        dict(src.get('risk') or market_context.get('risk') or {}),
        [
            'execution_risk_score',
            'atr_pct_15m',
            'spread_cost_pct',
            'leverage_heat_score',
            'oi_change_abs_5m_pct',
            'funding_abs_bps',
        ],
    )
    risk_level = str((src.get('risk') or market_context.get('risk') or {}).get('execution_risk_level') or '').strip()
    if risk_level:
        risk['execution_risk_level'] = risk_level[:20]
    ...
    execution_src = dict(src.get('execution_policy') or market_context.get('execution_policy') or {})
    execution_policy = _clean_numeric_mapping(
        execution_src,
        ['fixed_leverage', 'min_order_margin_usdt', 'fixed_order_notional_usdt', 'margin_pct_range'],
    )
    leverage_mode = str(execution_src.get('leverage_mode') or '').strip()
    if leverage_mode:
    ...
    if raw_candidate_payload:
        raw_candidate_payload_clean = {
            ...
            'orderbook_history': _clean_numeric_mapping(
                ...
                [
                    ...
                    'bid_wall_persistence_sec', 'ask_wall_persistence_sec',
                ],
            ),
            'flow_context': _clean_numeric_mapping(
                ...
                [
                    ...
                    'market_buy_notional_5m', 'market_sell_notional_5m',
                    'market_buy_notional_15m', 'market_sell_notional_15m',
                    'market_buy_sell_ratio_1m', 'market_buy_sell_ratio_5m', 'market_buy_sell_ratio_15m',
                    'cvd_notional_1m', 'cvd_notional_5m', 'cvd_notional_15m',
                    'cvd_slope_1m', 'cvd_slope_5m', 'cvd_slope_15m',
                    'large_trade_count_1m', 'large_buy_count_1m', 'large_sell_count_1m',
                    'absorption_detected', 'exhaustion_detected',
                ],
            ),
            'derivatives_context': _clean_numeric_mapping(
                dict(raw_candidate_payload.get('derivatives_context') or {}),
                [
                    'funding_rate', 'basis_pct', 'open_interest', 'open_interest_value_usdt',
                    'open_interest_change_pct_5m', 'open_interest_change_pct_15m', 'open_interest_change_pct_1h',
                    'long_short_ratio', 'top_trader_long_short_ratio', 'whale_position_change_pct', 'liquidation_volume_24h',
                ],
            ),
            'calculated_metrics': _clean_numeric_mapping(
            ...
            'recent_trades': _clean_numeric_mapping(
                ...
                [
                    'window_sec', 'buy_notional', 'sell_notional',
                    'buy_notional_5m', 'sell_notional_5m',
                    'buy_notional_15m', 'sell_notional_15m',
                    'trade_count',
            ...
            'expansion_state': _clean_numeric_mapping(
                dict(raw_candidate_payload.get('expansion_state') or {}),
                ['compression_score', 'expansion_triggered', 'expansion_strength'],
            ),
            'breakout_quality': _clean_numeric_mapping(
                dict(raw_candidate_payload.get('breakout_quality') or {}),
                ['structure_break', 'volume_confirm', 'cvd_confirm', 'oi_confirm', 'retest_success', 'score'],
            ),
            'move_type': _clean_numeric_mapping(
                dict(raw_candidate_payload.get('move_type') or {}),
                ['confidence'],
            ),
            'resistance_analysis': _clean_numeric_mapping(
                dict(raw_candidate_payload.get('resistance_analysis') or {}),
                ['recent_rejection_count', 'rejection_strength', 'supply_present', 'failed_breakout_recent'],
            ),
            'liquidity_map': _clean_numeric_mapping(
                dict(raw_candidate_payload.get('liquidity_map') or {}),
                ['liquidity_vacuum_up', 'liquidity_vacuum_down'],
            ),
            'fakeout_risk': _clean_numeric_mapping(
                dict(raw_candidate_payload.get('fakeout_risk') or {}),
                ['score'],
            ),
            'decision_context': _clean_numeric_mapping(
                dict(raw_candidate_payload.get('decision_context') or {}),
                ['is_expansion_setup', 'is_pullback_only', 'should_wait_for_breakout'],
            ),
            'timeframe_bars': _compact_timeframe_bars(
        ...
        raw_expansion_state = str((raw_candidate_payload.get('expansion_state') or {}).get('state') or '').strip().lower()
        if raw_expansion_state in ('none', 'pre_breakout', 'expanding', 'exhausted'):
            raw_candidate_payload_clean.setdefault('expansion_state', {})['state'] = raw_expansion_state
        raw_move_type = str((raw_candidate_payload.get('move_type') or {}).get('type') or '').strip().lower()
        if raw_move_type in ('pullback', 'reversal', 'expansion'):
            raw_candidate_payload_clean.setdefault('move_type', {})['type'] = raw_move_type
        raw_res_density = str((raw_candidate_payload.get('liquidity_map') or {}).get('resistance_density') or '').strip().lower()
        raw_sup_density = str((raw_candidate_payload.get('liquidity_map') or {}).get('support_density') or '').strip().lower()
        if raw_res_density in ('low', 'medium', 'high'):
            raw_candidate_payload_clean.setdefault('liquidity_map', {})['resistance_density'] = raw_res_density
        if raw_sup_density in ('low', 'medium', 'high'):
            raw_candidate_payload_clean.setdefault('liquidity_map', {})['support_density'] = raw_sup_density
        raw_fakeout_reasons: list[str] = []
        for item in list((raw_candidate_payload.get('fakeout_risk') or {}).get('reasons') or []):
            text = _short_text(item, 100)
            if text:
                raw_fakeout_reasons.append(text)
        if raw_fakeout_reasons:
            raw_candidate_payload_clean.setdefault('fakeout_risk', {})['reasons'] = raw_fakeout_reasons[:6]
    orderbook_history = _clean_numeric_mapping(
    ...
    large_trade_net_side = str((src.get('flow_context') or market_context.get('flow_context') or raw_candidate_payload_clean.get('flow_context') or {}).get('large_trade_net_side') or '').strip()
    if large_trade_net_side:
        flow_context['large_trade_net_side'] = large_trade_net_side[:16]
    expansion_state_src = dict(src.get('expansion_state') or market_context.get('expansion_state') or raw_candidate_payload_clean.get('expansion_state') or {})
    expansion_state = _clean_numeric_mapping(
        expansion_state_src,
        ['compression_score', 'expansion_triggered', 'expansion_strength'],
    )
    expansion_state_value = str(expansion_state_src.get('state') or '').strip().lower()
    if expansion_state_value not in ('none', 'pre_breakout', 'expanding', 'exhausted'):
        expansion_state_value = 'none'
    expansion_state['state'] = expansion_state_value
    expansion_state = _ensure_keys(expansion_state, ['compression_score', 'expansion_strength'], 0.0)
    expansion_state = _ensure_keys(expansion_state, ['expansion_triggered'], False)

    breakout_quality_src = dict(src.get('breakout_quality') or market_context.get('breakout_quality') or raw_candidate_payload_clean.get('breakout_quality') or {})
    breakout_quality = _clean_numeric_mapping(
        breakout_quality_src,
        ['structure_break', 'volume_confirm', 'cvd_confirm', 'oi_confirm', 'retest_success', 'score'],
    )
    breakout_quality = _ensure_keys(breakout_quality, ['structure_break', 'volume_confirm', 'cvd_confirm', 'oi_confirm', 'retest_success'], False)
    breakout_quality['score'] = int(_coerce_float(breakout_quality_src.get('score', breakout_quality.get('score', 0)), 0.0))

    move_type_src = dict(src.get('move_type') or market_context.get('move_type') or raw_candidate_payload_clean.get('move_type') or {})
    move_type = _clean_numeric_mapping(move_type_src, ['confidence'])
    move_type_value = str(move_type_src.get('type') or '').strip().lower()
    if move_type_value not in ('pullback', 'reversal', 'expansion'):
        move_type_value = 'pullback'
    move_type['type'] = move_type_value
    move_type = _ensure_keys(move_type, ['confidence'], 0.0)

    resistance_analysis = _clean_numeric_mapping(
        dict(src.get('resistance_analysis') or market_context.get('resistance_analysis') or raw_candidate_payload_clean.get('resistance_analysis') or {}),
        ['recent_rejection_count', 'rejection_strength', 'supply_present', 'failed_breakout_recent'],
    )
    resistance_analysis = _ensure_keys(resistance_analysis, ['recent_rejection_count', 'rejection_strength'], 0.0)
    resistance_analysis = _ensure_keys(resistance_analysis, ['supply_present', 'failed_breakout_recent'], False)

    liquidity_map_src = dict(src.get('liquidity_map') or market_context.get('liquidity_map') or raw_candidate_payload_clean.get('liquidity_map') or {})
    liquidity_map = _clean_numeric_mapping(
        liquidity_map_src,
        ['liquidity_vacuum_up', 'liquidity_vacuum_down'],
    )
    resistance_density = str(liquidity_map_src.get('resistance_density') or '').strip().lower()
    support_density = str(liquidity_map_src.get('support_density') or '').strip().lower()
    if resistance_density not in ('low', 'medium', 'high'):
        resistance_density = 'low'
    if support_density not in ('low', 'medium', 'high'):
        support_density = 'low'
    liquidity_map['resistance_density'] = resistance_density
    liquidity_map['support_density'] = support_density
    liquidity_map = _ensure_keys(liquidity_map, ['liquidity_vacuum_up', 'liquidity_vacuum_down'], False)

    fakeout_risk_src = dict(src.get('fakeout_risk') or market_context.get('fakeout_risk') or raw_candidate_payload_clean.get('fakeout_risk') or {})
    fakeout_risk = _clean_numeric_mapping(fakeout_risk_src, ['score'])
    fakeout_score = _coerce_float(fakeout_risk_src.get('score', fakeout_risk.get('score', 0.0)), 0.0)
    fakeout_risk['score'] = _compact_number(max(fakeout_score, 0.0))
    reasons: list[str] = []
    for item in list(fakeout_risk_src.get('reasons') or []):
        text = _short_text(item, 100)
        if text:
            reasons.append(text)
    if not reasons:
        reasons = ['no_major_fakeout_signal']
    fakeout_risk['reasons'] = reasons[:6]

    decision_context = _clean_numeric_mapping(
        dict(src.get('decision_context') or market_context.get('decision_context') or raw_candidate_payload_clean.get('decision_context') or {}),
        ['is_expansion_setup', 'is_pullback_only', 'should_wait_for_breakout'],
    )
    decision_context = _ensure_keys(decision_context, ['is_expansion_setup', 'is_pullback_only', 'should_wait_for_breakout'], False)
    clean_payload = {
        'symbol': symbol,
...
def build_candidate_payload(
    *,
    ...
    if not execution_policy:
        execution_policy = dict(constraints or {})
    ...
    risk_context = dict(market_context.get('risk') or signal.get('risk') or risk_status or {})
    expansion_state = dict(market_context.get('expansion_state') or {})
    breakout_quality = dict(market_context.get('breakout_quality') or {})
    move_type = dict(market_context.get('move_type') or {})
    resistance_analysis = dict(market_context.get('resistance_analysis') or {})
    liquidity_map = dict(market_context.get('liquidity_map') or {})
    fakeout_risk = dict(market_context.get('fakeout_risk') or {})
    decision_context = dict(market_context.get('decision_context') or {})
    ...
    return {
        ...
        'orderbook_history': _compact_mapping(
            orderbook_history,
            ['depth_imbalance_change_1m', 'depth_imbalance_change_5m', 'largest_bid_wall_size_change_3m_pct', 'largest_ask_wall_size_change_3m_pct', 'bid_wall_following_price', 'ask_wall_getting_thinner', 'wall_pull_or_spoof_risk', 'bid_wall_persistence_sec', 'ask_wall_persistence_sec'],
            text_limit=140,
        ),
        'flow_context': _compact_mapping(
            flow_context,
            ['market_buy_notional_1m', 'market_sell_notional_1m', 'market_buy_notional_5m', 'market_sell_notional_5m', 'market_buy_notional_15m', 'market_sell_notional_15m', 'market_buy_sell_ratio_1m', 'market_buy_sell_ratio_5m', 'market_buy_sell_ratio_15m', 'cvd_notional_1m', 'cvd_notional_5m', 'cvd_notional_15m', 'cvd_slope_1m', 'cvd_slope_5m', 'cvd_slope_15m', 'large_trade_count_1m', 'large_buy_count_1m', 'large_sell_count_1m', 'large_trade_net_side', 'absorption_detected', 'exhaustion_detected'],
            text_limit=140,
        ),
        ...
        'risk': _compact_mapping(
            risk_context,
            ['execution_risk_score', 'execution_risk_level', 'atr_pct_15m', 'spread_cost_pct', 'leverage_heat_score', 'oi_change_abs_5m_pct', 'funding_abs_bps'],
            text_limit=100,
        ),
        'expansion_state': _compact_mapping(
            expansion_state,
            ['compression_score', 'expansion_triggered', 'expansion_strength'],
            text_limit=80,
        ) | {'state': _short_text(expansion_state.get('state') or 'none', 20)},
        'breakout_quality': _compact_mapping(
            breakout_quality,
            ['structure_break', 'volume_confirm', 'cvd_confirm', 'oi_confirm', 'retest_success', 'score'],
            text_limit=80,
        ),
        'move_type': _compact_mapping(
            move_type,
            ['confidence'],
            text_limit=80,
        ) | {'type': _short_text(move_type.get('type') or 'pullback', 20)},
        'resistance_analysis': _compact_mapping(
            resistance_analysis,
            ['recent_rejection_count', 'rejection_strength', 'supply_present', 'failed_breakout_recent'],
            text_limit=80,
        ),
        'liquidity_map': _compact_mapping(
            liquidity_map,
            ['liquidity_vacuum_up', 'liquidity_vacuum_down'],
            text_limit=80,
        ) | {
            'resistance_density': _short_text(liquidity_map.get('resistance_density') or 'low', 10),
            'support_density': _short_text(liquidity_map.get('support_density') or 'low', 10),
        },
        'fakeout_risk': _compact_mapping(
            fakeout_risk,
            ['score', 'reasons'],
            text_limit=100,
        ),
        'decision_context': _compact_mapping(
            decision_context,
            ['is_expansion_setup', 'is_pullback_only', 'should_wait_for_breakout'],
            text_limit=80,
        ),
        ...
        'execution_policy': _compact_mapping(
            execution_policy,
            ['fixed_leverage', 'leverage_mode', 'min_order_margin_usdt', 'fixed_order_notional_usdt', 'margin_pct_range'],
            text_limit=100,
        ),
...
def _build_messages(
    ...
    *,
    ...
    system_text = (
        ...
        'You must explicitly evaluate expansion_state, breakout_quality, move_type, resistance_analysis, liquidity_map, fakeout_risk, and decision_context before deciding ENTER or SKIP. '
...
def _build_request_body(
    ...
    *,
    ...
    model_name = str(model or config.get('model') or 'gpt-4.1-mini').strip()
    ...
    # Some gpt-4.1-mini deployments reject verbosity=low.
    if model_lower.startswith('gpt-4.1-mini'):
        verbosity = 'medium'
    body['text'] = {
        'verbosity': verbosity,
        'format': text_format,
    ...
    if effort in {'low', 'medium', 'high'}:
        ...
        # gpt-4.1-mini rejects low/high in some accounts; keep only supported medium.
        if model_lower.startswith('gpt-4.1-mini'):
    ...
    try:
        ...
        try:
            ...
            if status_code == 401:
                ...
                quota_exceeded = 'exceeded your current quota' in str(body_text or '').lower()
                status = 'budget_paused' if quota_exceeded else 'rate_limit'
                if quota_exceeded:
                    detail = 'OpenAI 配額已用盡（429 quota exceeded）。請到平台補齊 billing/額度後再啟用。'
                else:
