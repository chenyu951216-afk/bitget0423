...
def _build_messages(
    ...
    *,
    ...
    rr_discipline_rules = (
        f'RR discipline: prefer trades with RR >= {max(min_rr_for_entry, 1.6):.2f}. '
        f'If RR is between {max(min_rr_for_entry - 0.25, 1.25):.2f} and {max(min_rr_for_entry, 1.6):.2f}, '
        'only ENTER when structure, invalidation clarity, and liquidity/flow confirmation are all strong. '
        f'If RR is below {max(min_rr_for_entry - 0.25, 1.25):.2f}, default to OBSERVE or SKIP unless an exceptional mispricing edge is obvious.'
    )
    if compact:
        system_text = (
