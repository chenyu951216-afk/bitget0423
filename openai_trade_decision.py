...
def build_candidate_payload(
    *,
    ...
    return {
        ...
        'constraints': _compact_mapping(dict(constraints or {}), ['min_margin_pct', 'max_margin_pct', 'min_leverage', 'max_leverage', 'fixed_leverage', 'leverage_policy', 'min_order_margin_usdt', 'fixed_order_notional_usdt', 'trade_style', 'max_open_positions', 'max_same_direction', 'expected_side', 'side_lock'], text_limit=60),
...
def _raw_default_stop_take(candidate: Dict[str, Any], side: str, entry_price: float) -> tuple[float, float]:
    ...
    reward_floor = max(atr_price * 2.0, entry_price * 0.01)
    ...
    if str(side or '').lower() == 'short':
        ...
        if tp_default <= 0:
            tp_default = entry_price - max(risk * 2.2, reward_floor)
        if tp_default >= entry_price:
            tp_default = entry_price - max(risk * 2.2, reward_floor)
        min_rr_take = entry_price - (risk * 2.0)
...
def _constraint_brief(candidate: Dict[str, Any], constraints: Dict[str, Any]) -> str:
    ...
    min_rr = float(constraints.get('min_rr_for_entry', 2.0) or 2.0)
    ...
    *,
    ...
    if compact:
        system_text = (
            ...
            'take_profit in your JSON is analysis-only for trade quality screening; live execution TP is recomputed locally from your stop_loss with R>=2.0. '
        ...
        user_text = (
            ...
            'Your take_profit is for analysis quality only. The execution bot will recompute live TP locally from your stop_loss using R>=2.0.\n'
...
def _should_promote_to_observe(decision: Dict[str, Any]) -> bool:
    ...
    if str(decision.get('watch_retest_rule') or 'none') != 'none':
        return True
    if str(decision.get('recheck_reason') or '').strip():
        return True
    if str(decision.get('watch_structure_condition') or '').strip():
        return True
    if str(decision.get('watch_volume_condition') or '').strip():
        return True
    text = ' '.join(
        [
            str(decision.get('entry_plan') or ''),
...
def _normalize_decision(raw: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if decision['action'] == 'enter' and decision['rr_ratio'] < 2.0:
        decision['action'] = 'observe'
        decision['should_trade'] = False
        decision['timing_state'] = 'wait_breakout'
        if decision['watch_trigger_type'] == 'none':
            decision['watch_trigger_type'] = _watch_trigger_type_from_side_timing(side, 'wait_breakout')
        rr_note = '目前此單 RR {:.2f} 低於 1:2，先觀察。'.format(decision['rr_ratio'])
        reason_to_skip = str(decision.get('reason_to_skip') or '').strip()
        decision['reason_to_skip'] = rr_note if not reason_to_skip else '{} {}'.format(rr_note, reason_to_skip)
        if not str(decision.get('entry_plan') or '').strip():
            decision['entry_plan'] = '目前風險報酬不足 1:2，先觀察結構是否改善後再送 AI。'
