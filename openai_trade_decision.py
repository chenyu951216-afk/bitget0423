...
def default_trade_config(env_getter: Callable[[str, str], str]) -> Dict[str, Any]:
    ...
    return {
        ...
        'max_output_tokens': max(_env_int(env_getter, 'OPENAI_TRADE_MAX_OUTPUT_TOKENS', 420), 240),
...
def _build_messages(
    ...
    *,
    ...
    rr_discipline_rules = (
        ...
        'Prefer RR >= 1.10; RR around 1.05-1.30 can still ENTER when structure, flow, and liquidity confirmation are strong and execution is clean. '
        'If RR is below 1.05, default to OBSERVE or SKIP unless there is exceptional mispricing edge with very clear invalidation.'
    )
    soft_guidance_note = (
...
def consult_trade_decision(
    *,
    ...
    try:
        ...
        max_tokens = max(int(config.get('max_output_tokens', 420) or 420), 240)
        attempts = [
            {
                'model': primary_model,
                'structured': True,
        ...
        for _, attempt in enumerate(attempts):
            ...
            try:
                ...
                if projected_attempt_cost_twd > per_call_budget_twd:
                    ...
                    min_needed_tokens = 180 if bool(attempt.get('structured', True)) else 120
