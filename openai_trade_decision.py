...
def default_trade_config(env_getter: Callable[[str, str], str]) -> Dict[str, Any]:
    ...
    return {
        ...
        'max_output_tokens': max(_env_int(env_getter, 'OPENAI_TRADE_MAX_OUTPUT_TOKENS', 2048), 800),
...
def _build_messages(candidate: Dict[str, Any], *, compact: bool = False) -> list[Dict[str, Any]]:
    ...
    if compact:
        system_text = (
            ...
            'Return one complete JSON object only. Never leave output empty. '
            'All numeric fields must be raw JSON numbers. If the setup is unclear, return should_trade=false with a narrow watch plan. '
            'Be decisive when structure, invalidation, liquidity, and confirmation align; otherwise be precise, not vague.'
        )
        user_text = (
            f'Decide whether to place a {trade_style} crypto perpetual futures order.\n'
            f'Use {fixed_leverage}x leverage and margin_pct between {min_margin_pct:.4f} and {max_margin_pct:.4f}. '
            f'Notional is fixed at {fixed_order_notional_usdt:.4f} USDT.\n'
            'Short-term horizon is 30 minutes to 6 hours. Use 1D/4H for larger bias, 1H for trend quality, 15m for entry, 5m for confirmation, 1m only for micro-timing.\n'
            'Calculate entry_price, stop_loss, take_profit, and RR yourself from the supplied structure/current price/liquidity/invalidation; do not trust machine RR hints.\n'
            'If order_type=limit, also return limit_cancel_price, limit_cancel_timeframe, limit_cancel_condition, limit_cancel_note. If order_type=market, set those limit cancel fields to 0/empty.\n'
            'If not trading, still return the full object with should_trade=false, a concrete watch plan, watch trigger, invalidation, watch timeframe, checklist, confirmations, invalidations, and recheck_reason. The watch trigger must be specific enough that the bot can wait first and only ask again after the condition is clearly met.\n'
            'If force_recheck=true, either upgrade the previous watch idea into an executable plan or explain the single missing factor.\n'
            'If candidate_source=short_gainers, do not short just because price rose; require failed continuation, exhaustion, reclaim failure, VWAP/EMA loss, or breakdown confirmation.\n'
            'JSON shape reminder:\n'
        ...
        user_text = (
            f'Decide whether to place a {trade_style} crypto perpetual futures order.\n'
            f'Hard bounds:\n- leverage must be exactly {fixed_leverage}\n'
            ...
            '- order_type must be market or limit; stop_loss and take_profit are mandatory protection orders\n'
            '- calculate entry_price, stop_loss, take_profit, and RR yourself from supplied structure/current price/liquidity/invalidation; machine price/RR hints are low-trust only\n'
            '- every numeric field must be a raw JSON number only; never put prose into numeric fields\n'
            '- never return an empty message, partial object, or commentary outside the JSON object\n'
            '- if uncertain or payload is noisy, still return the full schema with should_trade=false and a narrow watch plan\n'
            '- analyze multi-timeframe like this: 1D/4H larger bias, 1H trend quality, 15m main entry frame, 5m confirmation, 1m micro-timing only\n'
            '- optimize for holding windows around 30 minutes to 6 hours\n'
            '- use timeframe_bars, multi_timeframe summaries, liquidity_context, derivatives_context, and news_context together; do not rely on only one block\n'
            '- market_read must explicitly mention trend state, key support/resistance, and fake-breakout risk\n'
            '- choose one best executable path now: market now, precise limit pullback/retest, or no trade yet with a precise recheck trigger\n'
            '- be slightly selective, not overly selective: if the thesis is still good but price is stretched, prefer a better limit entry or confirmation trigger instead of blanket rejection\n'
            '- if structure, invalidation, and confirmation already align, do not hide behind vague caution\n'
            '- if order_type is limit, also return limit_cancel_price, limit_cancel_timeframe, limit_cancel_condition, and limit_cancel_note so the bot can cancel before fill if invalidated; for market, set those to 0/empty\n'
            '- if should_trade is false, preserve the idea with watch_trigger_type, watch_trigger_price, watch_invalidation_price, watch_note, recheck_reason, watch_timeframe, watch_price_zone_low/high, watch_structure_condition, watch_volume_condition, watch_checklist, watch_confirmations, watch_invalidations, and watch_recheck_priority\n'
            '- if should_trade is false, the watch plan must be precise enough for the bot to wait first and re-ask only after the condition is clearly met; do not give vague observation instructions\n'
            '- if force_recheck is true, the bot is returning because the previous watch condition appears met; either upgrade to an executable plan or explain the one missing factor still preventing execution\n'
            ...
            '- if candidate_source is short_gainers, do not approve only because price rose; require failed continuation, exhaustion wick, lower high, VWAP/EMA loss, breakdown confirmation, or another invalidation-based trigger\n'
            'JSON shape reminder:\n'
            + _response_shape_hint()
            + '\nCandidate payload JSON:\n'
            + json.dumps(candidate, ensure_ascii=False, separators=(',', ':'))
    ...
    *,
    ...
    try:
        ...
        attempts = [
            ...
            {'model': primary_model, 'structured': False, 'effort': retry_effort, 'max_tokens': min(max_tokens, 1280), 'compact_prompt': True},
        ]
        if allow_upgrade and upgrade_model and upgrade_model != primary_model and rank <= 1 and score_abs >= max(float(config.get('min_score_abs', 38.0) or 38.0), 52.0):
            attempts.append({'model': upgrade_model, 'structured': False, 'effort': retry_effort, 'max_tokens': min(max_tokens, 1280), 'compact_prompt': True})
        if fallback_model and fallback_model != primary_model:
            attempts.append({'model': fallback_model, 'structured': False, 'effort': retry_effort, 'max_tokens': min(max_tokens, 1280), 'compact_prompt': True})
        ...
        for attempt_index, attempt in enumerate(attempts):
            ...
            if resp.status_code == 400 and bool(attempt.get('structured', True)):
                ...
                request_body = _build_request_body(
                    ...
                    max_output_tokens=min(max_tokens, 1280),
                    compact_prompt=True,
                )
                resp = requests.post(base_url, headers=headers, json=request_body, timeout=timeout_sec)
