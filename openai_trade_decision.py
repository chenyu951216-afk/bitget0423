...
def default_trade_config(env_getter: Callable[[str, str], str]) -> Dict[str, Any]:
    ...
    return {
        ...
        'max_output_tokens': max(_env_int(env_getter, 'OPENAI_TRADE_MAX_OUTPUT_TOKENS', 420), 280),
    ...
    *,
    ...
    if compact:
        system_text = (
            ...
            'User preference: prioritize asymmetric PnL (RR-first, win-rate secondary), so you may be aggressive when edge is clear; however never auto-agree with noisy/random candidate picks. '
            'Local bot may only execute and later trail/breakeven; it must not invent initial entry/stop/take-profit. '
            'Do not output bot_note; use market_read + entry_plan + stop_logic + recheck_reason instead. '
            '15m stop must be structural invalidation from prior closed swings (roughly 3-12 bars), with anti-sweep buffer around 0.15-0.45 ATR15. '
            'OBSERVE must set entry_price=0, stop_loss=0, take_profit=0 and include exactly one machine-watch path. '
            ...
            'action must be lower-case (enter/observe/skip), order_type lower-case (market/limit), bot_instruction one of ENTER_MARKET/ENTER_LIMIT/OBSERVE/SKIP. '
            + lane_instruction + ' '
            + prebreakout_rules + ' '
            + rr_discipline_rules + ' '
        ...
        user_text = (
            ...
            'Evaluate three archetypes: trend continuation/pullback, reversal reclaim, pre-breakout release. Pick best executable edge now.\n'
            'Do not chase into nearby resistance/support. If waiting is required, return OBSERVE with one precise trigger path only.\n'
            ...
            'For OBSERVE enforce entry_price=0, stop_loss=0, take_profit=0.\n'
            'Keep analysis concise but concrete in market_read, entry_plan, stop_logic, recheck_reason.\n'
...
def consult_trade_decision(
    *,
    ...
    estimated_output_tokens = max(int(config.get('max_output_tokens', 420) or 420), 220)
    ...
    try:
        ...
        max_tokens = max(int(config.get('max_output_tokens', 420) or 420), 220)
        attempts = [
            {
                'model': primary_model,
                'structured': False,
