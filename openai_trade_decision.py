...
def _sanitize_same_bar_stop(decision: Dict[str, Any], candidate: Dict[str, Any]) -> None:
    ...
    allow_same_bar = bool(reference_trade_plan.get('machine_stop_same_bar_exception', False))
    guard = _candidate_stop_guard(candidate, entry_price)
    if allow_same_bar or bool(guard.get('explosive', False)):
        return
...
def _build_messages(candidate: Dict[str, Any], *, compact: bool = False) -> list[Dict[str, Any]]:
    ...
    if compact:
        system_text = (
            ...
            'Tighten only slightly: if the setup is borderline because it is extended, the retest has not held, or the 5m/1m confirmation is still incomplete, prefer one precise observe trigger instead of forcing entry. '
        ...
        system_text = (
            ...
            'Tighten only slightly: for borderline setups that are already stretched, still waiting on a retest, or missing one last 5m/1m confirmation, prefer a precise observe plan instead of forcing entry. '
        ...
        user_text = (
            ...
            'Raise the bar only slightly: if the setup is marginal because it is extended, the retest/reclaim is unfinished, or short-term follow-through is not clean yet, prefer observe with one exact trigger. Do not turn clean aligned setups into unnecessary skips.\n'
...
def _fallback_trade_decision(candidate: Dict[str, Any], *, reason: str = '') -> Dict[str, Any]:
    ...
    return {
        ...
        'stop_loss_reason': '失效價優先放在 15m 前幾根有意義的 swing 高低點外側並保留小緩衝；只有明顯急拉急殺例外才會參考當前K棒極值。',
...
def consult_trade_decision(
    *,
    ...
    if not force_recheck and score_abs < float(config.get('min_score_abs', 0.0) or 0.0):
    ...
    if spent_twd >= soft_budget_twd and score_abs < max(float(config.get('min_score_abs', 0.0) or 0.0) + 8.0, 52.0):
        result = {'status': 'budget_paused', 'decision': None, 'payload_hash': payload_hash, 'symbol_state': symbol_state}
