...
def _raw_default_stop_take(candidate: Dict[str, Any], side: str, entry_price: float) -> tuple[float, float]:
    ...
    explosive = bool(tf15.get('xr', False))
    min_gap = max(atr_price * 1.25, entry_price * 0.0065)
    max_gap = max(atr_price * (2.8 if explosive else 3.4), min_gap * 1.35)
...
def _sanitize_same_bar_stop(decision: Dict[str, Any], candidate: Dict[str, Any]) -> None:
    ...
    anti_sweep_buffer = max(atr_price * 0.28, entry_price * 0.002)
    min_gap = max(atr_price * 1.2, entry_price * 0.006)
    max_gap = max(
        atr_price * (2.8 if bool(guard.get('explosive', False)) else 3.6),
        min_gap * 1.3,
    )
    ...
    *,
    ...
    if not strategy_lane:
        if candidate_source == 'short_gainers':
            ...
            strategy_lane = 'trend_following_execution'
    lane_instruction = 'lane=general'
    if strategy_lane in {'countertrend_reversal', 'countertrend_short_reversal', 'gainer_followup_discovery'}:
        lane_instruction = 'lane=short_gainers_followup'
    elif strategy_lane == 'pre_breakout_discovery':
        lane_instruction = 'lane=prebreakout_discovery'
    if compact:
        system_text = (
            'You are the only trade-decision brain; local bot is data/execution only. '
            'Output exactly one compact JSON object (no markdown). Numeric fields must be JSON numbers; human-readable fields in Traditional Chinese. '
            'Decide only from raw payload; ignore machine scores/labels/prior plans. '
            'User preference: RR/PnL first, win-rate secondary; aggressive but never random. '
            'Counter-trend is allowed when short-term structure/flow + invalidation are clear; do not auto-skip only due opposite 1D/4H. '
            'Required fields: should_trade,action,market_regime,trend_state,timing_state,trade_side,order_type,bot_instruction,entry_price,stop_loss,take_profit,rr_ratio. '
            'Enums: action=enter|skip; trade_side=long|short|neutral; order_type=market|limit; bot_instruction=ENTER_MARKET|ENTER_LIMIT|SKIP; '
            'market_regime=trend_continuation|trend_pullback|range_reversion|consolidation_squeeze|transition_chop; trend_state=trending_up|trending_down|range_mixed|transitioning|trend_unclear; timing_state=enter_now|avoid_near_term. '
            'OBSERVE is disabled. Never output observe/OBSERVE/wait_* states. '
            'Contract: enter => should_trade=true and entry_price>0 stop_loss>0 take_profit>0 with trade_side long/short. '
            'skip => should_trade=false and entry_price=stop_loss=take_profit=0. '
            'When action=skip, reason_to_skip must be a concrete short sentence (<=120 Chinese chars). '
            'For enter: stop_loss must be structural invalidation from prior closed 15m swings + anti-sweep buffer around 0.20-0.50 ATR15. '
            'Aggressive is allowed but do not chase pumps/dumps; prefer pullback or reclaim entries. '
            'Avoid long into nearby resistance / short into nearby support / late chase. '
            'If price is too close to resistance (for long) or too close to support (for short), do SKIP. '
            'Near-level rule: if long and distance to nearest resistance is <0.25 ATR15, SKIP; '
            'if short and distance to nearest support is <0.25 ATR15, SKIP. '
            'Keep strings concise and executable (prefer <=120 chars each field).'
        )
    else:
        system_text = (
            'You are the only trade-decision brain; local bot is data/execution only. '
            'Return exactly one compact JSON object, no markdown. Numeric fields must be JSON numbers. '
            'Human-readable fields must be Traditional Chinese. '
            'User preference: RR/PnL first, win-rate secondary; can be aggressive but never random. '
            'Counter-trend trading is allowed when short-term structure/flow/invalidations are clear; do not auto-skip only because 1D/4H trend is opposite. '
            'Ignore machine-generated scores/labels/prior plans; decide only from raw payload market data. '
            'Local bot must receive explicit machine command only: no-trade, market-entry, or limit-entry. '
            'Required fields: should_trade,action,market_regime,trend_state,timing_state,trade_side,order_type,bot_instruction,entry_price,stop_loss,take_profit,rr_ratio. '
            'Allowed enums: action=enter|skip; trade_side=long|short|neutral; order_type=market|limit; '
            'bot_instruction=ENTER_MARKET|ENTER_LIMIT|SKIP; '
            'market_regime=trend_continuation|trend_pullback|range_reversion|consolidation_squeeze|transition_chop; '
            'trend_state=trending_up|trending_down|range_mixed|transitioning|trend_unclear; '
            'timing_state=enter_now|avoid_near_term. '
            'OBSERVE is disabled. Never output observe/OBSERVE/wait_* states. '
            'Hard execution contract: if action=enter then should_trade=true and provide entry_price>0, stop_loss>0, take_profit>0, trade_side long/short. '
            'If action=skip then should_trade=false and entry_price=stop_loss=take_profit=0. '
            'If action=skip, reason_to_skip must be concrete and concise (<=120 Chinese chars). '
            'For enter decisions, stop_loss must be structural invalidation from prior closed 15m swings plus anti-sweep buffer (roughly 0.20-0.50 ATR15, prefer not too tight to reduce sweep-out). '
            'Aggressive entries are allowed, but never chase pumps/dumps. '
            'Do not long directly into nearby resistance and do not short directly into nearby support; avoid late chase. '
            'If price is too close to resistance/support to keep clean execution space, choose SKIP. '
            'Near-level rule: long with resistance distance <0.25 ATR15 => SKIP; short with support distance <0.25 ATR15 => SKIP. '
            'Aggressive but filtered: if executable edge is clear (structure + liquidity/flow + RR + clear invalidation), prefer ENTER instead of over-waiting.'
        )
    lane_rule = ''
    if candidate_source == 'prebreakout_scanner':
        lane_rule = (
            'prebreakout_scanner is candidate-selection metadata only; '
            'it cannot decide direction/entry/stop/tp/rr. Re-derive fully from raw data.'
        )
    elif candidate_source == 'short_gainers':
        lane_rule = (
            'short_gainers only means 24h movers selection; do not assume reversal or continuation by default. '
            'Re-derive direction from structure/flow.'
        )
    if compact:
        user_text = (
            f'Generate one {trade_style} tactical execution-analysis JSON object.\n'
            f'Constraints (soft): {constraint_brief}.\n'
            f'{lane_instruction}. {lane_rule}\n'
            'Use 1D/4H bias + 1H trend + 15m execution + 5m/1m timing.\n'
            'HTF bias is guidance not veto. Counter-trend is allowed with clear rejection/reclaim + flow turn + executable stop + acceptable RR.\n'
            'Aggressive but no chase: avoid buying near pressure and avoid selling near support; too close to pressure/support => SKIP.\n'
            'Distance guard: long if distance to nearest resistance <0.25 ATR15 => SKIP; short if distance to nearest support <0.25 ATR15 => SKIP.\n'
            'Evaluate: trend continuation/pullback, reversal reclaim, pre-breakout release; choose best executable edge now.\n'
            'If executable edge is clear now => ENTER (market/limit). Otherwise => SKIP (observe disabled).\n'
            'ENTER must include concise market_read, entry_plan, stop_logic, stop_anchor_timeframe, stop_anchor_source, stop_anchor_price, stop_buffer_atr15, entry_to_stop_atr15.\n'
            'SKIP must keep entry_price/stop_loss/take_profit all 0 and provide concrete reason_to_skip.\n'
            'Output only JSON object, one line, no extra text.\n'
        ...
        user_text = (
            ...
            'Higher-timeframe bias is guidance, not veto: do not skip solely because it conflicts with a valid short-term setup.\n'
            'Counter-trend entries are allowed when there is clear rejection/reclaim structure, short-term flow turn, executable stop, and acceptable RR.\n'
            'Aggressive but no chase: do not chase breakout candles blindly; if too close to nearby pressure/support and execution space is poor, return SKIP.\n'
            'Distance guard: long if distance to nearest resistance <0.25 ATR15 => SKIP; short if distance to nearest support <0.25 ATR15 => SKIP.\n'
            'Prioritize RR and risk quality together: do not enter only because RR looks good on paper; confirm with current short-term candles and live metrics.\n'
            'Evaluate three archetypes then choose the clearest executable edge now: trend continuation/pullback, exhaustion reversal/reclaim, pre-breakout release.\n'
            'If edge is clear and executable now, return ENTER (market or executable limit). If not qualified, return SKIP only (observe is disabled).\n'
            'For ENTER include concise market_read, entry_plan, stop_logic, stop_anchor_timeframe, stop_anchor_source, stop_anchor_price, stop_buffer_atr15, entry_to_stop_atr15.\n'
            'For SKIP keep entry_price/stop_loss/take_profit all 0 and write concrete reason_to_skip.\n'
            'Candidate payload JSON:\n'
            + json.dumps(clean_payload, ensure_ascii=False, separators=(',', ':'))
        )
    return [
