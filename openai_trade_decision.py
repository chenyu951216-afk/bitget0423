...
def _normalize_decision(raw: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if decision['watch_trigger_type'] == 'none':
        watch_trigger_text = str(raw.get('watch_trigger') or '').lower()
    ...
    if not decision['entry_plan']:
        timing_map = {
            'enter_now': 'Current structure is actionable now; execute with the supplied entry and invalidation.',
            'wait_pullback': 'Do not chase now; wait for pullback into the planned entry zone before executing.',
            'wait_breakout': 'Do not enter yet; wait for breakout/breakdown confirmation before executing.',
            'avoid_near_term': 'Avoid this setup for now; near-term structure is not good enough for execution.',
        }
    ...
    if not decision['should_trade'] and generic_skip:
        if decision['action'] == 'observe':
            ...
            decision['reason_to_skip'] = ('Not entering yet: wait for {}.'.format(blocker[:140]) if blocker else 'Not entering yet: a key confirmation is still missing.')
        else:
            blocker = decision['market_read'] or decision['breakout_assessment'] or decision['stop_loss_reason']
            decision['reason_to_skip'] = ('Near-term not suitable: {}.'.format(blocker[:140]) if blocker else 'Near-term not suitable because structure and execution quality do not align.')
