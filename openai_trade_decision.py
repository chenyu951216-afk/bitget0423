...
def _build_messages(
    ...
    *,
    ...
    system_text = (
        ...
        'Allowed enums: action=enter|observe|skip; trade_side=long|short|neutral; order_type=market|limit; '
        'bot_instruction=ENTER_MARKET|ENTER_LIMIT|OBSERVE|SKIP; '
        ...
        'timing_state=enter_now|wait_pullback|wait_breakout|avoid_near_term. '
        ...
        'If action is observe/skip then should_trade=false and entry_price=stop_loss=take_profit=0. '
        'For enter decisions, stop_loss must be structural invalidation from prior closed 15m swings plus anti-sweep buffer (roughly 0.15-0.45 ATR15). '
    ...
    user_text = (
        ...
        'If edge is clear and executable now, prefer ENTER (market or executable limit). If not ready, return OBSERVE or SKIP with concrete reason.\n'
        'For ENTER include concise market_read, entry_plan, stop_logic, recheck_reason, stop_anchor_timeframe, stop_anchor_source, stop_anchor_price, stop_buffer_atr15, entry_to_stop_atr15.\n'
        'For OBSERVE keep one precise watch path (single primary trigger), but still keep entry_price/stop_loss/take_profit all 0.\n'
...
def _normalize_decision(raw: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _normalize_action(value: Any) -> str:
        ...
        if text.startswith('enter') or '進場' in text or '入場' in text:
            return 'enter'
        if text.startswith('obs') or 'observe' in text or '觀察' in text or '等待' in text:
            return 'observe'
        if text.startswith('skip') or '略過' in text or '不做' in text or '放棄' in text:
            return 'skip'
        return ''
    ...
    def _normalize_bot_instruction(value: Any) -> str:
        ...
        if text.startswith('ENTER_MARKET') or text in {'MARKET', 'ENTRY_MARKET', 'ENTERM'}:
            return 'ENTER_MARKET'
        if text.startswith('OBS') or text in {'WAIT', 'WATCH'}:
            return 'OBSERVE'
        if text.startswith('SKIP') or text in {'PASS'}:
            return 'SKIP'
        return ''
    ...
    def _normalize_market_regime(value: Any) -> str:
        ...
        if 'transition' in text or 'chop' in text or '混沌' in text:
    ...
    def _normalize_timing_state(value: Any, action_hint: str = '') -> str:
        text = str(value or '').strip().lower()
        if text in allowed_timing_states:
            return text
        if 'enter' in text or 'now' in text or '立即' in text or '可進' in text:
            return 'enter_now'
        if 'pullback' in text or '回踩' in text or '回撤' in text:
            return 'wait_pullback'
        if 'breakout' in text or '突破' in text:
            return 'wait_breakout'
        if 'avoid' in text or 'skip' in text or '不做' in text:
            return 'avoid_near_term'
        if action_hint == 'enter':
            return 'enter_now'
        if action_hint == 'observe':
            return 'wait_pullback'
        return 'avoid_near_term'
    ...
    timing_state = _normalize_timing_state(raw.get('timing_state'), action)
    ...
    inferred_side = _normalize_side(
        ' '.join(
            [
                str(raw.get('market_read') or ''),
                str(raw.get('entry_plan') or ''),
                str(raw.get('breakout_assessment') or ''),
                str(raw.get('regime_note') or ''),
            ]
        )
    )
    if not side and inferred_side in {'long', 'short'}:
        side = inferred_side
    if not side and action != 'enter':
        side = 'neutral'
    ...
    decision = {
        ...
        'watch_trigger_type': str(raw.get('watch_trigger_type') or 'none').strip().lower(),
        ...
        'watch_trigger_candle': str(raw.get('watch_trigger_candle') or 'none').strip().lower(),
        'watch_retest_rule': str(raw.get('watch_retest_rule') or 'none').strip().lower(),
        'watch_volume_ratio_min': max(_num('watch_volume_ratio_min', 0.0), 0.0),
        'watch_micro_vwap_rule': str(raw.get('watch_micro_vwap_rule') or 'none').strip().lower(),
        'watch_micro_ema20_rule': str(raw.get('watch_micro_ema20_rule') or 'none').strip().lower(),
    ...
    if expected_action and decision['action'] != expected_action:
        ...
        warnings.append('normalized:action_from_bot_instruction')
    ...
    if decision['action'] == 'enter':
        ...
        if decision['trade_side'] not in {'long', 'short'}:
            ...
            decision['action'] = 'observe'
        elif decision['entry_price'] <= 0 or decision['stop_loss'] <= 0:
            warnings.append('enter_missing_entry_or_stop_downgraded')
            decision['action'] = 'observe'
    if decision['action'] == 'observe':
        decision['should_trade'] = False
        decision['timing_state'] = _normalize_timing_state(decision.get('timing_state'), 'observe')
        ...
        if decision['watch_trigger_type'] == 'none':
            decision['watch_trigger_type'] = _watch_trigger_type_from_side_timing(decision.get('trade_side'), decision.get('timing_state'))
    ...
    if decision['action'] in {'observe', 'skip'} and not str(decision.get('recheck_reason') or '').strip():
        decision['recheck_reason'] = '等待下一個可執行結構訊號後再重審。'
    if decision['action'] == 'enter' and not str(decision.get('stop_logic') or '').strip():
        decision['stop_logic'] = '以 15m 先前已收棒的結構失效位作為止損基準。'
    ...
    if decision['timing_state'] not in allowed_timing_states:
        decision['timing_state'] = 'wait_pullback' if decision['action'] == 'observe' else ('enter_now' if decision['action'] == 'enter' else 'avoid_near_term')
...
def consult_trade_decision(
    *,
    ...
    estimated_output_tokens = max(int(config.get('max_output_tokens', 560) or 560), 220)
    ...
    try:
        ...
        max_tokens = max(int(config.get('max_output_tokens', 560) or 560), 220)
        attempts = [
            {
                'model': primary_model,
                'structured': False,
