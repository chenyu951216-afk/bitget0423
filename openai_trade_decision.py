...
def _build_messages(candidate: Dict[str, Any], *, compact: bool = False) -> list[Dict[str, Any]]:
    ...
    if compact:
        system_text = (
            ...
            'Aggressive-but-controlled mandate: if trend, invalidation, and liquidity are acceptable, you may approve a fast tactical trade instead of defaulting to caution. '
        ...
        user_text = (
            ...
            'market_read must clearly answer whether the market currently has trend, does not have trend, or is transitioning. entry_plan must clearly answer whether to enter now, wait for pullback, wait for breakout, or avoid near term.\n'
...
def _normalize_decision(raw: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not decision['watch_note'] and watch_plan.get('watch_trigger'):
        decision['watch_note'] = str(watch_plan.get('watch_trigger') or '').strip()
    if not decision['recheck_reason'] and watch_plan.get('recheck_reason'):
        decision['recheck_reason'] = str(watch_plan.get('recheck_reason') or '').strip()
    ...
    if not decision['watch_note'] and raw.get('watch_trigger'):
        decision['watch_note'] = str(raw.get('watch_trigger') or '').strip()
