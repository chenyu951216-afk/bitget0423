...
def _openai_pending_advice_trigger(sig, advice, now_ts=None):
    ...
    min_recheck_gap = max(int(OPENAI_TRADE_CONFIG.get('cooldown_minutes', 180) or 180), 1) * 60
...
"tp_mult": tp_m,
            ...
            pending_watch_candidates = list(sigs)
            if bool(OPENAI_TRADE_CONFIG.get('enabled', True) and OPENAI_API_KEY):
                try:
                    ...
                    pending_symbols = set()
                sig_symbols = {str((s or {}).get('symbol') or '') for s in sigs}
            ...
            top10_for_order = sorted(
                list(top10) + pending_trigger_rows,
                key=lambda x:(1 if x.get('_pending_openai_advice') else 0, x.get('priority_score', abs(x['score'])), abs(x['score'])),
                reverse=True,
            )[:10]
