...
OPENAI_SOURCE_ROTATION = ["prebreakout_scanner", "short_gainers", "general"]
...
OPENAI_MIN_RR_FOR_ENTRY = max(0.8, min(3.0, float(env_or_blank("OPENAI_MIN_RR_FOR_ENTRY", "1.6") or 1.6)))
...
def build_signal_from_context(symbol: str, market: Dict[str, Any], context: Dict[str, Any], btc_context: Dict[str, Any] | None = None, *, candidate_source: str = "general", forced_side: str | None = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "scanner_intent": (
            "gainer_followup_review"
            if candidate_source == "short_gainers"
            else "pre_breakout_discovery"
            if candidate_source == "prebreakout_scanner"
            else "general_rank_review"
        ),
...
def build_openai_constraints(
    ...
    *,
    ...
    if source == "short_gainers":
        lane_min_rr = max(lane_min_rr, 1.7)
    elif source == "prebreakout_scanner":
        lane_min_rr = max(lane_min_rr, 1.6)
    return {
        ...
        "scanner_metadata_trust": "low_if_prebreakout_scanner",
    }
...
def _reorder_openai_candidates_by_source(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    if not rows:
        return []
    pending_rows = [row for row in rows if str(row.get("candidate_source") or "").strip().lower() == "pending_advice"]
    normal_rows = [row for row in rows if str(row.get("candidate_source") or "").strip().lower() != "pending_advice"]
    if not normal_rows:
        return pending_rows
    rotation = [str(src or "").strip().lower() for src in OPENAI_SOURCE_ROTATION if str(src or "").strip()]
    if not rotation:
        return pending_rows + normal_rows
    start_idx = safe_int((OPENAI_TRADE_STATE or {}).get("_source_rotation_idx"), 0) % len(rotation)
    buckets: Dict[str, List[Dict[str, Any]]] = {src: [] for src in rotation}
    ...
    for offset in range(len(rotation)):
        src = rotation[(start_idx + offset) % len(rotation)]
        ordered.extend(buckets.get(src, []))
    ordered.extend(fallback)
    return pending_rows + ordered
...
def _prebreakout_snapshot_from_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    scores = dict(signal.get("prebreakout_scanner_scores") or {})
    ...
    return {
        ...
        "side_hint": str(signal.get("prebreakout_side_hint") or signal.get("scanner_side") or signal.get("side") or "").lower(),
        "price": safe_float(signal.get("price"), 0.0),
        "prebreakout_score": safe_float(signal.get("prebreakout_score"), safe_float(scores.get("prebreakout_score"), 0.0)),
...
def _prebreakout_material_change(prev_snapshot: Dict[str, Any], new_snapshot: Dict[str, Any]) -> tuple[bool, List[str]]:
    ...
    score_delta = abs(safe_float(curr.get("prebreakout_score"), 0.0) - safe_float(prev.get("prebreakout_score"), 0.0))
    if score_delta >= 6.0:
        changes.append("score_delta={:.2f}".format(score_delta))
    if str(prev.get("side_hint") or "") != str(curr.get("side_hint") or ""):
        changes.append("side_hint_changed")
...
def exposure_snapshot() -> Dict[str, Any]:
    ...
    for row in pending_orders.values():
        if is_prebreakout_source((row or {}).get("candidate_source")):
            continue
        side = _side_from_value((row or {}).get("side"))
        if side == "long":
            long_general_total += 1
        elif side == "short":
            short_general_total += 1
    prebreakout_total = len(active_prebreakout) + len(pending_prebreakout)
    total_all = len(active_positions) + len(pending_orders)
...
def build_prebreakout_signals(
    ...
    for row in list(leaderboard.get("top_candidates") or [])[:PREBREAKOUT_TOP_PICK]:
        ...
        side_hint = str((row or {}).get("side_hint") or "").lower()
        payload = dict(candidate_payloads.get(symbol) or {})
        signal["rank"] = safe_int((row or {}).get("rank"), 0)
        signal["priority_score"] = safe_float((row or {}).get("prebreakout_score"), safe_float(signal.get("priority_score"), 0.0))
        signal["prebreakout_score"] = safe_float((row or {}).get("prebreakout_score"), 0.0)
        signal["prebreakout_side_hint"] = side_hint
        signal["prebreakout_reason_tags"] = list((row or {}).get("reason_tags") or [])[:8]
        signal["prebreakout_scanner_scores"] = dict(payload.get("scanner_scores") or {})
        signal["prebreakout_scanner_scores"]["prebreakout_score"] = signal["prebreakout_score"]
        ...
        signal["scanner_intent"] = "pre_breakout_discovery"
        tags = ", ".join(list(signal.get("prebreakout_reason_tags") or [])[:4])
        signal["desc"] = "pre-breakout candidate{}{}".format(": " if tags else "", tags)
...
def maybe_run_openai(
    ...
    global OPENAI_TRADE_STATE
    reviewed_candidates = _reorder_openai_candidates_by_source(
    ...
    for reviewed in reviewed_candidates:
        ...
        if source == "prebreakout_scanner":
            candidate["scanner_note"] = (
                "scanner_score只能作為候選排序，不可作為交易理由；"
                "最終決策必須由 raw OHLCV、流動性、衍生品與風控重新推導。"
            )
            candidate["scanner_rank"] = safe_int(reviewed.get("rank"), 1)
            candidate["scanner_scores"] = dict(reviewed.get("prebreakout_scanner_scores") or {})
            candidate["scanner_features"] = _strip_direction_hints(dict(reviewed.get("prebreakout_scanner_features") or {}))
            ...
            candidate["reason_tags"] = [
                tag for tag in list(reviewed.get("prebreakout_reason_tags") or [])[:8]
                if not str(tag or "").strip().lower().startswith("side_hint_")
            ]
        try:
            with OPENAI_LOCK:
            ...
            for idx, row in enumerate(target_pool):
                if row.get("symbol") == reviewed.get("symbol"):
                    ...
                    break
            if source_lower in OPENAI_SOURCE_ROTATION:
                OPENAI_TRADE_STATE["_source_rotation_idx"] = (OPENAI_SOURCE_ROTATION.index(source_lower) + 1) % len(OPENAI_SOURCE_ROTATION)
    ...
    try:
        ...
        for symbol, market, ticker in short_gainer_universe:
            try:
                ...
                continue
        prebreakout_signals, prebreakout_leaderboard = build_prebreakout_signals(markets, tickers, context_map, btc_context)
        pending_advice_signals: List[Dict[str, Any]] = []
        ...
        prebreakout_top = sorted(
            list(prebreakout_signals or []),
            key=lambda row: (
                safe_int(row.get("rank"), 9999),
                -safe_float(row.get("prebreakout_score"), safe_float(row.get("priority_score"), 0.0)),
            ),
        ...
        maybe_run_openai(general_top, short_gainer_top, prebreakout_top, pending_advice_signals)
