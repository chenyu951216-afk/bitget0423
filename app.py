...
MAX_ACTIVE_POSITION_SCAN_INTERVAL_SEC = max(5, int(float(env_or_blank("MAX_ACTIVE_POSITION_SCAN_INTERVAL_SEC", "30") or 30)))
...
def fixed_order_notional_usdt(symbol: str) -> float:
    return BTC_ETH_MIN_ORDER_NOTIONAL_USDT if base_asset(symbol) in {"BTC", "ETH"} else FIXED_ORDER_NOTIONAL_USDT
...
def derive_trade_levels(side: str, context: Dict[str, Any], total_score: float) -> Dict[str, float]:
    ...
    if side == "long":
        entry = price
        structure_stop = support - atr * stop_buffer_atr if 0 < support < entry else entry - atr * min_stop_gap_atr
        ...
        entry = price
        structure_stop = resistance + atr * stop_buffer_atr if resistance > entry else entry + atr * min_stop_gap_atr
...
def build_signal_from_context(symbol: str, market: Dict[str, Any], context: Dict[str, Any], btc_context: Dict[str, Any] | None = None, *, candidate_source: str = "general", forced_side: str | None = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "reference_trade_plan": {
            ...
            "note": "Signal-specific execution hints derived from the selected side, live structure, and ATR.",
        },
...
def priority_position_symbols(limit: int = MAX_OPEN_POSITIONS) -> List[str]:
    with STATE_LOCK:
        active = list(STATE.get("active_positions") or [])
    ranked = []
    for row in active:
        ...
        ranked.append(
            (
                abs(safe_float(row.get("drawdown_pct"), 0.0)) + abs(safe_float(row.get("leveraged_pnl_pct"), 0.0)),
                symbol,
            )
        )
    ranked.sort(key=lambda item: item[0], reverse=True)
    return [symbol for _, symbol in ranked[: max(int(limit), 1)]]
...
def risk_unit_from_rule(rule: Dict[str, Any]) -> float:
    return abs(safe_float(rule.get("initial_entry_price"), 0.0) - safe_float(rule.get("initial_stop_loss"), 0.0))
...
def reduce_position(symbol: str, side: str, qty: float, reason: str) -> Dict[str, Any]:
    ...
    params = {"reduceOnly": True, "marginMode": "cross", "posSide": "long" if str(side).lower() == "long" else "short"}
    try:
...
def process_position_rules(positions_payload: List[Dict[str, Any]]) -> None:
    ...
    for symbol, position in active_map.items():
        ...
        hard_stop_hit = (side == "long" and mark_price <= stop) or (side == "short" and mark_price >= stop)
        if hard_stop_hit:
            result = reduce_position(symbol, side, current_qty, "hard_stop_loss")
            if result.get("ok"):
        ...
        if rule.get("scale_in_enabled") and not rule.get("scale_in_done"):
            ...
            if should_scale:
                ...
                if result.get("ok"):
                    ...
                    rule["remaining_qty"] = current_qty + safe_float(result.get("qty"), 0.0)
        if rule.get("trailing_active"):
            if side == "long":
                trail_stop = max(stop, safe_float(rule.get("highest_price"), mark_price) - risk)
                rule["trailing_stop"] = trail_stop
                if mark_price <= trail_stop and current_qty > 0:
                    reduce_position(symbol, side, current_qty, "trailing_stop")
                    remove_position_rule(symbol)
                    continue
            else:
                trail_stop = min(stop, safe_float(rule.get("lowest_price"), mark_price) + risk)
                rule["trailing_stop"] = trail_stop
                if mark_price >= trail_stop and current_qty > 0:
                    reduce_position(symbol, side, current_qty, "trailing_stop")
                    remove_position_rule(symbol)
                    continue
            rule["trail_pct"] = abs((mark_price - safe_float(rule.get("trailing_stop"), mark_price)) / max(mark_price, 1e-9))
...
def build_scan_universe(markets: Dict[str, Any], tickers: Dict[str, Any]) -> tuple[List[tuple[str, Dict[str, Any], Dict[str, Any]]], List[tuple[str, Dict[str, Any], Dict[str, Any]]], List[tuple[str, Dict[str, Any], Dict[str, Any]]], List[tuple[str, Dict[str, Any], Dict[str, Any]]], List[tuple[str, Dict[str, Any], Dict[str, Any]]]]:
    tradeable = []
    ...
    active_position_universe = []
    for symbol in priority_position_symbols(MAX_OPEN_POSITIONS):
        market = dict(markets.get(symbol) or {})
        if not market:
            continue
        ticker = dict(tickers.get(symbol) or {})
        active_position_universe.append((symbol, market, ticker))
    watch_universe = []
    ...
    for symbol, market, ticker in active_position_universe + volume_universe + short_gainer_universe + watch_universe:
    ...
    return list(union.values()), volume_universe, short_gainer_universe, watch_universe, active_position_universe
...
def perform_scan_cycle() -> None:
    ...
    try:
        ...
        selected_union, volume_universe, short_gainer_universe, watch_universe, active_position_universe = build_scan_universe(markets, tickers)
        if active_position_universe:
            set_backend_thread("scan", "running", "Prioritizing {} held symbols before the regular scan.".format(len(active_position_universe)))
...
def positions_thread() -> None:
    ...
    while True:
        ...
        sleep_sec = min(POSITION_INTERVAL_SEC, MAX_ACTIVE_POSITION_SCAN_INTERVAL_SEC) if active_count > 0 else POSITION_INTERVAL_SEC
