...
OPENAI_SHORT_GAINERS_LIMIT = max(1, int(float(env_or_blank('OPENAI_SHORT_GAINERS_LIMIT', '8') or 8)))
OPENAI_SHORT_GAINERS_PREFILTER = max(10, int(float(env_or_blank('OPENAI_SHORT_GAINERS_PREFILTER', '80') or 80)))
OPENAI_SHORT_GAINERS_MIN_24H_PCT = max(0.5, float(env_or_blank('OPENAI_SHORT_GAINERS_MIN_24H_PCT', '6.0') or 6.0))
                ...
                try:
                    ...
                    if status in ('closed', 'filled'):
                        ...
                        if pending_meta:
                            try:
                                ...
                                print("掛單成交後補建倉狀態失敗 {}: {}".format(symbol, fill_err))
                        update_state(fvg_orders=dict(FVG_ORDERS))
                        continue
                    if status == 'canceled':
                        with FVG_LOCK:
                    ...
                    if ai_limit_cancel_price > 0:
                        if order['side'] == 'long' and curr <= ai_limit_cancel_price:
                            cancel_reason = 'OpenAI取消掛單 {:.6f} [{}] {}'.format(
...
def _openai_pending_advice_trigger(sig, advice, now_ts=None):
    ...
    if trigger_type == 'pullback_to_entry':
        ...
        hit = score_now >= max(float(advice.get('score', 0) or 0) + 6.0, 54.0) or priority_now >= float(advice.get('priority_score', 0) or 0) + 6.0
    else:
        hit = False
    if hit:
...
def scan_thread():
    ...
    _touch_backend_thread('scan', '啟動中，等待其他執行緒就緒')
    time.sleep(10)
    _refresh_learn_summary()
    while True:
        try:
            _touch_backend_thread('scan', '準備開始新一輪市場掃描')
            # 每輪開始清空本輪下單記錄
            with _ORDERED_LOCK:
            ...
            _touch_backend_thread('scan', '第{}輪：抓取市場資料'.format(sc + 1))
            try:
            ...
            _touch_backend_thread('scan', '第{}輪：分析 {} 個幣'.format(sc + 1, len(symbols)))
...
"tp_mult": tp_m,
            ...
            _touch_backend_thread('scan', '第{}輪：排行榜完成，共 {} 個低相關候選'.format(sc + 1, len(top7)))
            with STATE_LOCK:
            ...
            if pos_cnt < review_position_cap:
                _touch_backend_thread('scan', '第{}輪：送審與下單評估中'.format(sc + 1))
                order_delay = 0
                ...
                for rank_index, best in enumerate(top10_for_order):
                    ...
                    # 大盤只做輔助權重，避免單一 BTC/ETH 大盤判斷主導短線選幣。
                    eff_threshold = ORDER_THRESHOLD + (MARKET_NEUTRAL_THRESHOLD_ADD if mkt_dir == "中性" and mkt_str >= 0.5 else 0)
                    ...
                    if best['symbol'] in pos_syms:
                        hard_gate_reasons.append('同幣已有持倉')
                    if best['symbol'] in already_closing:
                        hard_gate_reasons.append('該幣正在反向平倉')
                    if best['symbol'] in SHORT_TERM_EXCLUDED:
                        hard_gate_reasons.append('該幣在短期排除名單')
                    ...
                    if openai_mode_active:
                        ...
                        rank_ok = is_short_gainer_review or rank_index < openai_rank_limit
                        ...
                        effective_hard_gate_reasons = [] if openai_pending_triggered else hard_gate_reasons
                        can_send_openai = (
                            rank_ok and
                        ...
                        if can_send_openai:
                            ...
                            _candidate, openai_result = _consult_openai_trade_for_signal(
                                best,
                                min(rank_index, openai_rank_limit - 1) if is_short_gainer_review else rank_index,
                                top10_for_order,
                                dict(MARKET_STATE),
                                risk_snapshot,
                                portfolio,
                    ...
                    if openai_mode_active:
                        ...
                        if openai_status == 'not_ranked':
                            ...
                            if openai_decision.get('should_trade'):
                                ...
                                reasons.append('OpenAI拒單: {}'.format(openai_decision.get('reason_to_skip') or '模型認為不適合'))
                        elif openai_status == 'cooldown_active':
                            reasons.append('OpenAI同幣冷卻中')
                        elif openai_status == 'budget_paused':
                            reasons.append('OpenAI月預算已達上限')
            ...
            _touch_backend_thread('scan', '第{}輪完成，60 秒後下一輪'.format(STATE["scan_count"]))
            ...
            print("掃描異常: {}".format(e))
            print(traceback.format_exc())
            _set_backend_thread_state('scan', 'crashed', '掃描執行緒異常，10 秒後重試', str(e))
            time.sleep(10)
...
def watchdog(target_func, name):
    ...
    while True:
        _set_backend_thread_state(name, 'starting', '準備啟動')
        try:
            print("=== 執行緒啟動: {} ===".format(name))
            _set_backend_thread_state(name, 'running', '執行中')
            target_func()
            print("=== 執行緒正常結束（不應發生）: {} ===".format(name))
            _set_backend_thread_state(name, 'stopped', '執行緒意外結束')
        except BaseException as e:
            import traceback
            print("=== 執行緒崩潰 {} : {} ===".format(name, e))
            print(traceback.format_exc())
            _set_backend_thread_state(name, 'crashed', '執行緒崩潰，等待自動重啟', str(e))
        print("=== 執行緒5秒後重啟: {} ===".format(name))
        _set_backend_thread_state(name, 'restarting', '5秒後自動重啟')
        time.sleep(5)
...
def build_short_gainer_signal(row, existing=None):
    ...
    bd.update({
        ...
        'RR': rr,
        'EntryGate': max(float(bd.get('EntryGate', bd.get('進場品質', 0)) or 0), 2.0),
...
def start_all_threads():
    ...
    update_state(ai_panel=dict(AI_PANEL), auto_backtest=dict(AUTO_BACKTEST_STATE), news_score=0, news_sentiment='已停用', latest_news_title='新聞系統已停用', backend_threads=backend_threads)
