...
BITGET_COPY_PERMISSION_FALLBACK_TO_SWAP = str(env_or_blank("BITGET_COPY_PERMISSION_FALLBACK_TO_SWAP", "1") or "1").strip().lower() in {"1", "true", "yes", "on"}
...
def _bitget_order_product_types() -> List[str]:
    ...
    if BITGET_USE_COPY_TRADER_API:
        ...
        if BITGET_COPY_PERMISSION_FALLBACK_TO_SWAP:
...
def _create_contract_order(
    ...
    *,
    ...
    if BITGET_USE_COPY_TRADER_API:
        ...
        if copy_mode_blocked and not BITGET_COPY_PERMISSION_FALLBACK_TO_SWAP:
            raise RuntimeError("copy_permission_missing:need trace write permission")
        if not BITGET_COPY_PERMISSION_FALLBACK_TO_SWAP:
...
def sync_positions_once() -> None:
    ...
    try:
        ...
        if BITGET_USE_COPY_TRADER_API:
            ...
            if equity <= 0 and copy_permission_blocked:
