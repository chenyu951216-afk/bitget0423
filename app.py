...
OPENAI_SOURCE_ROTATION = ["short_gainers", "prebreakout_scanner", "general"]
...
OPENAI_TRADE_CONFIG["cooldown_minutes"] = 60
OPENAI_TRADE_CONFIG["same_payload_reuse_minutes"] = 60
...
def build_openai_constraints(
    ...
    *,
    ...
    if source == "short_gainers":
        lane_min_rr = max(lane_min_rr, 1.45)
    elif source == "prebreakout_scanner":
        lane_min_rr = max(lane_min_rr, 1.4)
    return {
...
def _prioritize_openai_candidates_by_source(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if short_ready:
        return pending_rows + short_rows
...
def _apply_openai_trade_plan_to_signal(sig: Dict[str, Any], decision: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sig["take_profit_source"] = "ai_direct"
...
def place_order_from_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with ORDER_LOCK:
        ...
        signal["take_profit_source"] = "ai_direct"
...
def maybe_run_openai(
    ...
    global OPENAI_TRADE_STATE
    reviewed_candidates = _prioritize_openai_candidates_by_source(
