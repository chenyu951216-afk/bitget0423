...
EXCHANGE_PROTECTION_MODE = str(env_or_blank("EXCHANGE_PROTECTION_MODE", "local") or "local").strip().lower()
...
LOCAL_TP_MIN_R = max(2.0, float(env_or_blank("LOCAL_TP_MIN_R", "2.0") or 2.0))
LOCAL_TP_DEFAULT_R = max(LOCAL_TP_MIN_R, float(env_or_blank("LOCAL_TP_DEFAULT_R", "2.2") or 2.2))
LOCAL_TP_MAX_R = max(LOCAL_TP_DEFAULT_R, float(env_or_blank("LOCAL_TP_MAX_R", "4.0") or 4.0))
...
def _find_recent_structure_anchor(side: str, bars: List[Dict[str, float]], entry_price: float) -> tuple[float, str]:
    ...
    if side == "long":
        ...
        if valid_pivot_lows:
            return max(valid_pivot_lows), "pivot_low_15m"
        ...
        if valid_pivot_highs:
            return min(valid_pivot_highs), "pivot_high_15m"
...
def _derive_structure_stop_15m(side: str, context: Dict[str, Any], entry_price: float, atr_price: float) -> tuple[float, float, str, bool]:
    ...
    anchor, anchor_source = _find_recent_structure_anchor(side, bars, entry_price)
    ...
    anti_sweep = max(atr_price * 0.16, entry_price * 0.0012)
    ...
    min_gap = max(atr_price * 0.75, entry_price * 0.004)
    max_gap = max(atr_price * (2.15 if explosive_move else 2.8), min_gap * 1.4)
...
def build_openai_constraints(symbol: str = "", reference_price: float = 0.0) -> Dict[str, Any]:
    ...
    return {
        ...
        "min_rr_for_entry": LOCAL_TP_MIN_R,
...
def candidate_review_gate(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if status == "observe":
        ...
        last_recheck_sent_ts = safe_float(tracker.get("last_recheck_sent_ts"), 0.0)
        next_recheck_ts = safe_float(tracker.get("next_recheck_ts"), 0.0)
        if next_recheck_ts <= 0 and last_recheck_sent_ts > 0:
            next_recheck_ts = last_recheck_sent_ts + AI_OBSERVE_RECHECK_COOLDOWN_SEC
        allow_recheck = ready and (next_recheck_ts <= 0 or now >= next_recheck_ts)
        stage = str(eval_result.get("stage") or "bot_tracking_active")
        reason_text = str(eval_result.get("reason") or "")
        gate_reason = "watch_waiting"
        if ready and allow_recheck:
            gate_reason = "watch_triggered"
        elif ready and not allow_recheck:
            gate_reason = "recheck_cooldown"
            stage = "recheck_cooldown"
            reason_text = "trigger_ready_waiting_recheck_window_until={}".format(tw_from_ts(next_recheck_ts))
        with REVIEW_LOCK:
            if symbol in REVIEW_TRACKER:
                ...
                REVIEW_TRACKER[symbol]["trigger_ready"] = ready
                REVIEW_TRACKER[symbol]["next_recheck_ts"] = next_recheck_ts
        return {
            "ok": allow_recheck,
            "reason": gate_reason,
...
def apply_review_tracker(signal: Dict[str, Any], result: Dict[str, Any]) -> str:
    ...
    with REVIEW_LOCK:
        ...
        row = {
            ...
            "last_recheck_sent_ts": safe_float(existing.get("last_recheck_sent_ts"), 0.0),
            "next_recheck_ts": safe_float(existing.get("next_recheck_ts"), 0.0),
        ...
        if action == "observe":
            ...
            row["tracking_status"] = str(existing.get("tracking_status") or "bot_tracking_active")
            row["tracking_reason"] = str(existing.get("tracking_reason") or "")
            if bool(signal.get("force_openai_recheck")):
                row["last_recheck_sent_ts"] = now
                row["next_recheck_ts"] = now + AI_OBSERVE_RECHECK_COOLDOWN_SEC
                row["tracking_status"] = "recheck_cooldown"
                row["tracking_reason"] = "recheck_cooldown_until={}".format(tw_from_ts(row["next_recheck_ts"]))
                row["trigger_ready"] = False
            if row["next_recheck_ts"] <= 0 and row["last_recheck_sent_ts"] > 0:
                row["next_recheck_ts"] = row["last_recheck_sent_ts"] + AI_OBSERVE_RECHECK_COOLDOWN_SEC
            ...
            if not str(decision.get("recheck_reason") or "").strip():
                trigger_price = safe_float(decision.get("watch_trigger_price"), 0.0)
                tf = str(decision.get("watch_timeframe") or "15m")
                decision["recheck_reason"] = (
                    f"等待 {tf} 主觸發價 {trigger_price:.8f} 條件成立後重送 AI。"
                    if trigger_price > 0
                    else f"等待 {tf} 主觸發條件成立後重送 AI。"
                )
            decision["next_recheck_ts"] = row["next_recheck_ts"]
            decision["next_recheck_at"] = tw_from_ts(row["next_recheck_ts"])
...
def sync_openai_pending_advice() -> None:
    ...
    with REVIEW_LOCK:
        ...
        for symbol, row in REVIEW_TRACKER.items():
            ...
            last_recheck_sent_ts = safe_float(row.get("last_recheck_sent_ts"), 0.0)
            next_recheck_ts = safe_float(row.get("next_recheck_ts"), 0.0)
            pending[symbol] = {
                "symbol": symbol,
                "side": row.get("side"),
                "status": "watching",
                ...
                "last_recheck_sent_ts": last_recheck_sent_ts,
                "last_recheck_sent_at": tw_from_ts(last_recheck_sent_ts),
                "next_recheck_ts": next_recheck_ts,
                "next_recheck_at": tw_from_ts(next_recheck_ts),
            ...
            if not str(pending[symbol].get("recheck_reason") or "").strip():
                tf = str(decision.get("watch_timeframe") or "15m")
                pending[symbol]["recheck_reason"] = (
                    f"等待 {tf} 主觸發價 {trigger:.8f} 條件成立後重送 AI。"
                    if trigger > 0
                    else f"等待 {tf} 主觸發條件成立後重送 AI。"
                )
    ...
    ref.update(
        {
            ...
            "summary": "機器人已確認先前 AI 要求的觀察條件成立，現在必須重新送 AI 確認是否可執行。",
            ...
            "note": (
                "剛達到你先前要求觀察的條件：{} | trigger={} price={} invalidation={} timeframe={} recheck={}".format(
                    trigger_reason,
...
def place_order_from_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with ORDER_LOCK:
        ...
        instruction = str(decision.get("bot_instruction") or "").strip().upper()
        decision_side = str(decision.get("trade_side") or signal.get("side") or "").lower()
        if decision_side not in ("long", "short"):
            if instruction in ("ENTER_MARKET", "ENTER_LIMIT"):
                decision_side = str(signal.get("side") or "long").lower()
            else:
                decision_side = "long"
...
def maybe_run_openai(general_top: List[Dict[str, Any]], short_gainers: List[Dict[str, Any]], pending_advice_signals: List[Dict[str, Any]]) -> None:
    ...
    for reviewed in reviewed_candidates:
        if not reviewed.get("local_gate_ok"):
            ...
            if str(reviewed.get("local_gate_reason") or "") == "recheck_cooldown":
                next_ts = safe_float(reviewed.get("local_gate_next_recheck_ts"), 0.0)
                if next_ts > 0:
                    blocker = "{}:recheck_cooldown_until={}".format(compact_symbol(reviewed.get("symbol")), tw_from_ts(next_ts))
        ...
        if reviewed.get("candidate_source") == "pending_advice" and reviewed.get("force_openai_recheck"):
            ...
            if tracker:
                eval_result = evaluate_watch_condition(reviewed, tracker)
                if bool(eval_result.get("ready", False)):
                    reviewed = _attach_pending_recheck_context(reviewed, tracker, eval_result)
