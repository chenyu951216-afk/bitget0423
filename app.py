...
if BITGET_MARGIN_MODE not in {"crossed", "isolated"}:
    BITGET_MARGIN_MODE = "crossed"
BITGET_API_MODE = str(env_or_blank("BITGET_API_MODE", "mix_public") or "mix_public").strip().lower()
...
def close_position(symbol: str, hold_side: str, size: float, *, client_oid: str = "") -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        ...
        "holdSide": hold,
        "orderType": "market",
        "clientOid": str(client_oid or "bot-close-{}-{}".format(int(time.time() * 1000), symbol_contract_id(symbol)[:12])),
    }
    data = _bitget_private_request("POST", BITGET_ENDPOINTS["mix_place_order"], body=payload)
    item = dict(data or {})
    return {
        "id": str(item.get("orderId") or item.get("clientOid") or ""),
...
def _bitget_copy_fetch_current_tracks(symbol: str = "") -> List[Dict[str, Any]]:
    return _bitget_mix_fetch_all_positions(symbol)
