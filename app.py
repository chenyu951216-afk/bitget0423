...
def get_balance() -> Dict[str, float]:
    rows = _bitget_private_request(
        "GET",
        BITGET_ENDPOINTS["mix_accounts"],
        query={"productType": BITGET_PRODUCT_TYPE},
    )
    if isinstance(rows, dict):
        items = [dict(row or {}) for row in list(rows.get("list") or rows.get("assets") or [])]
    else:
        items = [dict(row or {}) for row in list(rows or [])]
    target = {}
    for row in items:
        if str(row.get("marginCoin") or "").strip().upper() == BITGET_MARGIN_COIN:
            target = row
            break
    ...
    available = safe_float(
        ...
        safe_float(target.get("availableBalance"), 0.0),
    )
    equity = max(
        safe_float(target.get("accountEquity"), 0.0),
        safe_float(target.get("equity"), 0.0),
        safe_float(copy_summary.get("totalEquity"), 0.0),
    )
    margin = safe_float(
        target.get("locked"),
        safe_float(target.get("margin"), safe_float(target.get("positionMargin"), 0.0)),
    )
    unrealized = safe_float(
        target.get("unrealizedPL"),
        safe_float(target.get("unrealizedPnl"), safe_float(target.get("upl"), 0.0)),
    )
    return {
        "available": available,
        "equity": equity,
