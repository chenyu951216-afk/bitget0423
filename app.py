...
def _bitget_auth_headers(method: str, request_path: str, query_string: str = "", body_text: str = "") -> Dict[str, str]:
    ...
    return {
        ...
        "ACCESS-PASSPHRASE": str(env_or_blank("BITGET_PASSWORD", "") or ""),
...
def _bitget_copy_fetch_current_tracks(symbol: str = "") -> List[Dict[str, Any]]:
    ...
    data = _bitget_private_request("GET", "/api/v2/copy/mix-trader/order-current-track", query=query)
...
def _bitget_uta_positions(category: str) -> List[Dict[str, Any]]:
    data = _bitget_private_request("GET", "/api/v3/position/current-position", query={"category": category})
    payload = dict(data or {})
...
def _create_contract_order(
    ...
    *,
    ...
    if BITGET_USE_COPY_TRADER_API:
        try:
            return _bitget_uta_place_order(symbol, order_type, side, qty, price, reduce_only=reduce_only)
    ...
    try:
        ...
        errors.append(str(exc)[:220])
    raise RuntimeError("order_failed:" + " | ".join(errors[:2]))
    ...
    try:
        ...
        if BITGET_USE_COPY_TRADER_API:
            ...
            try:
                ...
                equity = max(
                    safe_float(assets.get("usdtEquity"), 0.0),
                    safe_float(assets.get("accountEquity"), 0.0),
                )
                if equity <= 0:
                    for row in list(assets.get("assets") or []):
                        item = dict(row or {})
                        if str(item.get("coin") or "").upper() == BITGET_MARGIN_COIN:
                            equity = max(
                                safe_float(item.get("equity"), 0.0),
                                safe_float(item.get("available"), 0.0) + safe_float(item.get("locked"), 0.0),
                            )
                            if equity > 0:
                                break
            except Exception as exc:
                uta_assets_error = str(exc)[:180]
                equity = 0.0
            try:
                uta_positions = _bitget_uta_positions(BITGET_PRODUCT_TYPE)
                for row in uta_positions:
                    size = abs(safe_float(row.get("total"), safe_float(row.get("available"), 0.0)))
                    if size <= 0:
                        continue
                    ...
                    symbol = symbol_id + "/USDT:USDT" if symbol_id and symbol_id.endswith("USDT") else symbol_id
                    entry = safe_float(row.get("avgPrice"), 0.0)
                    mark = safe_float(row.get("markPrice"), entry)
                    leverage = safe_float(row.get("leverage"), 1.0)
                    unrealized = safe_float(row.get("unrealisedPnl"), 0.0)
                    pct = safe_float(row.get("profitRate"), 0.0) * 100.0
                ...
                uta_positions_error = str(exc)[:180]
            try:
            ...
            if not positions_payload:
                for row in tracks:
                    ...
                    try:
                        ...
                        mark = safe_float((ticker or {}).get("last"), 0.0)
                    except Exception:
                        mark = entry
            ...
            if equity <= 0 and not positions_payload:
                set_backend_thread(
                    "positions",
                    "running",
                    "Position sync active but balance unavailable (UTA/copy endpoints).",
                    error="uta_assets={} uta_positions={}".format(uta_assets_error, uta_positions_error)[:220],
                )
        else:
            try:
                ...
                equity = safe_float(total.get(BITGET_MARGIN_COIN), 0.0)
            except Exception:
                equity = safe_float(STATE.get("equity"), 0.0)
            try:
                raw_positions = list(exchange.fetch_positions(None, {"productType": BITGET_PRODUCT_TYPE, "marginCoin": BITGET_MARGIN_COIN}) or [])
            except Exception:
                try:
    ...
    try:
        if BITGET_USE_COPY_TRADER_API:
            ...
            if not order:
                ...
                try:
                    order = _bitget_uta_place_order(symbol, "market", close_side, qty, None, reduce_only=True)
                except Exception as exc:
                    close_errors.append(str(exc)[:180])
            if not order:
...
def ensure_exchange_protection(symbol: str, side: str, qty: float, stop_loss: float, take_profit: float, *, manage_take_profit_locally: bool = True) -> Dict[str, bool]:
    if BITGET_USE_COPY_TRADER_API:
        ...
        try:
            ...
            tracking_no = ""
        sl_ok = safe_float(stop_loss, 0.0) > 0
        tp_ok = True if manage_take_profit_locally else (safe_float(take_profit, 0.0) > 0)
        if tracking_no:
            _bitget_copy_modify_tpsl(symbol, tracking_no, stop_loss if sl_ok else 0.0, 0.0 if manage_take_profit_locally else take_profit)
        return {"sl_ok": sl_ok, "tp_ok": tp_ok}
...
def place_order_from_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with ORDER_LOCK:
        ...
        try:
            if order_type == "limit":
                order = _create_contract_order(symbol, "limit", order_side, qty, signal["price"], reduce_only=False)
                ...
                order = _create_contract_order(symbol, "market", order_side, qty, None, reduce_only=False)
