...
ACTIVE_POSITION_PRICE_POLL_SEC = max(2, int(float(env_or_blank("ACTIVE_POSITION_PRICE_POLL_SEC", "2") or 2)))
...
BTC_BETA_MAX_CONCURRENT = max(1, min(MAX_OPEN_POSITIONS, int(float(env_or_blank("BTC_BETA_MAX_CONCURRENT", "2") or 2))))
...
EXCHANGE_SNAPSHOT_CACHE_TTL_SEC = max(8.0, float(env_or_blank("EXCHANGE_SNAPSHOT_CACHE_TTL_SEC", "20") or 20))
...
def fetch_exchange_snapshot() -> tuple[Dict[str, Any], Dict[str, Any], str]:
    global LAST_MARKETS_CACHE, LAST_TICKERS_CACHE, LAST_MARKETS_TS, LAST_TICKERS_TS
    markets: Dict[str, Any] = {}
    tickers: Dict[str, Any] = {}
    notes: List[str] = []
    now = time.time()
    use_markets_cache = bool(LAST_MARKETS_CACHE) and (now - LAST_MARKETS_TS) <= EXCHANGE_SNAPSHOT_CACHE_TTL_SEC
    use_tickers_cache = bool(LAST_TICKERS_CACHE) and (now - LAST_TICKERS_TS) <= EXCHANGE_SNAPSHOT_CACHE_TTL_SEC
...
def build_signal_from_context(symbol: str, market: Dict[str, Any], context: Dict[str, Any], btc_context: Dict[str, Any] | None = None, *, candidate_source: str = "general", forced_side: str | None = None) -> Dict[str, Any]:
    ...
    btc_tf15 = dict(((btc_context or {}).get("multi_timeframe") or {}).get("15m") or {})
    btc_ret12 = safe_float(btc_tf15.get("ret_12bars_pct"), 0.0)
    symbol_ret12 = safe_float(tf15.get("ret_12bars_pct"), 0.0)
    btc_drive_aligned = (
        _is_btc_beta_symbol(symbol)
        and abs(btc_ret12) >= 1.2
        and (
            (side == "long" and btc_ret12 > 0 and symbol_ret12 > 0)
            or (side == "short" and btc_ret12 < 0 and symbol_ret12 < 0)
        )
    )
    return {
        "symbol": symbol,
        ...
        "margin_pct": round(clamp((fixed_order_notional_usdt(symbol, levels["price"]) / max(_get_symbol_max_leverage(symbol), 1)) / max(safe_float(STATE.get("equity"), 100.0), 100.0), 0.01, 0.25), 4),
        ...
        "btc_drive_aligned": bool(btc_drive_aligned),
        "btc_ret_12bars_pct": round(btc_ret12, 4),
        "symbol_ret_12bars_pct": round(symbol_ret12, 4),
        "atr": levels["atr"],
        "atr15": levels["atr"],
...
def market_context_ready_for_ai(context: Dict[str, Any]) -> tuple[bool, str]:
    ...
    for tf in required:
        ...
        if close_price <= 0:
            return False, "missing_last_close_{}".format(tf)
        bars = list(bars_map.get(tf) or [])
        min_bars = _ai_required_min_bars(tf)
        if len(bars) < min_bars:
            return False, "insufficient_bars_{}_{}".format(tf, len(bars))
    return True, "ok"
...
def build_openai_constraints(
    ...
    *,
    ...
    if expected_side not in ("long", "short"):
        expected_side = ""
    strategy_lane = "countertrend_short_reversal" if source == "short_gainers" else "trend_following_execution"
    return {
        ...
        "max_same_direction": MAX_SAME_DIRECTION_POSITIONS,
        "max_btc_beta_cluster": BTC_BETA_MAX_CONCURRENT,
        "leverage_policy": "always_use_symbol_max",
        "min_rr_for_entry": 0.0,
        "execution_tp_mode": "local_r_from_ai_stop",
        "expected_side": expected_side,
        "side_lock": bool(side_lock and expected_side in ("long", "short")),
...
def _is_btc_drive_signal(signal: Dict[str, Any]) -> bool:
    if not _is_btc_beta_symbol(str(signal.get("symbol") or "")):
        return False
    if bool(signal.get("btc_drive_aligned", False)):
        return True
    context = dict(signal.get("openai_market_context") or {})
    tf15 = dict((context.get("multi_timeframe") or {}).get("15m") or {})
    ret12 = safe_float(tf15.get("ret_12bars_pct"), 0.0)
    side = _side_from_value(signal.get("side"))
    if side == "long":
        return ret12 >= 1.2
    if side == "short":
        return ret12 <= -1.2
...
def exposure_snapshot() -> Dict[str, Any]:
    with STATE_LOCK:
        ...
        pending_orders = dict(STATE.get("fvg_orders") or {})
    ...
    return {
        ...
        "long_total": len(long_positions) + len(long_pending),
        "short_total": len(short_positions) + len(short_pending),
        "total": len(active_positions) + len(pending_orders),
...
def btc_beta_cluster_count(snapshot: Dict[str, Any], side: str) -> int:
    norm_side = _side_from_value(side)
    if norm_side not in ("long", "short"):
        return 0
    symbols: set[str] = set()
    key_open = "long_symbols" if norm_side == "long" else "short_symbols"
    key_pending = "pending_long_symbols" if norm_side == "long" else "pending_short_symbols"
    symbols.update(set(snapshot.get(key_open) or set()))
    symbols.update(set(snapshot.get(key_pending) or set()))
    return sum(1 for sym in symbols if _is_btc_beta_symbol(sym))
...
def enter_decision_ready(decision: Dict[str, Any]) -> tuple[bool, str]:
    ...
    action = review_action(plan)
    instruction = str(plan.get("bot_instruction") or "").strip().upper()
    should_trade = bool(plan.get("should_trade", False))
    timing_state = str(plan.get("timing_state") or "").strip().lower()
    if action != "enter":
        return False, "action_{}".format(action or "none")
    if instruction not in ("ENTER_MARKET", "ENTER_LIMIT"):
        return False, "instruction_not_enter"
    if not should_trade:
        return False, "should_trade_false"
    if timing_state and timing_state != "enter_now":
        return False, "timing_not_enter_now"
    return True, "ok"
...
def candidate_review_gate(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if status == "observe":
        ...
        reason_text = (
            "manual_send_requested_at={}".format(tw_from_ts(manual_requested_ts))
            if allow_recheck
            else "manual_only_waiting_until={}".format(tw_from_ts(expires_ts))
        )
        ...
        with REVIEW_LOCK:
            if symbol in REVIEW_TRACKER:
                ...
                REVIEW_TRACKER[symbol]["trigger_ready"] = False
...
def _build_position_rule_from_live_position(position: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    if stop_loss <= 0:
        stop_loss = entry * (0.985 if side == "long" else 1.015)
...
def process_position_rules(positions_payload: List[Dict[str, Any]]) -> None:
    ...
    for symbol, position in active_map.items():
        ...
        if side == "long" and (stop <= 0 or stop >= entry):
            repaired_stop = safe_float(rule.get("risk_reference_stop_loss"), 0.0)
            if repaired_stop <= 0 or repaired_stop >= entry:
                repaired_stop = entry * 0.985
            stop = repaired_stop
            rule["initial_stop_loss"] = stop
            rule["risk_reference_stop_loss"] = stop
            print("[POSITION_RULE_FIX] symbol={} side=long repaired invalid stop to {}".format(symbol, round(stop, 8)))
        elif side == "short" and (stop <= 0 or stop <= entry):
            repaired_stop = safe_float(rule.get("risk_reference_stop_loss"), 0.0)
            if repaired_stop <= entry:
                repaired_stop = entry * 1.015
            stop = repaired_stop
            rule["initial_stop_loss"] = stop
            rule["risk_reference_stop_loss"] = stop
            print("[POSITION_RULE_FIX] symbol={} side=short repaired invalid stop to {}".format(symbol, round(stop, 8)))
...
def _apply_openai_trade_plan_to_signal(sig: Dict[str, Any], decision: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    scanner_side = str(sig.get("scanner_side") or sig.get("side") or "").lower()
    if scanner_side in ("long", "short"):
        sig["scanner_side"] = scanner_side
    ...
    elif str(plan.get("bot_instruction") or "").upper() in ("ENTER_MARKET", "ENTER_LIMIT"):
        sig["side"] = str(sig.get("side") or "long").lower()
    ...
    guarded_stop, stop_guard_reason = _enforce_structure_stop_guard(
        str(sig.get("side") or ""),
        safe_float(sig.get("planned_entry_price"), safe_float(sig.get("price"), 0.0)),
        safe_float(sig.get("stop_loss"), 0.0),
        dict(sig.get("openai_market_context") or {}),
    )
    if guarded_stop > 0 and not math.isclose(guarded_stop, safe_float(sig.get("stop_loss"), 0.0), rel_tol=1e-6, abs_tol=max(abs(guarded_stop) * 1e-7, 1e-9)):
        sig["stop_loss"] = guarded_stop
        plan["stop_loss"] = guarded_stop
        note = "本地風控改用 15m 結構止損（前幾根 K swing），避免過近掃損。"
        existing_reason = str(plan.get("stop_loss_reason") or "").strip()
        plan["stop_loss_reason"] = note if not existing_reason else "{} {}".format(note, existing_reason)
    sig["stop_loss_guard_reason"] = stop_guard_reason
    sig["ai_take_profit_hint"] = float(plan.get("take_profit", sig.get("take_profit", 0)) or sig.get("take_profit", 0))
    entry_ref = safe_float(sig.get("planned_entry_price"), safe_float(sig.get("price"), 0.0))
    local_take_profit = _local_take_profit_from_r(str(sig.get("side") or ""), entry_ref, safe_float(sig.get("stop_loss"), 0.0), plan)
    sig["take_profit"] = local_take_profit if local_take_profit > 0 else float(plan.get("take_profit", sig.get("take_profit", 0)) or sig.get("take_profit", 0))
    sig["take_profit_source"] = "local_r_from_ai_stop"
...
def place_order_from_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with ORDER_LOCK:
        ...
        instruction = str(decision.get("bot_instruction") or "").strip().upper()
        scanner_side = _side_from_value(signal.get("scanner_side") or signal.get("side"))
        decision_side = _side_from_value(decision.get("trade_side"))
        if decision_side not in ("long", "short"):
            decision_side = scanner_side if scanner_side in ("long", "short") else _side_from_value(signal.get("side")) or "long"
        source = str(signal.get("candidate_source") or "").lower()
        if source in ("short_gainers", "pending_advice") and scanner_side in ("long", "short") and decision_side != scanner_side:
            return {"ok": False, "error": "source_side_locked:{}->{}".format(scanner_side, decision_side)}
        active_symbols = open_position_symbols()
        pending_symbols = pending_order_symbols()
        if symbol in active_symbols or symbol in pending_symbols:
            return {"ok": False, "error": "already_active"}
        exposure = exposure_snapshot()
        if safe_int(exposure.get("total"), 0) >= MAX_OPEN_POSITIONS:
            return {"ok": False, "error": "max_positions"}
        side_total = safe_int(exposure.get("long_total"), 0) if decision_side == "long" else safe_int(exposure.get("short_total"), 0)
        if side_total >= MAX_SAME_DIRECTION_POSITIONS:
            return {"ok": False, "error": "max_same_direction"}
        if _is_btc_drive_signal(signal):
            if btc_beta_cluster_count(exposure, decision_side) >= BTC_BETA_MAX_CONCURRENT:
                return {"ok": False, "error": "max_btc_beta_cluster"}
        ...
        if order_type == "market":
            chase_risk, chase_reason = _local_market_entry_chase_risk(
                decision_side,
                symbol,
                safe_float(signal.get("price"), 0.0),
                dict(signal.get("openai_market_context") or {}),
            )
            if chase_risk:
                pullback_entry = _derive_pullback_limit_entry(
                    decision_side,
                    safe_float(signal.get("price"), 0.0),
                    dict(signal.get("openai_market_context") or {}),
                )
                if pullback_entry > 0:
                    signal["planned_entry_price"] = pullback_entry
                    signal["price"] = pullback_entry
                    decision["order_type"] = "limit"
                    decision["bot_instruction"] = "ENTER_LIMIT"
                    decision["entry_price"] = pullback_entry
                    signal["anti_chase_adjusted"] = True
                    signal["anti_chase_reason"] = chase_reason
                    order_type = "limit"
                else:
                    return {"ok": False, "error": "anti_chase_market_blocked:{}".format(chase_reason or "entry_stretched")}
        ...
        if guarded_stop > 0:
            signal["stop_loss"] = guarded_stop
            decision["stop_loss"] = guarded_stop
        signal["stop_loss_guard_reason"] = stop_guard_reason
        signal["ai_take_profit_hint"] = safe_float(decision.get("take_profit"), safe_float(signal.get("ai_take_profit_hint"), 0.0))
        local_take_profit = _local_take_profit_from_r(decision_side, signal["price"], signal["stop_loss"], decision)
        signal["take_profit"] = local_take_profit if local_take_profit > 0 else safe_float(decision.get("take_profit"), safe_float(signal.get("take_profit"), 0.0))
        signal["take_profit_source"] = "local_r_from_ai_stop"
        if not _levels_consistent_for_side(decision_side, signal["price"], signal["stop_loss"], signal["take_profit"]):
            err = _execution_level_error(decision_side, signal["price"], signal["stop_loss"], signal["take_profit"])
            print(
                "[ORDER_BLOCKED] symbol={} instruction={} side={} reason={} entry={} stop={} tp={}".format(
                    symbol,
                    instruction or "UNKNOWN",
                    decision_side,
                    err,
                    round(safe_float(signal.get("price"), 0.0), 8),
                    round(safe_float(signal.get("stop_loss"), 0.0), 8),
                    round(safe_float(signal.get("take_profit"), 0.0), 8),
                )
            )
            return {"ok": False, "error": "invalid_levels:{}".format(err)}
        ...
        try:
            ...
            fill_take_profit = _local_take_profit_from_r(decision_side, signal["price"], signal["stop_loss"], decision)
            if fill_take_profit > 0:
...
def manage_pending_limit_orders() -> None:
    ...
    for symbol, row in pending.items():
        ...
        if status in ("closed", "filled"):
            ...
            local_take_profit = _local_take_profit_from_r(side, entry_price, stop_loss, decision)
...
def choose_review_candidates(general_top: List[Dict[str, Any]], short_gainers: List[Dict[str, Any]], pending_advice_signals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if safe_int(snapshot.get("total"), 0) >= MAX_OPEN_POSITIONS:
        return candidates
    virtual_long = safe_int(snapshot.get("long_total"), 0)
    virtual_short = safe_int(snapshot.get("short_total"), 0)
    virtual_btc_beta_long = btc_beta_cluster_count(snapshot, "long")
    virtual_btc_beta_short = btc_beta_cluster_count(snapshot, "short")
    ...
    for pool_name, pool_rows in pools:
        for row in pool_rows:
            ...
            if not side:
                side = "short" if safe_float(row.get("score"), 0.0) < 0 else "long"
            picked = dict(row)
            picked["side"] = side
            picked["queue_pool"] = pool_name
            picked["queue_rank"] = safe_int(picked.get("rank"), 0)

            side_total = virtual_long if side == "long" else virtual_short
            if side_total >= MAX_SAME_DIRECTION_POSITIONS:
                picked["force_openai_recheck"] = False
                picked["local_gate_ok"] = False
                picked["local_gate_reason"] = "same_direction_full"
            ...
            btc_cluster = virtual_btc_beta_long if side == "long" else virtual_btc_beta_short
            if _is_btc_drive_signal(picked) and btc_cluster >= BTC_BETA_MAX_CONCURRENT:
                picked["force_openai_recheck"] = False
                picked["local_gate_ok"] = False
                picked["local_gate_reason"] = "btc_beta_cluster_full"
                picked["local_gate_next_allowed_ts"] = now_ts() + ORDER_BLOCK_EXPIRY_SEC
                picked["local_gate_next_recheck_ts"] = 0.0
                picked["observe_expires_ts"] = 0.0
                candidates.append(picked)
            ...
            if bool(gate.get("ok", False)):
                if side == "long":
                    virtual_long += 1
                    if _is_btc_drive_signal(picked):
                        virtual_btc_beta_long += 1
                else:
                    virtual_short += 1
                    if _is_btc_drive_signal(picked):
                        virtual_btc_beta_short += 1
    return candidates
...
def maybe_run_openai(general_top: List[Dict[str, Any]], short_gainers: List[Dict[str, Any]], pending_advice_signals: List[Dict[str, Any]]) -> None:
    ...
    for reviewed in reviewed_candidates:
        if not reviewed.get("local_gate_ok"):
            ...
            if str(reviewed.get("local_gate_reason") or "") in ("capacity_blocked", "same_direction_full", "btc_beta_cluster_full"):
                next_ts = safe_float(reviewed.get("local_gate_next_allowed_ts"), 0.0)
                if next_ts > 0:
            ...
            continue
        reviewed["scanner_side"] = str(reviewed.get("scanner_side") or reviewed.get("side") or "").lower()
        ...
        expected_side = str(reviewed.get("scanner_side") or reviewed.get("side") or "").lower()
        side_lock = str(reviewed.get("candidate_source") or "") in ("short_gainers", "pending_advice")
        ...
        try:
            ...
            if decision:
                ...
                if action == "enter":
                    plan_ok, plan_reason = enter_decision_ready(dict(reviewed.get("openai_trade_plan") or {}))
                    if not plan_ok:
                        order_result = {"ok": False, "error": "non_executable_enter:{}".format(plan_reason)}
                    else:
                    ...
                    if not bool(order_result.get("ok", False)):
                        ...
                        if any(
                            ...
                            for prefix in ("max_positions", "max_same_direction", "max_btc_beta_cluster", "already_active")
                        ):
...
def perform_scan_cycle() -> None:
    ...
    try:
        markets, tickers, snapshot_note = fetch_exchange_snapshot()
        if snapshot_note:
        ...
        for idx, (symbol, market, ticker) in enumerate(selected_union, start=1):
            update_state(scan_progress="Scanning {}/{} {}".format(idx, len(selected_union), compact_symbol(symbol)), _persist=False)
            try:
