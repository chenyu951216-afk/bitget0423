...
_copy_role_raw = str(env_or_blank("BITGET_COPY_ROLE", "") or "").strip().lower()
if not _copy_role_raw:
    if BITGET_API_MODE in {"mix", "swap", "standard"}:
        _copy_role_raw = "mix"
    elif BITGET_API_MODE in {"public_copy", "copy_public", "public"}:
        _copy_role_raw = "public_copy"
    elif BITGET_API_MODE in {"copy_follower", "follower"}:
        _copy_role_raw = "follower"
    else:
        _copy_role_raw = "trader"
if _copy_role_raw not in {"trader", "follower", "mix", "public_copy"}:
    _copy_role_raw = "trader"
BITGET_COPY_ROLE = _copy_role_raw
BITGET_ROLE_IS_TRADER = BITGET_COPY_ROLE == "trader"
BITGET_ROLE_IS_FOLLOWER = BITGET_COPY_ROLE == "follower"
BITGET_ROLE_IS_MIX = BITGET_COPY_ROLE == "mix"
BITGET_ROLE_IS_PUBLIC_COPY = BITGET_COPY_ROLE == "public_copy"
BITGET_ROLE_USE_MIX_ENDPOINTS = BITGET_ROLE_IS_MIX or BITGET_ROLE_IS_PUBLIC_COPY
BITGET_USE_COPY_TRADER_API = BITGET_ROLE_IS_TRADER
...
BITGET_ENDPOINTS: Dict[str, str] = {
    ...
    "copy_follower_current_orders": "/api/v2/copy/mix-follower/query-current-orders",
    "copy_follower_close_positions": "/api/v2/copy/mix-follower/close-positions",
    ...
    "mix_all_positions": "/api/v2/mix/position/all-position",
...
OPEN_INTEREST_HISTORY: Dict[str, List[Dict[str, float]]] = {}
ORDERBOOK_METRIC_HISTORY: Dict[str, List[Dict[str, float]]] = {}
FLOW_METRIC_HISTORY: Dict[str, List[Dict[str, float]]] = {}
...
def _append_metric_history(
    ...
    *,
    ...
    rows = list(bucket.get(token) or [])
    snapshot: Dict[str, float] = {"ts": safe_float(row.get("ts"), now_ts_val)}
    for key, value in dict(row or {}).items():
        if key == "ts":
            continue
        snapshot[key] = safe_float(value, 0.0)
    rows.append(snapshot)
    rows = [
        r
        for r in rows
        if (now_ts_val - safe_float(r.get("ts"), 0.0)) <= max(int(ttl_sec), 60)
    ][-max(int(limit), 20):]
    bucket[token] = rows
    return rows


def _history_value_ago(rows: List[Dict[str, float]], key: str, seconds: int, now_ts_val: float) -> float:
    cutoff = now_ts_val - max(int(seconds), 1)
    candidates = [r for r in list(rows or []) if safe_float(r.get("ts"), 0.0) <= cutoff]
    if not candidates:
        return 0.0
    return safe_float(candidates[-1].get(key), 0.0)


def _active_bitget_endpoint_map() -> Dict[str, str]:
    mapping: Dict[str, str] = {
        "mix_place_order": BITGET_ENDPOINTS["mix_place_order"],
        "mix_order_detail": BITGET_ENDPOINTS["mix_order_detail"],
        "mix_cancel_order": BITGET_ENDPOINTS["mix_cancel_order"],
        "spot_account_info": BITGET_ENDPOINTS["spot_account_info"],
    }
    if BITGET_ROLE_IS_TRADER:
        mapping.update(
            {
                "copy_trader_open_order": BITGET_ENDPOINTS["copy_trader_open_order"],
                "copy_current_track": BITGET_ENDPOINTS["copy_current_track"],
                "copy_close_positions": BITGET_ENDPOINTS["copy_close_positions"],
                "copy_modify_tpsl": BITGET_ENDPOINTS["copy_modify_tpsl"],
                "copy_symbol_settings": BITGET_ENDPOINTS["copy_symbol_settings"],
                "copy_total_detail": BITGET_ENDPOINTS["copy_total_detail"],
            }
        )
    elif BITGET_ROLE_IS_FOLLOWER:
        mapping.update(
            {
                "copy_follower_current_orders": BITGET_ENDPOINTS["copy_follower_current_orders"],
            }
        )
    elif BITGET_ROLE_IS_PUBLIC_COPY:
        mapping.update(
            {
                "mix_all_positions": BITGET_ENDPOINTS["mix_all_positions"],
                "mix_accounts": BITGET_ENDPOINTS["mix_accounts"],
                "mix_place_order": BITGET_ENDPOINTS["mix_place_order"],
            }
        )
    else:
...
def _emit_bitget_startup_config() -> None:
    endpoint_map = _active_bitget_endpoint_map()
    print(
        "[BITGET_CONFIG] BITGET_API_MODE={} BITGET_COPY_ROLE={} BITGET_PRODUCT_TYPE={}".format(
            BITGET_API_MODE,
            BITGET_COPY_ROLE,
            BITGET_PRODUCT_TYPE,
        )
    ...
    for key, endpoint in endpoint_map.items():
        print("[BITGET_ENDPOINT] {}={}".format(key, endpoint))
...
def request(method: str, endpoint: str, *, params: Dict[str, Any] | None = None, payload: Dict[str, Any] | None = None) -> Any:
    ...
    if code == "40037" and "apikey does not exist" in msg.lower():
        msg = "API 身份 endpoint 不匹配：目前 API key 不適用此 endpoint，請檢查 trader/follower/mix/public_copy 身份是否選錯"
    if code == "40014" and ("incorrect permissions" in msg.lower() or "trace read" in msg.lower() or "trace write" in msg.lower()):
        msg = "API 身份/權限 endpoint 不匹配：目前 API key 不適用 trace endpoint，請檢查 trader/follower/mix/public_copy 身份是否選錯"
...
def _require_copy_trader_mode(caller: str) -> None:
    if BITGET_ROLE_IS_TRADER:
        return
    raise RuntimeError(
        "{}_requires_copy_trader_mode:BITGET_API_MODE={} BITGET_COPY_ROLE={}".format(
...
def _extract_rows_payload(data: Any) -> List[Dict[str, Any]]:
...
def get_balance() -> Dict[str, float]:
    ...
    if BITGET_ROLE_USE_MIX_ENDPOINTS:
        return _balance_from_mix_accounts_rows(_bitget_mix_fetch_accounts())
    try:
        ...
        return {"available": 0.0, "equity": 0.0, "margin": 0.0, "unrealized_pnl": 0.0}


def get_positions(symbol: str = "") -> Dict[str, Dict[str, float]]:
    if BITGET_ROLE_IS_TRADER:
        rows = _bitget_copy_fetch_current_tracks(symbol)
    elif BITGET_ROLE_IS_FOLLOWER:
        rows = _bitget_follower_fetch_current_orders(symbol)
    elif BITGET_ROLE_USE_MIX_ENDPOINTS:
        rows = _bitget_mix_fetch_all_positions(symbol)
    else:
        rows = []
    ...
    for row in rows:
        ...
        size = abs(
            safe_float(
                row.get("openSize", row.get("total", row.get("available"))),
                0.0,
            )
        )
        if size <= 0:
            continue
        entry = safe_float(row.get("openPriceAvg", row.get("avgOpenPrice")), 0.0)
        margin = abs(safe_float(row.get("openMarginSz", row.get("marginSize")), 0.0))
        unrealized = safe_float(
            row.get("unrealizedPL", row.get("unrealizedPnl", row.get("unrealizedProfit"))),
            0.0,
        )
...
def _bitget_copy_fetch_current_tracks(symbol: str = "") -> List[Dict[str, Any]]:
    ...
    return _extract_rows_payload(data)


def _bitget_follower_fetch_current_orders(symbol: str = "") -> List[Dict[str, Any]]:
    if not BITGET_ROLE_IS_FOLLOWER:
        return []
    query: Dict[str, Any] = {
        "productType": BITGET_PRODUCT_TYPE,
        "pageSize": "50",
    ...
    if symbol_id:
        query["symbol"] = symbol_id
    data = _bitget_private_request("GET", BITGET_ENDPOINTS["copy_follower_current_orders"], query=query)
    return _extract_rows_payload(data)


def _bitget_mix_fetch_all_positions(symbol: str = "") -> List[Dict[str, Any]]:
    query: Dict[str, Any] = {
        "productType": BITGET_PRODUCT_TYPE,
        "marginCoin": BITGET_MARGIN_COIN,
    }
    ...
    if symbol_id:
        query["symbol"] = symbol_id
    data = _bitget_private_request("GET", BITGET_ENDPOINTS["mix_all_positions"], query=query)
    return _extract_rows_payload(data)


def _bitget_mix_fetch_accounts() -> List[Dict[str, Any]]:
    query: Dict[str, Any] = {
        "productType": BITGET_PRODUCT_TYPE,
        "marginCoin": BITGET_MARGIN_COIN,
    }
    data = _bitget_private_request("GET", BITGET_ENDPOINTS["mix_accounts"], query=query)
    return _extract_rows_payload(data)


def _balance_from_mix_accounts_rows(rows: List[Dict[str, Any]]) -> Dict[str, float]:
    equity = 0.0
    available = 0.0
    margin = 0.0
    for row in list(rows or []):
        item = dict(row or {})
        equity += safe_float(
            item.get("accountEquity", item.get("equity", item.get("totalEquity"))),
            0.0,
        )
        available += safe_float(
            item.get("available", item.get("availableBalance", item.get("free"))),
            0.0,
        )
        margin += safe_float(
            item.get("crossedMargin", item.get("usedMargin", item.get("frozen"))),
            0.0,
        )
    if equity <= 0 and (available > 0 or margin > 0):
        equity = available + margin
    return {
        "available": round(max(available, 0.0), 8),
        "equity": round(max(equity, 0.0), 8),
        "margin": round(max(margin, 0.0), 8),
        "unrealized_pnl": 0.0,
    }
...
def _bitget_copy_total_equity_usdt() -> float:
    ...
    if BITGET_ROLE_USE_MIX_ENDPOINTS:
        return safe_float(_balance_from_mix_accounts_rows(_bitget_mix_fetch_accounts()).get("equity"), 0.0)
    return 0.0
...
def _create_contract_order(
    ...
    *,
    ...
    errors: List[str] = []
    if not reduce_only:
        lev, _, lev_error, lev_ok = _force_set_symbol_max_leverage(symbol, side)
        if not lev_ok and not BITGET_USE_COPY_TRADER_API:
            raise RuntimeError("leverage_failed:{}".format(lev_error or "unknown"))
    ...
    if BITGET_ROLE_IS_PUBLIC_COPY:
        trade_side = "close" if reduce_only else "open"
        side_text = str(side or "").strip().lower()
        hold_side = "long" if (side_text == "buy") == (not reduce_only) else "short"
        payload: Dict[str, Any] = {
            "symbol": symbol_contract_id(symbol),
            "productType": BITGET_PRODUCT_TYPE,
            "marginMode": BITGET_MARGIN_MODE,
...
def build_liquidity_context(symbol: str, ticker: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = {
        ...
        "recent_trades": {
            ...
            "sell_notional": 0.0,
            "buy_notional_5m": 0.0,
            "sell_notional_5m": 0.0,
            "buy_notional_15m": 0.0,
            "sell_notional_15m": 0.0,
            "trade_count": 0,
    ...
    buy_5m_notional = 0.0
    sell_5m_notional = 0.0
    buy_15m_notional = 0.0
    sell_15m_notional = 0.0
    ...
    try:
        ...
        for trade in trades:
            ...
            if age_ms <= 5 * 60 * 1000:
                vol_5m += notional
                if side == "buy":
                    buy_5m_notional += notional
                elif side == "sell":
                    sell_5m_notional += notional
            elif age_ms <= 10 * 60 * 1000:
                vol_prev_5m += notional
            if age_ms <= 15 * 60 * 1000:
                vol_15m += notional
                if side == "buy":
                    buy_15m_notional += notional
                elif side == "sell":
                    sell_15m_notional += notional
        ...
        result["recent_trades"] = {
            ...
            "buy_notional_5m": round(buy_5m_notional, 4),
            "sell_notional_5m": round(sell_5m_notional, 4),
            "buy_notional_15m": round(buy_15m_notional, 4),
            "sell_notional_15m": round(sell_15m_notional, 4),
...
def build_derivatives_context(symbol: str, ticker: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = {
        ...
        "open_interest_change_pct_1h": None,
    ...
    oi_rows = _append_metric_history(
        OPEN_INTEREST_HISTORY,
        symbol,
        {"ts": now_ts_val, "oi": curr_oi},
        ttl_sec=2 * 3600,
        limit=420,
    )
    if curr_oi > 0:
        for field, seconds in (
            ("open_interest_change_pct_5m", 5 * 60),
            ("open_interest_change_pct_15m", 15 * 60),
            ("open_interest_change_pct_1h", 60 * 60),
        ):
            prev_val = _history_value_ago(oi_rows, "oi", seconds, now_ts_val)
            if prev_val > 0:
                result[field] = round(((curr_oi / prev_val) - 1.0) * 100.0, 4)
    ...
    try:
        ...
        pass
    try:
        fetch_liquidations = getattr(exchange, "fetch_liquidations", None)
        if callable(fetch_liquidations) and safe_float(result.get("liquidation_volume_24h"), 0.0) <= 0:
            liq_rows = list(fetch_liquidations(symbol, limit=200) or [])
            if liq_rows:
                ...
                for row in liq_rows:
                    ...
                    liq_notional += max(amount * price, 0.0)
                if liq_notional > 0:
                    result["liquidation_volume_24h"] = round(liq_notional, 4)
                    result["liquidation_map_status"] = "from_liquidations"
    except Exception:
        pass
    try:
        fetch_ls_hist = getattr(exchange, "fetch_long_short_ratio_history", None)
        if callable(fetch_ls_hist):
            ls_rows = list(fetch_ls_hist(symbol, timeframe="5m", limit=4) or [])
            if ls_rows:
                latest = dict(ls_rows[-1] or {})
                ls_hist_value = safe_float(
                    latest.get("longShortRatio", latest.get("long_short_ratio")),
                    0.0,
                )
                if ls_hist_value > 0 and safe_float(result.get("long_short_ratio"), 0.0) <= 0:
                    result["long_short_ratio"] = round(ls_hist_value, 4)
                if ls_hist_value > 0 and safe_float(result.get("top_trader_long_short_ratio"), 0.0) <= 0:
                    result["top_trader_long_short_ratio"] = round(ls_hist_value, 4)
    except Exception:
        pass
...
def build_orderbook_history_context(symbol: str, liquidity: Dict[str, Any], current_price: float) -> Dict[str, Any]:
    now_ts_val = time.time()
    row = {
        "ts": now_ts_val,
        "depth_imbalance_10": safe_float(liquidity.get("depth_imbalance_10"), 0.0),
        "largest_bid_wall_size": safe_float(liquidity.get("largest_bid_wall_size"), 0.0),
        "largest_ask_wall_size": safe_float(liquidity.get("largest_ask_wall_size"), 0.0),
        "largest_bid_wall_price": safe_float(liquidity.get("largest_bid_wall_price"), 0.0),
        "largest_ask_wall_price": safe_float(liquidity.get("largest_ask_wall_price"), 0.0),
        "last_price": safe_float(current_price, 0.0),
    }
    rows = _append_metric_history(
        ORDERBOOK_METRIC_HISTORY,
        symbol,
        row,
        ttl_sec=2 * 3600,
        limit=420,
    )
    depth_now = safe_float(row.get("depth_imbalance_10"), 0.0)
    depth_prev_1m = _history_value_ago(rows, "depth_imbalance_10", 60, now_ts_val)
    depth_prev_5m = _history_value_ago(rows, "depth_imbalance_10", 5 * 60, now_ts_val)
    bid_size_now = safe_float(row.get("largest_bid_wall_size"), 0.0)
    ask_size_now = safe_float(row.get("largest_ask_wall_size"), 0.0)
    bid_size_prev_3m = _history_value_ago(rows, "largest_bid_wall_size", 3 * 60, now_ts_val)
    ask_size_prev_3m = _history_value_ago(rows, "largest_ask_wall_size", 3 * 60, now_ts_val)
    bid_price_prev_1m = _history_value_ago(rows, "largest_bid_wall_price", 60, now_ts_val)
    price_prev_1m = _history_value_ago(rows, "last_price", 60, now_ts_val)
    bid_wall_following_price = bid_price_prev_1m > 0 and safe_float(row.get("largest_bid_wall_price"), 0.0) >= bid_price_prev_1m and safe_float(current_price, 0.0) >= price_prev_1m
    bid_change_3m = ((bid_size_now - bid_size_prev_3m) / max(bid_size_prev_3m, 1e-9)) * 100.0 if bid_size_prev_3m > 0 else 0.0
    ask_change_3m = ((ask_size_now - ask_size_prev_3m) / max(ask_size_prev_3m, 1e-9)) * 100.0 if ask_size_prev_3m > 0 else 0.0
    ask_wall_getting_thinner = ask_change_3m <= -10.0
    wall_pull_or_spoof_risk = abs(ask_change_3m) >= 35.0 or abs(bid_change_3m) >= 35.0
    bid_persistence_sec = 0.0
    ask_persistence_sec = 0.0
    if bid_size_now > 0:
        for prev in reversed(rows):
            if safe_float(prev.get("largest_bid_wall_size"), 0.0) < (bid_size_now * 0.6):
                break
            bid_persistence_sec = max(now_ts_val - safe_float(prev.get("ts"), now_ts_val), 0.0)
    if ask_size_now > 0:
        for prev in reversed(rows):
            if safe_float(prev.get("largest_ask_wall_size"), 0.0) < (ask_size_now * 0.6):
                break
            ask_persistence_sec = max(now_ts_val - safe_float(prev.get("ts"), now_ts_val), 0.0)
    return {
        "depth_imbalance_change_1m": round(depth_now - depth_prev_1m, 6),
        "depth_imbalance_change_5m": round(depth_now - depth_prev_5m, 6),
        "largest_bid_wall_size_change_3m_pct": round(bid_change_3m, 4),
        "largest_ask_wall_size_change_3m_pct": round(ask_change_3m, 4),
        ...
        "bid_wall_persistence_sec": round(bid_persistence_sec, 2),
        "ask_wall_persistence_sec": round(ask_persistence_sec, 2),
    }


def build_flow_context(symbol: str, liquidity: Dict[str, Any]) -> Dict[str, Any]:
    now_ts_val = time.time()
    recent = dict(liquidity.get("recent_trades") or {})
    buy_1m = safe_float(recent.get("buy_notional"), 0.0)
    sell_1m = safe_float(recent.get("sell_notional"), 0.0)
    buy_5m = safe_float(recent.get("buy_notional_5m"), 0.0)
    sell_5m = safe_float(recent.get("sell_notional_5m"), 0.0)
    buy_15m = safe_float(recent.get("buy_notional_15m"), 0.0)
    sell_15m = safe_float(recent.get("sell_notional_15m"), 0.0)
    ...
    prev_cvd_1m = _history_value_ago(rows, "cvd_1m", 60, now_ts_val)
    prev_cvd_5m = _history_value_ago(rows, "cvd_5m", 5 * 60, now_ts_val)
    prev_cvd_15m = _history_value_ago(rows, "cvd_15m", 15 * 60, now_ts_val)
    cvd_slope_1m = cvd_1m - prev_cvd_1m
    cvd_slope_5m = (cvd_5m - prev_cvd_5m) / 5.0
    cvd_slope_15m = (cvd_15m - prev_cvd_15m) / 15.0
    large_buy_count = safe_int(recent.get("large_buy_count"), 0)
    large_sell_count = safe_int(recent.get("large_sell_count"), 0)
    large_trade_count = max(large_buy_count + large_sell_count, 0)
    large_trade_net_side = "buy" if large_buy_count > large_sell_count else "sell" if large_sell_count > large_buy_count else "neutral"
    ratio_1m = buy_1m / max(sell_1m, 1e-9) if (buy_1m > 0 or sell_1m > 0) else 1.0
    ratio_5m = buy_5m / max(sell_5m, 1e-9) if (buy_5m > 0 or sell_5m > 0) else 1.0
    ratio_15m = buy_15m / max(sell_15m, 1e-9) if (buy_15m > 0 or sell_15m > 0) else 1.0
    depth_imbalance = safe_float(liquidity.get("depth_imbalance_10"), 0.0)
    absorption_detected = (cvd_1m > 0 and depth_imbalance < -0.08) or (cvd_1m < 0 and depth_imbalance > 0.08)
    exhaustion_detected = (ratio_1m >= 1.8 or ratio_1m <= 0.55) and large_trade_count >= 2 and abs(cvd_slope_1m) < max(abs(cvd_slope_5m) * 0.35, 1.0)
    return {
        "market_buy_notional_1m": round(buy_1m, 4),
        "market_sell_notional_1m": round(sell_1m, 4),
        "market_buy_notional_5m": round(buy_5m, 4),
        "market_sell_notional_5m": round(sell_5m, 4),
        "market_buy_notional_15m": round(buy_15m, 4),
        "market_sell_notional_15m": round(sell_15m, 4),
        "market_buy_sell_ratio_1m": round(ratio_1m, 4),
        "market_buy_sell_ratio_5m": round(ratio_5m, 4),
        "market_buy_sell_ratio_15m": round(ratio_15m, 4),
        "cvd_notional_1m": round(cvd_1m, 4),
        "cvd_notional_5m": round(cvd_5m, 4),
        "cvd_notional_15m": round(cvd_15m, 4),
        "cvd_slope_1m": round(cvd_slope_1m, 4),
        "cvd_slope_5m": round(cvd_slope_5m, 4),
        "cvd_slope_15m": round(cvd_slope_15m, 4),
        "large_trade_count_1m": int(large_trade_count),
        "large_buy_count_1m": int(large_buy_count),
        "large_sell_count_1m": int(large_sell_count),
        "large_trade_net_side": large_trade_net_side,
        "absorption_detected": bool(absorption_detected),
        "exhaustion_detected": bool(exhaustion_detected),
    }


def _density_bucket(score: float) -> str:
...
def build_market_state_classification(
    *,
    current_price: float,
    multi_timeframe: Dict[str, Any],
    timeframe_bars: Dict[str, Any],
    liquidity: Dict[str, Any],
    orderbook_history: Dict[str, Any],
    flow_context: Dict[str, Any],
    derivatives: Dict[str, Any],
) -> Dict[str, Any]:
    tf5 = dict(multi_timeframe.get("5m") or {})
    tf15 = dict(multi_timeframe.get("15m") or {})
    tf1h = dict(multi_timeframe.get("1h") or {})
    structure = structure_profile({"timeframe_bars": {"15m": list(timeframe_bars.get("15m") or [])}}, "15m")
    bars15 = list(timeframe_bars.get("15m") or [])
    atr15 = max(safe_float(tf15.get("atr"), 0.0), safe_float(current_price, 0.0) * 0.002 if safe_float(current_price, 0.0) > 0 else 0.0)
    recent_resistance = safe_float(tf15.get("recent_structure_high"), safe_float(current_price, 0.0))
    recent_support = safe_float(tf15.get("recent_structure_low"), safe_float(current_price, 0.0))
    bb_width_pct = safe_float(tf15.get("bb_width_pct"), 0.0)
    atr_pct = safe_float(tf15.get("atr_pct"), 0.0)
    adx15 = safe_float(tf15.get("adx"), 0.0)
    vol_ratio_5m = safe_float(tf5.get("vol_ratio"), 0.0)
    vol_ratio_15m = safe_float(tf15.get("vol_ratio"), 0.0)
    volume_anomaly_5m = safe_float(liquidity.get("volume_anomaly_5m"), 1.0)
    volume_build_value = max(vol_ratio_5m, vol_ratio_15m, volume_anomaly_5m, 0.0)
    ratio_1m = safe_float(flow_context.get("market_buy_sell_ratio_1m"), 1.0)
    ratio_5m = safe_float(flow_context.get("market_buy_sell_ratio_5m"), 1.0)
    cvd_slope_1m = safe_float(flow_context.get("cvd_slope_1m"), 0.0)
    cvd_slope_5m = safe_float(flow_context.get("cvd_slope_5m"), 0.0)
    cvd_notional_5m = safe_float(flow_context.get("cvd_notional_5m"), 0.0)
    oi_change_5m = safe_float(derivatives.get("open_interest_change_pct_5m"), 0.0)
    oi_change_15m = safe_float(derivatives.get("open_interest_change_pct_15m"), 0.0)
    funding_rate = safe_float(derivatives.get("funding_rate"), 0.0)
    long_short_ratio = safe_float(derivatives.get("long_short_ratio"), 1.0)

    compression_score = clamp(
        (100.0 - linear_score(bb_width_pct, 2.8, 8.5)) * 0.40
        + (100.0 - linear_score(atr_pct, 0.35, 1.6)) * 0.22
        + (100.0 - linear_score(adx15, 16.0, 32.0)) * 0.20
        + (100.0 - linear_score(vol_ratio_15m, 0.9, 1.9)) * 0.18,
        0.0,
        100.0,
    )
    structure_break_up = bool(structure.get("close_above_prior_high"))
    structure_break_down = bool(structure.get("close_below_prior_low"))
    structure_break = bool(structure_break_up or structure_break_down)
    volume_confirm = bool(volume_build_value >= 1.15)
    oi_confirm = bool(abs(oi_change_5m) >= 0.35 or abs(oi_change_15m) >= 0.75)
    cvd_confirm = False
    if structure_break_up:
        cvd_confirm = bool(cvd_slope_1m > 0 and cvd_slope_5m > 0 and ratio_1m >= 1.01)
    elif structure_break_down:
        cvd_confirm = bool(cvd_slope_1m < 0 and cvd_slope_5m < 0 and ratio_1m <= 0.99)
    else:
        cvd_confirm = bool(abs(cvd_slope_5m) > 0 and (ratio_5m >= 1.08 or ratio_5m <= 0.92))

    last_bar = dict(bars15[-1] or {}) if bars15 else {}
    last_low = safe_float(last_bar.get("low"), safe_float(current_price, 0.0))
    last_high = safe_float(last_bar.get("high"), safe_float(current_price, 0.0))
    last_close = safe_float(last_bar.get("close"), safe_float(current_price, 0.0))
    retest_success = False
    if structure_break_up and recent_resistance > 0:
        touched = last_low <= recent_resistance * 1.0025
        retest_success = bool(touched and last_close > recent_resistance and ratio_1m >= 1.0)
    elif structure_break_down and recent_support > 0:
        touched = last_high >= recent_support * 0.9975
        retest_success = bool(touched and last_close < recent_support and ratio_1m <= 1.0)

    breakout_quality_score = int(
        round(
            clamp(
                (30.0 if structure_break else 0.0)
                + (20.0 if volume_confirm else 0.0)
                + (20.0 if cvd_confirm else 0.0)
                + (15.0 if oi_confirm else 0.0)
                + (15.0 if retest_success else 0.0),
                0.0,
                100.0,
            ),
            0,
    ...
    cvd_impulse_ratio = abs(cvd_slope_1m) + abs(cvd_slope_5m) * 2.0
    cvd_impulse_ratio = cvd_impulse_ratio / max(abs(cvd_notional_5m), 1.0)
    expansion_strength = clamp(
        breakout_quality_score * 0.58
        + linear_score(cvd_impulse_ratio, 0.01, 0.20) * 0.20
        + linear_score(abs(oi_change_5m) + abs(oi_change_15m) * 0.5, 0.25, 6.0) * 0.12
        + linear_score(volume_build_value, 1.0, 2.2) * 0.10,
        0.0,
        100.0,
    )
    expansion_triggered = bool(structure_break and volume_confirm and cvd_confirm)

    recent_rejection_count = 0
    support_reaction_count = 0
    upper_wick_total = 0.0
    lower_wick_total = 0.0
    failed_breakout_recent = False
    for row in bars15[-6:]:
        bar = dict(row or {})
        high = safe_float(bar.get("high"), 0.0)
        low = safe_float(bar.get("low"), 0.0)
        close = safe_float(bar.get("close"), 0.0)
        wick = candle_shape_metrics(bar)
        near_resistance = recent_resistance > 0 and high >= (recent_resistance - atr15 * 0.25)
        near_support = recent_support > 0 and low <= (recent_support + atr15 * 0.25)
        if near_resistance and wick["upper_wick_pct"] >= 38.0:
            recent_rejection_count += 1
            upper_wick_total += wick["upper_wick_pct"]
        if near_support and wick["lower_wick_pct"] >= 38.0:
            support_reaction_count += 1
            lower_wick_total += wick["lower_wick_pct"]
        if recent_resistance > 0 and high > recent_resistance * 1.0008 and close < recent_resistance and wick["upper_wick_pct"] >= 35.0:
            failed_breakout_recent = True
    upper_wick_avg = (upper_wick_total / max(recent_rejection_count, 1)) if recent_rejection_count > 0 else 0.0
    lower_wick_avg = (lower_wick_total / max(support_reaction_count, 1)) if support_reaction_count > 0 else 0.0
    rejection_strength = clamp(
        recent_rejection_count * 18.0
        + upper_wick_avg * 0.45
        + (20.0 if failed_breakout_recent else 0.0),
        0.0,
        100.0,
    )

    bid_depth_10 = max(safe_float(liquidity.get("bid_depth_10"), 0.0), 0.0)
    ask_depth_10 = max(safe_float(liquidity.get("ask_depth_10"), 0.0), 0.0)
    largest_bid_wall_size = max(safe_float(liquidity.get("largest_bid_wall_size"), 0.0), 0.0)
    largest_ask_wall_size = max(safe_float(liquidity.get("largest_ask_wall_size"), 0.0), 0.0)
    bid_persistence = safe_float(orderbook_history.get("bid_wall_persistence_sec"), 0.0)
    ask_persistence = safe_float(orderbook_history.get("ask_wall_persistence_sec"), 0.0)
    bid_wall_change = safe_float(orderbook_history.get("largest_bid_wall_size_change_3m_pct"), 0.0)
    ask_wall_change = safe_float(orderbook_history.get("largest_ask_wall_size_change_3m_pct"), 0.0)
    wall_pull_or_spoof_risk = bool(orderbook_history.get("wall_pull_or_spoof_risk", False))

    resistance_score = clamp(
        rejection_strength * 0.55
        + linear_score(ask_depth_10 / max(bid_depth_10, 1e-9), 1.0, 2.5) * 0.22
        + linear_score(largest_ask_wall_size / max(largest_bid_wall_size, 1e-9), 1.0, 2.5) * 0.13
        + (10.0 if failed_breakout_recent else 0.0),
        0.0,
        100.0,
    )
    support_score = clamp(
        (support_reaction_count * 16.0 + lower_wick_avg * 0.40) * 0.55
        + linear_score(bid_depth_10 / max(ask_depth_10, 1e-9), 1.0, 2.5) * 0.30
        + linear_score(largest_bid_wall_size / max(largest_ask_wall_size, 1e-9), 1.0, 2.5) * 0.15,
        0.0,
        100.0,
    )
    resistance_density = _density_bucket(resistance_score)
    support_density = _density_bucket(support_score)
    supply_present = bool(recent_rejection_count >= 2 or failed_breakout_recent or resistance_density == "high")

    liquidity_vacuum_up = bool(
        ask_depth_10 <= max(bid_depth_10 * 0.65, 1e-9)
        and (ask_persistence <= 90.0 or bool(orderbook_history.get("ask_wall_getting_thinner", False)))
    )
    liquidity_vacuum_down = bool(
        bid_depth_10 <= max(ask_depth_10 * 0.65, 1e-9)
        and (bid_persistence <= 90.0 or bid_wall_change <= -12.0)
    )

    expansion_state = "none"
    if (failed_breakout_recent and rejection_strength >= 45.0) or (bool(flow_context.get("exhaustion_detected", False)) and volume_build_value >= 1.2):
        expansion_state = "exhausted"
    elif expansion_triggered and expansion_strength >= 62.0:
        expansion_state = "expanding"
    elif compression_score >= 64.0 and not structure_break:
        expansion_state = "pre_breakout"

    move_type = "pullback"
    if expansion_state in ("pre_breakout", "expanding") and breakout_quality_score >= 55:
        move_type = "expansion"
    else:
        tf15_trend = str(tf15.get("trend_label") or "neutral")
        tf1h_trend = str(tf1h.get("trend_label") or "neutral")
        opposite_break = (structure_break_up and tf15_trend == "downtrend") or (structure_break_down and tf15_trend == "uptrend")
        if structure_break and (opposite_break or (tf1h_trend not in ("neutral", tf15_trend))):
            move_type = "reversal"
    move_confidence = 0.0
    if move_type == "expansion":
        move_confidence = max(expansion_strength, safe_float(breakout_quality_score, 0.0))
    elif move_type == "reversal":
        move_confidence = clamp(45.0 + rejection_strength * 0.35 + (18.0 if structure_break else 0.0), 0.0, 100.0)
    else:
        move_confidence = clamp(
            45.0
            + (18.0 if str(tf15.get("trend_label") or "") in ("uptrend", "downtrend") else 6.0)
            + (12.0 if not structure_break else 0.0)
            + (10.0 if abs(cvd_slope_1m) <= max(abs(cvd_slope_5m) * 0.65, 1.0) else 0.0),
            0.0,
            100.0,
        )

    fakeout_reasons: List[str] = []
    fakeout_score = 0.0
    if structure_break and not volume_confirm:
        fakeout_score += 22.0
        fakeout_reasons.append("breakout_without_volume_continuation")
    if structure_break and not oi_confirm:
        fakeout_score += 18.0
        fakeout_reasons.append("breakout_without_oi_expansion")
    if structure_break and not cvd_confirm:
        fakeout_score += 18.0
        fakeout_reasons.append("breakout_without_cvd_confirmation")
    if failed_breakout_recent:
        fakeout_score += 18.0
        fakeout_reasons.append("recent_failed_breakout_with_rejection_wick")
    if wall_pull_or_spoof_risk:
        fakeout_score += 12.0
        fakeout_reasons.append("orderbook_wall_pull_or_spoof_risk")
    if bool(flow_context.get("exhaustion_detected", False)):
        fakeout_score += 12.0
        fakeout_reasons.append("flow_exhaustion_detected")
    if structure_break_up and resistance_density == "high":
        fakeout_score += 16.0
        fakeout_reasons.append("upside_resistance_density_high")
    if structure_break_down and support_density == "high":
        fakeout_score += 16.0
        fakeout_reasons.append("downside_support_density_high")
    if abs(funding_rate) >= 0.0018:
        fakeout_score += 8.0
        fakeout_reasons.append("funding_rate_extreme")
    if long_short_ratio >= 2.4 or long_short_ratio <= 0.45:
        fakeout_score += 8.0
        fakeout_reasons.append("long_short_ratio_extreme")
    if not fakeout_reasons:
        fakeout_reasons.append("no_major_fakeout_signal")
    fakeout_score = clamp(fakeout_score, 0.0, 100.0)

    decision_is_expansion = bool(
        expansion_state in ("pre_breakout", "expanding")
        and breakout_quality_score >= 55
        and fakeout_score < 55.0
    )
    decision_is_pullback_only = bool(move_type == "pullback" and not decision_is_expansion)
    should_wait_for_breakout = bool((not structure_break) or breakout_quality_score < 50 or fakeout_score >= 60.0)
    ...
    return {
        "expansion_state": {
            ...
            "expansion_strength": round(expansion_strength, 4),
        },
        "breakout_quality": {
            "structure_break": bool(structure_break),
            "volume_confirm": bool(volume_confirm),
            "cvd_confirm": bool(cvd_confirm),
            "oi_confirm": bool(oi_confirm),
            "retest_success": bool(retest_success),
            "score": int(breakout_quality_score),
        },
        "move_type": {
            "type": move_type,
            "confidence": round(move_confidence, 4),
        },
        "resistance_analysis": {
            "recent_rejection_count": int(recent_rejection_count),
            "rejection_strength": round(rejection_strength, 4),
            "supply_present": bool(supply_present),
            "failed_breakout_recent": bool(failed_breakout_recent),
        },
        "liquidity_map": {
            "liquidity_vacuum_up": bool(liquidity_vacuum_up),
            "liquidity_vacuum_down": bool(liquidity_vacuum_down),
            "resistance_density": resistance_density,
            "support_density": support_density,
        },
        "fakeout_risk": {
            "score": round(fakeout_score, 4),
            "reasons": fakeout_reasons[:6],
        },
        "decision_context": {
            "is_expansion_setup": bool(decision_is_expansion),
            "is_pullback_only": bool(decision_is_pullback_only),
            "should_wait_for_breakout": bool(should_wait_for_breakout),
        },
    }
...
def build_risk_context(
    *,
    basic_market: Dict[str, Any],
    liquidity: Dict[str, Any],
    derivatives: Dict[str, Any],
    calculated_metrics: Dict[str, Any],
) -> Dict[str, Any]:
    price = safe_float(basic_market.get("current_price"), 0.0)
    atr_15m = safe_float(calculated_metrics.get("atr_15m"), 0.0)
    atr_pct_15m = ((atr_15m / max(price, 1e-9)) * 100.0) if price > 0 and atr_15m > 0 else 0.0
    spread_pct = safe_float(liquidity.get("spread_pct"), 0.0)
    leverage_heat = safe_float(derivatives.get("leverage_heat_score"), 0.0)
    oi_delta_5m = abs(safe_float(derivatives.get("open_interest_change_pct_5m"), 0.0))
    funding_abs_bps = abs(safe_float(derivatives.get("funding_rate"), 0.0)) * 10000.0
    score = min(
        atr_pct_15m * 6.0
        + spread_pct * 45.0
        + leverage_heat * 0.5
    ...
    if score >= 60:
        ...
        level = "medium"
    return {
        "execution_risk_score": round(score, 2),
        "execution_risk_level": level,
        "atr_pct_15m": round(atr_pct_15m, 4),
        "spread_cost_pct": round(spread_pct, 6),
        "leverage_heat_score": round(leverage_heat, 4),
        "oi_change_abs_5m_pct": round(oi_delta_5m, 4),
        "funding_abs_bps": round(funding_abs_bps, 4),
    }
...
def build_market_context(symbol: str, ticker: Dict[str, Any], market: Dict[str, Any]) -> Dict[str, Any]:
    ...
    basic_market = {
        ...
        "funding_rate": round(safe_float(derivatives.get("funding_rate"), 0.0), 8),
        "open_interest": round(safe_float(derivatives.get("open_interest"), 0.0), 4),
        "open_interest_value_usdt": round(safe_float(derivatives.get("open_interest_value_usdt"), 0.0), 4),
        "long_short_ratio": round(max(safe_float(derivatives.get("long_short_ratio"), 0.0), 0.0), 4),
        "top_trader_long_short_ratio": round(max(safe_float(derivatives.get("top_trader_long_short_ratio"), 0.0), 0.0), 4),
        "whale_position_change_pct": round(safe_float(derivatives.get("whale_position_change_pct"), 0.0), 4),
    }
    tf15 = dict(multi_timeframe.get("15m") or {})
    ...
    orderbook_history = build_orderbook_history_context(symbol, liquidity, price_now)
    flow_context = build_flow_context(symbol, liquidity)
    if safe_float(derivatives.get("long_short_ratio"), 0.0) <= 0:
        flow_ratio_5m = safe_float(flow_context.get("market_buy_sell_ratio_5m"), 0.0)
        if flow_ratio_5m > 0:
            derivatives["long_short_ratio"] = round(flow_ratio_5m, 4)
    if safe_float(derivatives.get("top_trader_long_short_ratio"), 0.0) <= 0 and safe_float(derivatives.get("long_short_ratio"), 0.0) > 0:
        derivatives["top_trader_long_short_ratio"] = round(safe_float(derivatives.get("long_short_ratio"), 0.0), 4)
    if safe_float(derivatives.get("whale_position_change_pct"), 0.0) == 0:
        ratio_1m = safe_float(flow_context.get("market_buy_sell_ratio_1m"), 0.0)
        ratio_5m = safe_float(flow_context.get("market_buy_sell_ratio_5m"), 0.0)
        if ratio_1m > 0 and ratio_5m > 0:
            derivatives["whale_position_change_pct"] = round(((ratio_1m / ratio_5m) - 1.0) * 100.0, 4)
    derivatives["open_interest_change_pct_5m"] = round(safe_float(derivatives.get("open_interest_change_pct_5m"), 0.0), 4)
    derivatives["open_interest_change_pct_15m"] = round(safe_float(derivatives.get("open_interest_change_pct_15m"), 0.0), 4)
    derivatives["open_interest_change_pct_1h"] = round(safe_float(derivatives.get("open_interest_change_pct_1h"), 0.0), 4)
    derivatives["long_short_ratio"] = round(max(safe_float(derivatives.get("long_short_ratio"), 1.0), 0.0), 4)
    if safe_float(derivatives.get("long_short_ratio"), 0.0) <= 0:
        derivatives["long_short_ratio"] = 1.0
    derivatives["top_trader_long_short_ratio"] = round(max(safe_float(derivatives.get("top_trader_long_short_ratio"), derivatives.get("long_short_ratio")), 0.0), 4)
    if safe_float(derivatives.get("top_trader_long_short_ratio"), 0.0) <= 0:
        derivatives["top_trader_long_short_ratio"] = round(safe_float(derivatives.get("long_short_ratio"), 1.0), 4)
    derivatives["whale_position_change_pct"] = round(safe_float(derivatives.get("whale_position_change_pct"), 0.0), 4)
    derivatives["liquidation_volume_24h"] = round(max(safe_float(derivatives.get("liquidation_volume_24h"), 0.0), 0.0), 4)
    basic_market["long_short_ratio"] = derivatives.get("long_short_ratio")
    basic_market["top_trader_long_short_ratio"] = derivatives.get("top_trader_long_short_ratio")
    basic_market["whale_position_change_pct"] = derivatives.get("whale_position_change_pct")
    basic_market["funding_rate"] = round(safe_float(derivatives.get("funding_rate"), 0.0), 8)
    basic_market["open_interest"] = round(safe_float(derivatives.get("open_interest"), 0.0), 4)
    basic_market["open_interest_value_usdt"] = round(safe_float(derivatives.get("open_interest_value_usdt"), 0.0), 4)
    ...
    state_classification = build_market_state_classification(
    ...
    return {
        ...
        "expansion_state": dict(state_classification.get("expansion_state") or {}),
        "breakout_quality": dict(state_classification.get("breakout_quality") or {}),
        "move_type": dict(state_classification.get("move_type") or {}),
        "resistance_analysis": dict(state_classification.get("resistance_analysis") or {}),
        "liquidity_map": dict(state_classification.get("liquidity_map") or {}),
        "fakeout_risk": dict(state_classification.get("fakeout_risk") or {}),
        "decision_context": dict(state_classification.get("decision_context") or {}),
...
def repair_market_context_for_ai(symbol: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        state_classification = build_market_state_classification(
            current_price=safe_float(((repaired.get("basic_market_data") or {}).get("current_price")), 0.0),
            multi_timeframe=dict(repaired.get("multi_timeframe") or {}),
            timeframe_bars=dict(repaired.get("timeframe_bars") or {}),
            liquidity=dict(repaired.get("liquidity_context") or {}),
            orderbook_history=dict(repaired.get("orderbook_history") or {}),
            flow_context=dict(repaired.get("flow_context") or {}),
            derivatives=dict(repaired.get("derivatives_context") or {}),
        )
    except Exception:
        state_classification = {}
    for key in ("expansion_state", "breakout_quality", "move_type", "resistance_analysis", "liquidity_map", "fakeout_risk", "decision_context"):
        if not isinstance(repaired.get(key), dict) or not dict(repaired.get(key) or {}):
            repaired[key] = dict(state_classification.get(key) or {})
    return repaired
...
def sync_positions_once() -> None:
    ...
    try:
        ...
        if BITGET_ROLE_IS_TRADER or BITGET_ROLE_IS_FOLLOWER or BITGET_ROLE_USE_MIX_ENDPOINTS:
            if BITGET_ROLE_IS_TRADER:
                raw_rows = _bitget_copy_fetch_current_tracks()
            elif BITGET_ROLE_IS_FOLLOWER:
                raw_rows = _bitget_follower_fetch_current_orders()
            else:
                raw_rows = _bitget_mix_fetch_all_positions()
            for row in raw_rows:
                size = abs(
                    safe_float(
                        row.get("openSize", row.get("total", row.get("available"))),
                ...
                entry = safe_float(row.get("openPriceAvg", row.get("avgOpenPrice")), 0.0)
                mark = safe_float(row.get("markPrice", row.get("mark_price")), 0.0)
                if mark <= 0:
                    mark = entry
                try:
                    ticker = exchange.fetch_ticker(symbol) if symbol else {}
                    mark = safe_float((ticker or {}).get("last"), mark if mark > 0 else entry)
                except Exception:
                    mark = mark if mark > 0 else entry
                leverage = safe_float(row.get("openLeverage", row.get("leverage")), 1.0)
                unrealized = safe_float(
                    row.get("unrealizedPL", row.get("unrealizedPnl", row.get("unrealizedProfit"))),
                    0.0,
                )
                if unrealized == 0 and entry > 0 and mark > 0 and size > 0:
                    unrealized = (mark - entry) * size if side == "long" else (entry - mark) * size
                pct = safe_float(row.get("achievedProfitsRate", row.get("percentage")), 0.0)
                if pct == 0 and entry > 0:
...
def scale_in_position(symbol: str, side: str, rule: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not lev_ok and not BITGET_USE_COPY_TRADER_API:
        return {"ok": False, "error": "scale_in_leverage_failed:{}".format(lev_error[:120])}
    try:
...
def process_position_rules(positions_payload: List[Dict[str, Any]]) -> None:
    ...
    for symbol, position in active_map.items():
        ...
        if side == "short" and take_profit > 0 and take_profit >= entry:
            ...
            rule["initial_take_profit"] = 0.0
        if stop <= 0:
            emergency_gap = max(entry * 0.006, abs(mark_price - entry) * 1.2, 1e-8)
            stop = entry - emergency_gap if side == "long" else entry + emergency_gap
            rule["initial_stop_loss"] = stop
            rule["risk_reference_stop_loss"] = stop
            rule["active_stop_loss"] = stop
            rule["stop_stage"] = "emergency_recover"
...
def _force_set_symbol_max_leverage(symbol: str, side: str) -> tuple[int, Dict[str, Any], str, bool]:
    ...
    for params in attempts:
        try:
            ...
            if "45054" in lowered or "no change in leverage" in lowered:
                return lev, params, "", True
            if BITGET_USE_COPY_TRADER_API and ("45089" in lowered or "leverage cannot be changed" in lowered):
                return lev, params, "", True
            errors.append(message[:180])
...
def place_order_from_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with ORDER_LOCK:
        ...
        level_error = _execution_level_error(
            decision_side,
            signal["price"],
            safe_float(signal.get("stop_loss"), 0.0),
            safe_float(signal.get("take_profit"), 0.0),
        )
        if level_error:
            return {"ok": False, "error": "invalid_enter_levels:{}".format(level_error)}
        ...
        try:
            if order_type == "limit":
                ...
                if not bool(protection_result.get("sl_ok")):
                    emergency_close = reduce_position(symbol, decision_side, qty, "initial_stop_protection_failed")
                    return {
                        "ok": False,
                        "error": "initial_stop_protection_failed",
                        "detail": str((protection_result or {}).get("sl_error") or ""),
                        "emergency_close_ok": bool((emergency_close or {}).get("ok")),
                    }
...
def manage_pending_limit_orders() -> None:
    ...
    for symbol, row in pending.items():
        ...
        if status in ("closed", "filled"):
            ...
            lev_value, _, lev_error, lev_ok = _force_set_symbol_max_leverage(symbol, order_side)
            if not lev_ok and not BITGET_USE_COPY_TRADER_API:
                clear_pending_order(symbol)
                append_trade_history(
                    {
                        ...
                        "take_profit": 0.0,
                        "pnl_pct": None,
                        "decision_source": "limit_fill_leverage_failed:{}".format(lev_error[:120]),
                    }
                )
                continue
            ...
            if not bool(protection_result.get("sl_ok")):
                reduce_position(symbol, side, safe_float(row.get("qty"), 0.0), "limit_fill_initial_stop_protection_failed")
                clear_pending_order(symbol)
                append_trade_history(
                    {
                        "time": tw_now_str(),
                        ...
                        "decision_source": "limit_fill_protection_failed",
            ...
            initialize_position_rule(
                ...
                safe_float(row.get("qty"), 0.0),
                lev_value if lev_value > 0 else safe_float(row.get("leverage"), 0.0),
                decision,
...
def api_bitget_diag():
    ...
    report: Dict[str, Any] = {
        ...
        "active_endpoints": _active_bitget_endpoint_map(),
        "checks": [],
    }
    ...
    try:
        ...
        _push("role_equity_probe", True, "equity={}".format(total))
    except Exception as exc:
        _push("role_equity_probe", False, str(exc))
    try:
        if BITGET_ROLE_IS_TRADER:
            rows = _bitget_copy_fetch_current_tracks()
            _push("trader_current_track", True, "count={}".format(len(rows)))
        elif BITGET_ROLE_IS_FOLLOWER:
            rows = _bitget_follower_fetch_current_orders()
            _push("follower_current_orders", True, "count={}".format(len(rows)))
        elif BITGET_ROLE_USE_MIX_ENDPOINTS:
            rows = _bitget_mix_fetch_all_positions()
            _push("mix_all_position", True, "count={}".format(len(rows)))
        else:
            _push("role_positions", False, "unsupported_role={}".format(BITGET_COPY_ROLE))
    except Exception as exc:
        _push("role_positions", False, str(exc))
    try:
        balance = get_balance()
