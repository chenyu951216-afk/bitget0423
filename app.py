...
if BITGET_MARGIN_MODE not in {"crossed", "isolated"}:
    BITGET_MARGIN_MODE = "crossed"
BITGET_API_MODE = str(env_or_blank("BITGET_API_MODE", "swap_standard") or "swap_standard").strip().lower()
...
BITGET_ENDPOINTS: Dict[str, str] = {
    # Unified Bitget V2 mix futures endpoints only.
...
def _emit_bitget_startup_config() -> None:
    ...
    print(
        "[BITGET_CONFIG] BITGET_API_MODE={} BITGET_PRODUCT_TYPE={}".format(
            BITGET_API_MODE,
            BITGET_PRODUCT_TYPE,
        )
    )
...
def request(method: str, endpoint: str, *, params: Dict[str, Any] | None = None, payload: Dict[str, Any] | None = None) -> Any:
    ...
    if code == "40037" and "apikey does not exist" in msg.lower():
        msg = "API 身份 endpoint 不匹配：目前 API key 不適用此 endpoint，請改用 V2 mix 合約 endpoint"
    if code == "40014" and ("incorrect permissions" in msg.lower() or "trace read" in msg.lower() or "trace write" in msg.lower()):
        msg = "API 身份/權限 endpoint 不匹配：目前 API key 不適用 trace endpoint，請改用 V2 mix 合約 endpoint"
...
def get_positions(symbol: str = "") -> Dict[str, Dict[str, float]]:
    rows = []
    try:
        raw_positions = _fetch_standard_swap_positions()
        if symbol:
            key_target = symbol_key(symbol)
            raw_positions = [row for row in raw_positions if symbol_key(str(row.get("symbol") or "")) == key_target]
        for row in raw_positions:
            item = dict(row or {})
            side_raw = str(item.get("side") or item.get("holdSide") or "").strip().lower()
            hold = "long" if side_raw == "long" else "short" if side_raw == "short" else ""
            if hold not in {"long", "short"}:
                continue
            rows.append(
                {
                    "holdSide": hold,
                    "openSize": abs(safe_float(item.get("contracts", item.get("openSize")), 0.0)),
                    "openPriceAvg": safe_float(item.get("entryPrice", item.get("openPriceAvg")), 0.0),
                    "openMarginSz": abs(safe_float(item.get("initialMargin", item.get("openMarginSz")), 0.0)),
                    "unrealizedPL": safe_float(item.get("unrealizedPnl", item.get("unrealizedPL")), 0.0),
                }
            )
    except Exception:
        rows = []
...
def place_order(
    ...
    *,
    ...
    order = _create_contract_order(
        symbol,
        str(order_type or "market").strip().lower(),
        side_clean,
        qty,
        price,
        reduce_only=False,
    ...
    return {
        "id": str((order or {}).get("id") or ""),
        "client_oid": str((order or {}).get("client_oid") or client_oid or ""),
        "info": dict((order or {}).get("info") or {}),
    }
...
def close_position(symbol: str, hold_side: str, size: float, *, client_oid: str = "") -> Dict[str, Any]:
    ...
    if qty <= 0:
        raise RuntimeError("invalid_size:{}".format(size))
    _enforce_copy_order_rate_limit()
    side_text = "sell" if hold == "long" else "buy"
    order = _create_contract_order(symbol, "market", side_text, qty, None, reduce_only=True)
    item = dict((order or {}).get("info") or {})
    return {
        "id": str((order or {}).get("id") or item.get("orderId") or item.get("clientOid") or ""),
        "client_oid": str((order or {}).get("client_oid") or client_oid or ""),
        "info": item,
    }
...
def _create_contract_order(
    ...
    *,
    ...
    try:
        order = dict(exchange.create_order(symbol, order_type, side, qty, price, params) or {})
        order_id = str(order.get("id") or order.get("orderId") or "")
        return {"id": order_id, "client_oid": str(order.get("clientOrderId") or order.get("clientOid") or ""), "info": order}
    except Exception as exc:
    ...
    payload: Dict[str, Any] = {
        ...
        "holdSide": "long" if (side_text == "buy") == (not reduce_only) else "short",
    ...
    try:
        ...
        errors.append("mix_v2_place_order:" + str(exc)[:220])
    raise RuntimeError("order_failed:" + " | ".join(errors[:3]))
...
def sync_positions_once() -> None:
    ...
    try:
        ...
        raw_std_positions = _fetch_standard_swap_positions()
        _append_ccxt_positions(positions_payload, raw_std_positions)
    ...
    for params in attempts:
        try:
            ...
            if "45054" in lowered or "no change in leverage" in lowered:
                return lev, params, "", True
            if "45089" in lowered or "leverage cannot be changed" in lowered:
                return lev, params, "", True
...
def ensure_exchange_protection(symbol: str, side: str, qty: float, stop_loss: float, take_profit: float, *, manage_take_profit_locally: bool = True) -> Dict[str, bool]:
    ...
    with STATE_LOCK:
        ...
        protection_state[symbol] = {
            ...
            "sl_mode": "exchange_trigger" if use_exchange_triggers else "local_bot_managed",
            "tp_mode": "local_bot_managed" if manage_take_profit_locally else ("exchange_take_profit" if use_exchange_triggers else "local_bot_managed"),
            "updated_at": tw_now_str(),
...
def manage_pending_limit_orders() -> None:
    ...
    for symbol, row in pending.items():
        ...
        if not order_id and not client_oid:
            ...
            continue
        try:
            order = dict(exchange.fetch_order(order_id, symbol) or {})
        except Exception:
            try:
...
def api_bitget_diag():
    ...
    try:
        ...
        _push("equity_probe", True, "equity={}".format(total))
    except Exception as exc:
        _push("equity_probe", False, str(exc))
    try:
        rows = _bitget_mix_fetch_all_positions()
        _push("mix_all_position", True, "count={}".format(len(rows)))
    except Exception as exc:
        _push("mix_all_position", False, str(exc))
    try:
        balance = get_balance()
        _push("swap_balance", True, "equity={} available={}".format(balance.get("equity"), balance.get("available")))
    except Exception as exc:
        _push("swap_balance", False, str(exc))
    try:
        pos = get_positions()
        _push(
            "positions_2way",
        ...
        _push("positions_2way", False, str(exc))
    try:
