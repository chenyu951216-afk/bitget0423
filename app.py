...
def _refresh_signal_with_latest_snapshot(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not market:
        market = dict(LAST_MARKETS_CACHE.get(symbol) or {})
    if not market:
        return row
    ticker = exchange.fetch_ticker(symbol) or {}
    if not ticker:
        return row
    context = build_market_context(symbol, dict(ticker), market)
    basic = dict(context.get("basic_market_data") or {})
    row["openai_market_context"] = context
    row["price"] = safe_float(basic.get("current_price"), safe_float(ticker.get("last"), safe_float(row.get("price"), 0.0)))
    return row
...
def maybe_run_openai(
    ...
    exposure = exposure_snapshot()
    if safe_int(exposure.get("total_all"), 0) >= MAX_OPEN_POSITIONS:
