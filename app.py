...
ACTIVE_POSITION_PRICE_POLL_SEC = max(2, int(float(env_or_blank("ACTIVE_POSITION_PRICE_POLL_SEC", "5") or 5)))
...
def positions_thread() -> None:
    ...
    while True:
        ...
        now = time.time()
        should_full_sync = active_count == 0 or (now - last_full_sync_ts) >= min(POSITION_INTERVAL_SEC, MAX_ACTIVE_POSITION_SCAN_INTERVAL_SEC)
        if should_full_sync:
            sync_positions_once()
