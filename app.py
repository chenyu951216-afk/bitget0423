...
BITGET_COPY_PRODUCT_TYPE = BITGET_PRODUCT_TYPE.lower()
...
BITGET_API_BASE_URL = str(env_or_blank("BITGET_API_BASE_URL", "https://api.bitget.com") or "https://api.bitget.com").strip().rstrip("/")
BITGET_ENABLE_UTA_ENDPOINTS = str(env_or_blank("BITGET_ENABLE_UTA_ENDPOINTS", "0") or "0").strip().lower() in {"1", "true", "yes", "on"}
...
def _bitget_order_product_types() -> List[str]:
    ...
    if BITGET_USE_COPY_TRADER_API:
        ordered.append(str(BITGET_COPY_PRODUCT_TYPE or "").strip())
        if BITGET_COPY_PERMISSION_FALLBACK_TO_SWAP and not BITGET_COPY_STRICT_ONLY:
            ordered.append(str(BITGET_PRODUCT_TYPE or "").strip())
    else:
        ordered.append(str(BITGET_PRODUCT_TYPE or "").strip())
    out: List[str] = []
...
def _bitget_auth_headers(method: str, request_path: str, query_string: str = "", body_text: str = "") -> Dict[str, str]:
    timestamp = str(int(time.time() * 1000))
    prehash = "{}{}{}".format(timestamp, str(method or "GET").upper(), request_path)
    if query_string:
        prehash += "?" + query_string
    prehash += str(body_text or "")
    secret = str(env_or_blank("BITGET_SECRET", "") or "")
    signature = base64.b64encode(hmac.new(secret.encode("utf-8"), prehash.encode("utf-8"), hashlib.sha256).digest()).decode("utf-8")
    passphrase = str(
...
def _bitget_private_request(method: str, request_path: str, *, query: Dict[str, Any] | None = None, body: Dict[str, Any] | None = None) -> Any:
    clean_query = {str(k): v for k, v in dict(query or {}).items() if v not in (None, "")}
    query_text = urlencode(clean_query)
    payload = dict(body or {})
    body_text = json.dumps(payload, separators=(",", ":"), ensure_ascii=False) if payload else ""
    headers = _bitget_auth_headers(method, request_path, query_text, body_text)
    url = "{}{}".format(BITGET_API_BASE_URL, request_path)
    if query_text:
        url = "{}?{}".format(url, query_text)
    response = requests.request(str(method or "GET").upper(), url, headers=headers, data=body_text if body_text else None, timeout=15)
    ...
    if int(getattr(response, "status_code", 0) or 0) >= 400:
        ...
        msg = str((data or {}).get("msg") or (data or {}).get("message") or raw_text[:240] or "http_error")
        raise RuntimeError("bitget_http_error:{}:{}:{}".format(int(response.status_code), code, msg[:220]))
    if str(data.get("code") or "") != "00000":
        raise RuntimeError("bitget_api_error:{}:{}".format(str(data.get("code") or ""), str(data.get("msg") or data.get("message") or "")[:180]))
    return data.get("data")


def _bitget_spot_account_info() -> Dict[str, Any]:
    data = _bitget_private_request("GET", "/api/v2/spot/account/info")
    return dict(data or {})
...
def _bitget_copy_fetch_current_tracks(symbol: str = "") -> List[Dict[str, Any]]:
    query: Dict[str, Any] = {"productType": BITGET_COPY_PRODUCT_TYPE, "limit": "50"}
    symbol_id = symbol_contract_id(symbol)
    if symbol_id:
        query["symbol"] = symbol_id
    errors: List[str] = []
    data: Any = {}
    for product_type in (BITGET_COPY_PRODUCT_TYPE, BITGET_PRODUCT_TYPE):
        try:
            probe_query = dict(query)
            probe_query["productType"] = product_type
            data = _bitget_private_request("GET", "/api/v2/copy/mix-trader/order-current-track", query=probe_query)
            errors = []
            break
        except Exception as exc:
            ...
            continue
    if errors:
        raise RuntimeError("copy_track_failed:" + " | ".join(errors[:2]))
...
def _bitget_copy_total_equity_usdt() -> float:
    data = _bitget_private_request("GET", "/api/v2/copy/mix-trader/order-total-detail")
    payload = dict(data or {})
    return safe_float(payload.get("totalEquity"), 0.0)


def _bitget_copy_close_tracking(symbol: str, tracking_no: str = "") -> Dict[str, Any]:
    payload: Dict[str, Any] = {"productType": BITGET_COPY_PRODUCT_TYPE}
    symbol_id = symbol_contract_id(symbol)
    if symbol_id:
        payload["symbol"] = symbol_id
    if str(tracking_no or "").strip():
        payload["trackingNo"] = str(tracking_no).strip()
    data = _bitget_private_request("POST", "/api/v2/copy/mix-trader/order-close-positions", body=payload)
    if isinstance(data, list):
        for row in data:
            item = dict(row or {})
...
def _bitget_copy_modify_tpsl(symbol: str, tracking_no: str, stop_loss: float, take_profit: float) -> bool:
    ...
    payload: Dict[str, Any] = {
        ...
        "productType": BITGET_COPY_PRODUCT_TYPE,
    ...
    if "stopSurplusPrice" not in payload and "stopLossPrice" not in payload:
        return False
    try:
        _bitget_private_request("POST", "/api/v2/copy/mix-trader/order-modify-tpsl", body=payload)
        return True
    except Exception:
        return False
...
def _bitget_copy_query_symbol_settings() -> List[Dict[str, Any]]:
    ...
    for product_type in (BITGET_COPY_PRODUCT_TYPE, BITGET_PRODUCT_TYPE):
        try:
            ...
            continue
    if errors:
        raise RuntimeError("copy_symbols_failed:" + " | ".join(errors[:2]))
    if isinstance(data, list):
        return [dict(row or {}) for row in data]
    if isinstance(data, dict):
...
def _bitget_uta_account_assets() -> Dict[str, Any]:
    data = _bitget_private_request("GET", "/api/v3/account/assets")
    if isinstance(data, list):
        return {"assets": [dict(row or {}) for row in data]}
    return dict(data or {})


def _bitget_uta_positions(category: str) -> List[Dict[str, Any]]:
    category_map = {
        "USDT-FUTURES": "USDT",
        "COIN-FUTURES": "COIN",
        "USDC-FUTURES": "USDC",
    }
    normalized_category = category_map.get(str(category or "").strip().upper(), str(category or "").strip().upper())
    data = _bitget_private_request("GET", "/api/v3/position/current-position", query={"category": normalized_category})
    payload = dict(data or {})
    return [dict(row or {}) for row in list(payload.get("list") or [])]
...
def _bitget_uta_place_order(
    ...
    *,
    ...
    payload: Dict[str, Any] = {
        ...
        "posSide": pos_side,
        "reduceOnly": "yes" if reduce_only else "no",
        "clientOid": "bot-uta-{}-{}".format(int(time.time() * 1000), symbol_contract_id(symbol)[:12]),
    }
    ...
    if not reduce_only:
        ...
        if init_tp > 0:
            payload["takeProfit"] = str(init_tp)
        if init_sl > 0:
            payload["stopLoss"] = str(init_sl)
    data = _bitget_private_request("POST", "/api/v3/trade/place-order", body=payload)
    ...
    *,
    ...
    if BITGET_USE_COPY_TRADER_API:
        ...
        for product_type in _bitget_order_product_types():
            ...
            try:
                data = _bitget_private_request("POST", "/api/v2/mix/order/place-order", body=payload)
                order_id = str((dict(data or {})).get("orderId") or (dict(data or {})).get("clientOid") or "")
                return {"id": order_id, "client_oid": str((dict(data or {})).get("clientOid") or ""), "info": data}
            except Exception as exc:
                if _is_copy_permission_error(exc):
                    copy_mode_blocked = True
                    errors.append("mix_v2_place_order_permission:" + str(exc)[:220])
                    continue
                errors.append("mix_v2_place_order:" + str(exc)[:220])
        # Optional UTA fallback can be enabled explicitly.
        if BITGET_ENABLE_UTA_ENDPOINTS:
            try:
                return _bitget_uta_place_order(
                    symbol,
                    order_type,
                    side,
                    qty,
                ...
                errors.append("uta_v3_place_order:" + str(exc)[:220])
        if copy_mode_blocked and (BITGET_COPY_STRICT_ONLY or not BITGET_COPY_PERMISSION_FALLBACK_TO_SWAP):
            raise RuntimeError("copy_permission_missing:need trace/future permission")
        if BITGET_COPY_STRICT_ONLY or not BITGET_COPY_PERMISSION_FALLBACK_TO_SWAP:
...
def sync_positions_once() -> None:
    ...
    try:
        ...
        copy_permission_blocked = False
        copy_trace_permission_blocked = False
        copy_permission_reason = ""
        if BITGET_USE_COPY_TRADER_API:
            copy_tracks_error = ""
            copy_equity_error = ""
            copy_accounts_error = ""
            authority_ok, authority_reason = _copy_authority_ready()
            if not authority_ok:
                set_backend_thread(
                    "positions",
                    "running",
                    "Copy API authority check failed.",
                    error=str(authority_reason)[:220],
                )
                return
            try:
                ...
                tracks = []
                copy_tracks_error = str(exc)[:180]
                copy_permission_blocked = _is_copy_permission_error(exc)
                copy_trace_permission_blocked = _is_trace_permission_error(exc)
                copy_permission_reason = copy_tracks_error
            for row in tracks:
            ...
            if equity <= 0:
                try:
                    equity = _bitget_copy_total_equity_usdt()
                except Exception as exc:
                    equity = 0.0
                    copy_equity_error = str(exc)[:180]
                    copy_permission_blocked = copy_permission_blocked or _is_copy_permission_error(exc)
                    copy_trace_permission_blocked = copy_trace_permission_blocked or _is_trace_permission_error(exc)
                    if not copy_permission_reason and _is_copy_permission_error(exc):
                        copy_permission_reason = copy_equity_error
            if equity <= 0 and not BITGET_COPY_STRICT_ONLY:
                try:
                    account_rows = _bitget_private_request(
                        "GET",
                        "/api/v2/mix/account/accounts",
                        query={"productType": BITGET_COPY_PRODUCT_TYPE},
                    )
                    raw_rows = [dict(row or {}) for row in list(account_rows or [])]
                    target_rows = [row for row in raw_rows if _bitget_account_row_matches_hint(row)]
                    if BITGET_ACCOUNT_HINT_TOKENS and not target_rows:
                        sample_labels = [label for label in [_bitget_account_row_label(row) for row in raw_rows] if label][:3]
                        copy_accounts_error = "account_name_not_matched:{} available={}".format(
                            BITGET_ACCOUNT_NAME_HINT,
                            " || ".join(sample_labels),
                        )[:180]
                    for row in (target_rows if target_rows else raw_rows):
                        item = dict(row or {})
                        if str(item.get("marginCoin") or "").upper() != BITGET_MARGIN_COIN:
                            continue
                        equity = max(
                            safe_float(item.get("accountEquity"), 0.0),
                            safe_float(item.get("available"), 0.0) + safe_float(item.get("locked"), 0.0),
                        )
                        if equity > 0:
                            break
                except Exception as exc:
                    copy_accounts_error = str(exc)[:180]
                    copy_permission_blocked = copy_permission_blocked or _is_copy_permission_error(exc)
                    copy_trace_permission_blocked = copy_trace_permission_blocked or _is_trace_permission_error(exc)
                    if not copy_permission_reason and _is_copy_permission_error(exc):
                        copy_permission_reason = copy_accounts_error
            if (not BITGET_COPY_STRICT_ONLY) and copy_permission_blocked and not positions_payload:
                _append_ccxt_positions(positions_payload, _fetch_standard_swap_positions())
            if (not BITGET_COPY_STRICT_ONLY) and equity <= 0 and copy_permission_blocked:
                equity = _fetch_standard_swap_equity()
            if copy_permission_blocked and not positions_payload:
                set_backend_thread(
                    "positions",
                    "running",
                    "Copy API permission restricted; cannot read copy-trade positions.",
                    error=(
                        "copy permission blocked; reason={}".format(copy_permission_reason or "unknown_permission_error")
                    )[:220],
                )
            if equity <= 0 and not positions_payload:
                set_backend_thread(
                    "positions",
                    "running",
                    "Copy-trader sync active but balance/positions unavailable.",
                    error="copy_tracks={} copy_equity={} mix_accounts={}".format(
                        copy_tracks_error,
                        copy_equity_error,
                        copy_accounts_error,
                    )[:220],
        ...
        if BITGET_USE_COPY_TRADER_API and copy_permission_blocked:
            summary = "Synced {} active positions (copy permission limited).".format(len(positions_payload))
            if copy_trace_permission_blocked:
                err_text = "缺少 trace read/trace write 或 future order read/write 權限；帶單資訊無法完整讀取。"
            else:
                err_text = "帶單 API 權限受限（非 trace 專屬），請檢查 key/口令/IP 白名單與 API 類型。"
            if copy_permission_reason:
                err_text = "{} reason={}".format(err_text, copy_permission_reason[:120])
            set_backend_thread(
                "positions",
                "running",
                summary,
    ...
    try:
        if BITGET_USE_COPY_TRADER_API:
            ...
            try:
                ...
                close_errors.append(str(exc)[:180])
            if not order:
                try:
                    track_no = ""
                    for row in _bitget_copy_fetch_current_tracks(symbol):
                        if symbol_key(str(row.get("symbol") or "")) == symbol_key(symbol):
                            track_no = str(row.get("trackingNo") or "")
                            break
                    order = _bitget_copy_close_tracking(symbol, track_no)
                except Exception as exc:
                    close_errors.append(str(exc)[:180])
            if not order:
                raise RuntimeError("close_failed:" + " | ".join(close_errors[:2]))
        else:
            close_side = "sell" if str(side).lower() == "long" else "buy"
            order = _create_contract_order(symbol, "market", close_side, qty, None, reduce_only=True)
...
def api_bitget_diag():
    ...
    if BITGET_ENABLE_UTA_ENDPOINTS:
        try:
            assets = _bitget_uta_account_assets()
            usdt_equity = safe_float(assets.get("usdtEquity"), 0.0)
            _push("uta_assets", True, "usdtEquity={}".format(usdt_equity))
        except Exception as exc:
            _push("uta_assets", False, str(exc))
        try:
            rows = _bitget_uta_positions(BITGET_PRODUCT_TYPE)
            _push("uta_positions", True, "count={}".format(len(rows)))
        except Exception as exc:
            _push("uta_positions", False, str(exc))
    else:
        _push("uta_assets", True, "skipped (BITGET_ENABLE_UTA_ENDPOINTS=0)")
        _push("uta_positions", True, "skipped (BITGET_ENABLE_UTA_ENDPOINTS=0)")
    try:
    ...
    for probe_type in _bitget_order_product_types():
        try:
            rows = _bitget_private_request(
                "GET",
                "/api/v2/mix/account/accounts",
                query={"productType": probe_type},
            )
            count = len(list(rows or []))
            _push("mix_accounts_{}".format(probe_type), True, "count={}".format(count))
        except Exception as exc:
            _push("mix_accounts_{}".format(probe_type), False, str(exc))
    try:
