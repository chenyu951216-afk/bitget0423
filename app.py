...
def build_openai_constraints(
    ...
    *,
    ...
    if source == "short_gainers":
        strategy_lane = "countertrend_reversal"
...
def build_prebreakout_signals(
    ...
    for row in list(leaderboard.get("top_candidates") or [])[:PREBREAKOUT_TOP_PICK]:
        ...
        forced_side = side_hint if side_hint in ("long", "short") else None
...
def choose_review_candidates(
    ...
    virtual_long_general = safe_int(snapshot.get("long_total_general"), 0)
    virtual_short_general = safe_int(snapshot.get("short_total_general"), 0)
        ...
        for row in pool_rows:
            ...
            if lane == "prebreakout":
                ...
                if virtual_general_total >= MAX_OPEN_POSITIONS:
                    ...
                    candidates.append(picked)
                    continue
                side_total = virtual_long_general if side == "long" else virtual_short_general
                if side_total >= MAX_SAME_DIRECTION_POSITIONS:
                    picked["force_openai_recheck"] = False
                    picked["local_gate_ok"] = False
            ...
            if bool(gate.get("ok", False)):
                if lane == "prebreakout":
                    ...
                    if side == "long":
                        virtual_long_general += 1
                    else:
                        virtual_short_general += 1
    return candidates
...
def maybe_run_openai(
    ...
    global OPENAI_TRADE_STATE
    reviewed_candidates = choose_review_candidates(general_top, short_gainers, prebreakout_top, pending_advice_signals)
    ...
    for reviewed in reviewed_candidates:
        ...
        if source == "prebreakout_scanner":
            ...
            candidate["scanner_features"] = dict(reviewed.get("prebreakout_scanner_features") or {})
            candidate["raw_candidate_payload"] = dict(reviewed.get("prebreakout_raw_candidate_payload") or {})
            candidate["side_hint"] = str(reviewed.get("prebreakout_side_hint") or reviewed.get("scanner_side") or "")
            candidate["reason_tags"] = list(reviewed.get("prebreakout_reason_tags") or [])[:8]
        try:
            with OPENAI_LOCK:
            ...
            if decision:
                ...
                continue
            for pool in (general_top, short_gainers, prebreakout_top):
...
def perform_scan_cycle() -> None:
    ...
    try:
        ...
        set_backend_thread(
            "scan",
            "running",
            "Updated {} ranked symbols, {} short-gainer reversal candidates, and {} pre-breakout candidates.".format(
                len(general_top),
                len(short_gainer_top),
                len(prebreakout_top),
            ),
