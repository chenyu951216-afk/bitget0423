...
OBSERVE_MANUAL_WINDOW_SEC = max(300, int(float(env_or_blank("OBSERVE_MANUAL_WINDOW_SEC", "1800") or 1800)))
...
def build_openai_constraints(symbol: str = "", reference_price: float = 0.0, *, expected_side: str = "", side_lock: bool = False) -> Dict[str, Any]:
...
def prune_expired_observe_trackers() -> List[str]:
    ...
    with REVIEW_LOCK:
        for symbol, row in list(REVIEW_TRACKER.items()):
            if str((row or {}).get("status") or "") != "observe":
                continue
    ...
    with REVIEW_LOCK:
        ...
        if action == "observe":
            ...
            if not str(decision.get("recheck_reason") or "").strip():
                decision["recheck_reason"] = "目前為觀察單，僅支援手動發送重審；30 分鐘後會自動移除。"
    ...
    with REVIEW_LOCK:
        ...
        for symbol, row in list(REVIEW_TRACKER.items()):
            ...
            if not str(pending[symbol].get("recheck_reason") or "").strip():
                pending[symbol]["recheck_reason"] = "目前觀察單僅支援手動重送 AI，30 分鐘到期自動移除。"
...
def place_order_from_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with ORDER_LOCK:
        ...
        instruction = str(decision.get("bot_instruction") or "").strip().upper()
        scanner_side = str(signal.get("scanner_side") or signal.get("side") or "").lower()
        decision_side = str(decision.get("trade_side") or "").lower()
        if decision_side not in ("long", "short"):
            decision_side = scanner_side if scanner_side in ("long", "short") else str(signal.get("side") or "long").lower()
