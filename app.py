        ...
        if fvg_price is not None:
            ...
            try:
                ...
                register_fvg_order(
                    ...
                    extra_meta={'strategy_source': 'openai' if openai_plan else 'fvg', 'order_kind': preferred_order_type}
                )
                if openai_plan:
...
def _attach_openai_pending_reference(sig, advice, trigger_reason):
    ...
    ref.update({
        ...
        'summary': 'Previous OpenAI advice is now triggered; re-check before any order.',
        'setup': str(advice.get('entry_plan') or advice.get('watch_note') or '')[:220],
        'risk': str(advice.get('reason_to_skip') or '')[:220],
        'note': '{} | trigger={} price={} invalidation={} entry={} sl={} tp={} recheck={}'.format(
            trigger_reason,
...
"tp_mult": tp_m,
                            ...
                            'candidates': [
                                {
                                    ...
                                    'price': s.get('price'),
                                    'status': s.get('status') or 'queued_for_openai',
                                }
            ...
            if bool(OPENAI_TRADE_CONFIG.get('enabled', True) and OPENAI_API_KEY):
                ...
                if pending_watch_checked or pending_trigger_rows:
                    print("OpenAI pending advice watch checked={} triggered={}".format(pending_watch_checked, len(pending_trigger_rows)))
            _order_rows = sorted(
                list(top10) + list(short_gainer_signals or []) + pending_trigger_rows,
                key=lambda x:(
                    1 if x.get('_pending_openai_advice') else 0,
                    1 if str(x.get('candidate_source') or x.get('source') or '') == 'short_gainers' else 0,
            ...
            if pos_cnt < MAX_OPEN_POSITIONS:
                ...
                for rank_index, best in enumerate(top10_for_order):
                    ...
                    if openai_mode_active:
                        ...
                        has_short_gainer_slot = (not is_short_gainer_review) or (openai_short_gainers_this_scan < OPENAI_SHORT_GAINERS_MAX_PER_SCAN)
                        has_regular_slot = openai_sent_this_scan < openai_sends_per_scan and has_short_gainer_slot
                        has_pending_slot = openai_pending_rechecks_this_scan < OPENAI_TRADE_PENDING_RECHECK_MAX_PER_SCAN
                        ...
                        can_send_openai = (
                            ...
                            (
                                (openai_pending_triggered and has_pending_slot) or
                                (has_regular_slot and not openai_recently_sent)
                            )
                        )
                        if can_send_openai:
