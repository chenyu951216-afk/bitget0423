    ...
    try:
        ...
        if BITGET_USE_COPY_TRADER_API:
            ...
            if copy_permission_blocked and not positions_payload:
                set_backend_thread(
                    "positions",
                    "running",
                    "Copy API key lacks trace permissions; cannot read copy-trade positions.",
                    error=(
                        "需要在 Bitget API 權限啟用 trace read / trace write（帶單讀寫）;"
                        " 目前僅能讀一般合約或行情。"
                    )[:220],
                )
        ...
        if BITGET_USE_COPY_TRADER_API and copy_permission_blocked:
            set_backend_thread(
                "positions",
                "running",
                "Synced {} active positions (copy permission limited).".format(len(positions_payload)),
                error=(
                    "缺少 trace read/trace write 權限，帶單帳戶資訊與帶單持倉無法完整讀取。"
                    " 請在 Bitget API 啟用帶單讀寫權限。"
                )[:220],
            )
        else:
